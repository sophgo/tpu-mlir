...
if [ "$1" = "op" ]; then
    ...
    $REGRESSION_PATH/main_entry.py --test_set maskrcnn --concise_log
    ...
    $REGRESSION_PATH/main_entry.py --test_type basic --test_set script model --concise_log
elif [ "$1" = "check_utests" ]; then
    echo "::RUN check tests and unit tests."
    cmake --build ${BUILD_PATH} --target check-tpumlir
elif [ "$1" = "cuda" ]; then
    echo "::RUN CUDA test."
    #$REGRESSION_PATH/main_entry.py --test_type basic --test_set cuda --concise_log
else
    echo "::RUN Other test."
fi
