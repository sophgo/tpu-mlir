...
class SgdWeightOpt:
    ...
    def update_alpha(self, iter, op, alpha, mini_batch, unsigned=False):
        ...
        alpha_new = np.clip(self.cal_alpha(iter, alpha, self.loss[op], self.grd[op], unsigned),
                            -np.log(11), -np.log(1.2 / 1.1 - 1))
...
class LearningAdaWeight:
    ...
    def cal_beta(self, iter):
        if iter < self.num_sample * self.epoch * self.beta_warmup:
            ...
            return int(self.beta_end + 0.5 * (self.beta_start - self.beta_end) *
                       (1.0 + np.cos((iter - self.num_sample * self.epoch * self.beta_warmup) /
                                     (self.num_sample * self.epoch *
                                      (1.0 - self.beta_warmup)) * np.pi)))
    ...
    def get_finetune_ops(self, excepts):
        ...
        for op in top_ops:
            if top_ops[op].type in LEARNING_WEIGHT_OPERATION and top_ops[
    ...
    def quant_requant_weight(self, op, bitwidth=8, hard=False):
        ...
        if not hard:
            if self.parser.get_op_type_by_op_name(op) == 'top.Conv':
                weight = (np.clip(np.floor(weight_tmp) + self.rec_sig(alpha), qmin, qmax).reshape(
            ...
            if self.parser.get_op_type_by_op_name(op) == 'top.Conv':
                weight = (
                    np.clip(np.floor(weight_tmp) +
                            (alpha >= 0).astype(np.float32), qmin, qmax).reshape(shape[0], -1) *
                    scales[:, None]).reshape(shape)
            elif self.parser.get_op_type_by_op_name(op) == 'top.MatMul':
                weight = np.clip(
                    np.floor(weight_tmp) + (alpha >= 0).astype(np.float32), qmin, qmax) * scales
    ...
    def quant_requant_weight_orig(self, op, bitwidth=8):
        ...
        if self.parser.get_op_type_by_op_name(op) == 'top.Conv':
            self.module.set_tensor(self.finetune_layer_weights[op],
                                   (weight.reshape(shape[0], -1) * scales[:, None]).reshape(shape))
        elif self.parser.get_op_type_by_op_name(op) == 'top.MatMul':
    ...
    def learning_one(self, epoch, op, total):
        ...
        for loop in np.arange(self.num_sample):
            ...
            if self.parser.get_op_type_by_op_name(op) == 'top.Conv':
                grd_w1 = (grd_w1.reshape(shape[0], -1) *
                          ((self.weights_scales[op] / qmax)[:, None])).reshape(shape)
            elif self.parser.get_op_type_by_op_name(op) == 'top.MatMul':
            ...
            if (epoch * self.num_sample + loop + 1) % self.mini_batch == 0:
                self.alpha[op] = self.opt.update_alpha(epoch * self.num_sample + loop, op,
                                                       self.alpha[op], self.mini_batch)
        ...
        if epoch == self.epoch - 1:
            ...
            if True:
                self.loger.logging(
                    f'{op} use trained weight {self.post_loss[op]} vs {self.pre_loss[op]}')
                ...
                self.loger.logging(
                    f'{op} do not use learned weight {self.post_loss[op]} vs {self.pre_loss[op]}')
    ...
    def adjust_weight(self, op, bitwidth=8):
        ...
        '''
        if self.parser.get_op_type_by_op_name(op) == 'top.Conv':
            weight = (np.clip((np.floor(w_reshape / ((self.weights_scales[op] / qmax)[:, None])) +
                               np.where(p >= 0, 1.0, 0.0).astype(np.float32)), qmin, qmax) *
                      ((self.weights_scales[op] / qmax)[:, None])).reshape(shape)
        elif self.parser.get_op_type_by_op_name(op) == 'top.MatMul':
            weight = (np.clip((np.floor(w_reshape / (self.weights_scales[op] / qmax)) +
                               np.where(p >= 0, 1.0, 0.0).astype(np.float32)), qmin, qmax) *
                      (self.weights_scales[op] / qmax)).reshape(shape)
        else:
            print("not support!")
            sys.exit(1)
        return weight
