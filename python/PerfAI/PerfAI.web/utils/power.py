...
perf_tiu_columns = [
    ...
    'Cmd Id', 'Data Type', 'uArch Rate', 'SimPower(w)'
]
perf_dma_columns = [
    'Core', 'Ip', 'Function Type', 'Function Name', 'Start Cycle', 'End Cycle', 'Asic Cycle',
    'Cmd Id', 'Direction', 'DDR Bandwidth(GB/s)', 'L2M Bandwidth(GB/s)', 'gdma_pw(w)', 'sdma_pw(w)',
    'cdma_pw(w)', 'noc_pw(w)', 'ddr_pw(w)', 'l2m_pw(w)'
]
...
def gdma_calculate_sim_power(row):
    ...
    try:
        ...
        if direction == 'DDR->LMEM':
            ...
            ddr_pw = round((ddr_bw / 546 * (12.6 - 7.2)) + (ddr_bw / 546 * (30.9 - 0.88)),
                           4)  # DDR read
            return pd.Series({
                'gdma_pw(w)': gdma_pw,
                'sdma_pw(w)': 0,
            ...
            noc_pw = round(bw / 546 * (16 - 8), 4)
            ddr_pw = round((ddr_bw / 546 * (12.6 - 7.2)) + (ddr_bw / 546 * (20.3 - 0.88)),
                           4)  # DDR write
            return pd.Series({
                'gdma_pw(w)': gdma_pw,
                'sdma_pw(w)': 0,
...
def sdma_calculate_sim_power(row):
    ...
    try:
        ...
        if direction == 'DDR->DDR':
            ...
            ddr_pw = round((ddr_bw / 546 * (12.6 - 7.2)) * 2 + (ddr_bw / 546 * (30.9 - 0.88)) +
                           (ddr_bw / 546 * (20.3 - 0.88)), 4)  # DDR read+write
            return pd.Series({
                'gdma_pw(w)': 0,
                'sdma_pw(w)': ip_pw,
                'cdma_pw(w)': 0,
            ...
            l2m_pw = round((l2m_bw / 128 * (0.25 - 0.11)) + (l2m_bw / 128 * (0.29 - 0.11)),
                           4)  # L2M read+write
            return pd.Series({
                'gdma_pw(w)': 0,
                'sdma_pw(w)': ip_pw,
...
def generate_power(dirpath):
    ...
    if len(cdma_instances) > 0:
        ip_list.append('cdma')
    max_corenum = max(len(tiu_instances), len(gdma_instances), len(sdma_instances),
                      len(cdma_instances))
    perf_power_dict = {}
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(read_data_and_cal_power, idx, tiu_instances, gdma_instances,
                            sdma_instances, cdma_instances) for idx in range(max_corenum)
        ]
...
def prepareJson(compiler_version, output_name, ip_list, pld_dict_to_js, perf_dict_to_js, tiu_header,
                dma_header, perf_tiu_columns, perf_dma_columns, pld_concat_timemark,
...
def prepareOutput(dirpath, version, name, html_path, out_path):
    ...
    prepareJson(version, name, ip_list, {}, perf_power_dict, [], [], perf_tiu_columns,
                perf_dma_columns, {}, {}, out_path)
