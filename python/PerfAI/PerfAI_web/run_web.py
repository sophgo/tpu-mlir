...
def run_web(reginfo_dir, name, file_path, power, version, layerinfo_dir):
    ...
    htmlfiles = [
        ...
        os.path.join(templates_dir, 'result.html')
    ]
    for f in htmlfiles:
    ...
    generate_jsfile(reginfo, name, out_path, file_path, layerinfo_dir)
    if power:
...
if __name__ == '__main__':
    ...
    parser.add_argument('reginfo_dir',
                        type=str,
                        default='',
    ...
    parser.add_argument('--power',
                        ...
                        type=bool,
                        default=False,
                        help='Input True for generating power charts. Default is False')
    parser.add_argument(
        '--version',
        '-v',
        type=str,
        default='',
        help=
        'AI compiler commit ID. Please provide this info if you need to present it on power chart.')
    args = parser.parse_args()
    run_web(args.reginfo_dir, args.name, args.file, args.power, args.version, args.layerinfo_dir)
