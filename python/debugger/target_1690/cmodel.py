...
class BM1690Runner(CModelRunner):
    lib_name = "libtpuv7_emulator.so"
    ...
    def __init__(self, memory_size, base_addr: Tuple[int, int]):
        ...
        super().__init__()
        self.base_addr = base_addr  # a list contains several base_addr, each one is manully set in BM1690Context, which represent the base_addr of 2 tags {TAG_WEIGHT,TAG_ACTIVATION} above.
        self.core_num = 8
...
class Memory(CModelMemory):
    ...
    def clear_memory(self):
        ...
        lut = np.array(BM1690Runner.gen_lookup_table(), np.uint32).view(np.uint8)
    ...
    def get_data(self, value_ref: ValueRef):
        ...
        # so the bm1690 backend config the same absolute address for LMEM on different cores, since a dma cmd would transfer
