...
def sCONV_converter(context: "BM1688Context", reg: sCONV_reg):
    FP16 = 1
    FP32 = 2
    BF16 = 5
    INT4 = 6
...
def sMM_converter(context: "BM1688Context", reg: sMM_reg):
    L_row = reg.opd0_n
...
def sMM2_converter(context: "BM1688Context", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "BM1688Context", reg: sCMP_reg):
...
def sSFU_converter(context: "BM1688Context", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sLIN_converter(context: "BM1688Context", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
...
def sVC_converter(context: "BM1688Context", reg: sVC_reg):
...
def sAR_converter(context: "BM1688Context", reg: sAR_reg):
...
def sCW_sBC_converter(context: "BM1688Context", reg: sCW_sBC_reg):
    if reg.tsk_eu_typ in (0, 1):
...
def sRQ_sDQ_converter(context: "BM1688Context", reg: sRQ_sDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
...
def sSG_converter(context: "BM1688Context", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def DMA_tensor_0x000__converter(context: "BM1688Context", reg: DMA_tensor_0x000__reg):
    NONE = 0
...
def DMA_masked_select_converter(context: "BM1688Context", reg: DMA_masked_select_reg):
    shape = tuple(reg[f"src_{d}size"] for d in "nchw")
    opd0 = dict(
...
def DMA_general_converter(context: "BM1688Context", reg: DMA_general_reg):
...
def DMA_nonzero_converter(context: "BM1688Context", reg: DMA_nonzero_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
...
def DMA_gather_converter(context: "BM1688Context", reg: DMA_gather_reg):
...
def DMA_reverse_converter(context: "BM1688Context", reg: DMA_reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h8, reg.src_start_addr_l32),
