...
def sCONV_converter(context: "SGTPUV8Context", reg: sCONV_reg):
    INT8 = 0
    FP32 = 2
    INT16 = 3
    INT32 = 4
...
def sMM_converter(context: "SGTPUV8Context", reg: sMM_reg):
    L_row = reg.opd0_n
...
def sMM2_converter(context: "SGTPUV8Context", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "SGTPUV8Context", reg: sCMP_reg):
...
def sSFU_converter(context: "SGTPUV8Context", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sLIN_converter(context: "SGTPUV8Context", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
...
def sVC_converter(context: "SGTPUV8Context", reg: sVC_reg):
...
def sAR_converter(context: "SGTPUV8Context", reg: sAR_reg):
...
def sPorD_converter(context: "SGTPUV8Context", reg: sPorD_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    # round mm
    opd0 = dict(
        address=reg.opd0_addr,
...
def sCW_sBC_converter(context: "SGTPUV8Context", reg: sCW_sBC_reg):
    if reg.tsk_eu_typ in (0, 1):
...
def sRQ_sDQ_converter(context: "SGTPUV8Context", reg: sRQ_sDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
...
def sSG_converter(context: "SGTPUV8Context", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSGL_converter(context: "SGTPUV8Context", reg: sSGL_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
def SYS_converter(context: "SGTPUV8Context", reg):
    """
    0: intr barrier
...
def dma_reg_fmt_base(reg: Union[DMA_tensor_0x000__reg, DMA_matrix_reg]):
    if isinstance(reg, DMA_tensor_0x000__reg):
        ...
        addr = [
            (reg.src_start_addr_l32, reg.src_start_addr_h13),
...
def DMA_tensor_0x000__converter(context: "SGTPUV8Context", reg: DMA_tensor_0x000__reg):
    NONE = 0
...
def DMA_matrix_converter(context: "SGTPUV8Context", reg: DMA_matrix_reg):
    res0, attr, opd0 = dma_reg_fmt_base(reg)
    l, r = memmap[MType.R]
    s_addr = opd0["address"]
...
def DMA_masked_select_converter(context: "SGTPUV8Context", reg: DMA_masked_select_reg):
    shape = tuple(reg[f"src_{d}size"] for d in "nchw")
    opd0 = dict(
...
def DMA_general_converter(context: "SGTPUV8Context", reg: DMA_general_reg):
...
def DMA_cw_transpose_converter(context: "SGTPUV8Context", reg: DMA_cw_transpose_reg):
...
def DMA_nonzero_converter(context: "SGTPUV8Context", reg: DMA_nonzero_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
...
def DMA_gather_converter(context: "SGTPUV8Context", reg: DMA_gather_reg):
    return dma_gather_base(context, reg)


def DMA_scatter_converter(context: "SGTPUV8Context", reg: DMA_scatter_reg):
...
def DMA_reverse_converter(context: "SGTPUV8Context", reg: DMA_reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
...
def sDMA_sys_converter(context: "SGTPUV8Context", reg: sDMA_sys_reg):
    ...
    elif reg.cmd_special_function > 7:
