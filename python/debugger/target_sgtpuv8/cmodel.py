...
class SGTPUV8Runner(CModelRunner):
    lib_name = "libcmodel_sgtpuv8.so"
    ...
    # ENGINE code, must same as defined in /nntoolchain/TPU1686/sgtpuv8/spec/include/engine_type.h
    ENGINE_GDMA = 1
    ENGINE_HAU = 2

    def __init__(self, memory_size, base_addr: Tuple[int, int]):
        # always init with 1 cores
        super().__init__()
    ...
    def init_memory(self, memory_size: int):
        ...
        for i in range(self.core_num):
            ...
            self.lib.set_cur_nodechip_idx(i)
            self.lib.atomic_set_base_ddr(base_idx, base_addr, 3, self.ENGINE_GDMA)
            g_npu_num = self.lib.tpu_npu_num()
...
class Memory(CModelMemory):
    ...
    def _local_mem_to_numpy(self, memref: MemRef, core_id):
        ...
        # referece: TPU1688/sgtpuv8/cmodel/src/cmodel_common.cpp
        ...
        def data_view_int4(shape, stride):
            ...
            for nidx in range(0, shape[0]):
                ...
                for cidx in range(0, shape[1]):
                    ...
                    c_offset = ((start_npu_idx + cidx) / info.NPU_SHIFT) * stride[1]
    ...
    def clear_memory(self):
        ...
        lut = np.array(SGTPUV8Runner.gen_lookup_table(), np.uint32).view(np.uint8)
