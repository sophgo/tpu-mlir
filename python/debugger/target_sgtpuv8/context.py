...
from .cmodel import SGTPUV8Runner
...
class SGTPUV8Context(BModelContext):
    ...
    def get_runner(self, memory_size: int) -> CModelRunner:
        assert self.using_cmodel, "SGTPUV8 currently only support cmodel mode"
        if self._runner is None:
            self._runner = SGTPUV8Runner(memory_size, self.base_addr)
        return self._runner
