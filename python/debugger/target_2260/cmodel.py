...
class SG2260Runner(CModelRunner):
    lib_name = "libcmodel_sg2260.so"
    # tag , set as will from 0 to 31
    TAG_WEIGHT = 0  # aka coeff
    TAG_ACTIVATION = 1  # aka computation space

    # ENGINE code, must same as defined in /workspace/nntoolchain/TPU1686/sg2260/spec/include/engine_type.h
    ENGINE_GDMA = 1
    # ENGINE_HAU = 2
    ...
    def __init__(self, memory_size, base_addr: Tuple[int, int]):
        ...
        super().__init__()
        self.base_addr = base_addr  # a list contains 2 base_addr, each one is manully set in memmap file, which represent the base_addr of 2 tags {TAG_WEIGHT,TAG_ACTIVATION} above.
        self.core_num = 8
    ...
    def init_memory(self, memory_size: int):
        self.memory_list = []
        base_idx = (ctypes.c_int32 * 2)(
            self.TAG_WEIGHT, self.TAG_ACTIVATION
        ...
        for i in range(self.core_num):
            ...
            DDR = c_array_to_ndarray(
                self.lib.get_global_memaddr(i), memory_size
            )  # initialize 4GB bytes
            print("DDR:", DDR)
            print(DDR.shape)

            L2M = c_array_to_ndarray(self.lib.get_l2_sram(i), 2**27)  # 128MB

            LMEM = c_array_to_ndarray(
                self.lib.get_local_mem(i).contents.raw_ptr, (64, 16, 1024 * 16)
            )
            print(self.lib.get_local_mem(i).contents.raw_ptr)
            print("LMEM:", LMEM)
            print(LMEM.shape)

            SMEM = c_array_to_ndarray(
                self.lib.get_static_memaddr_by_node(i), (64 * 1024,)
            )
            print("SMEM:", SMEM)
            print(SMEM.shape)
            print("END Init memory")
            self.memory_list.append(Memory(LMEM, DDR, SMEM))
    ...
    def DDR(self, core_id):
        return self.memory_list[core_id].DDR

    def LMEM(self, core_id):
        return self.memory_list[core_id].LMEM

    def SMEM(self, core_id):
        return self.memory_list[core_id].SMEM
...
class Memory(MemoryBase):
    ...
    def clear_memory(self):
        self.DDR.fill(0)
        self.LMEM.fill(0)
        lut = np.array(SG2260Runner.gen_lookup_table(), np.uint32).view(np.uint8)
