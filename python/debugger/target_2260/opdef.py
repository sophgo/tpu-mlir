        ...
        if self.operands == []:
            if self.attribute:
                ...
                return (
                    ...
                    + f"(%B{self.cmd.cmd_id_dep}C{ci}, %msg{msg_id})"
                    + attribute
                )
            else:
                return self.description
...
class sg_op(TiuCmdOp):
    ...
    eu_type = {
        ...
        # 2: "sg.pl_gather_rec", # not sure if is currently supported, not writen (Y) in SG2260 xlsx
...
class dma_lossy_compress(DmaCmdOp):
...
class dma_lossy_decompress(DmaCmdOp):
