...
def sCONV_converter(context: "SG2260Context", reg: sCONV_reg):
    INT8 = 0
    FP16 = 1
    FP32 = 2
    INT16 = 3
...
def sMM_converter(context: "SG2260Context", reg: sMM_reg):
    L_row = reg.opd0_n
...
def sMM2_converter(context: "SG2260Context", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "SG2260Context", reg: sCMP_reg):
...
def sSFU_converter(context: "SG2260Context", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sLIN_converter(context: "SG2260Context", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
...
def sCW_sBC_converter(context: "SG2260Context", reg: sCW_sBC_reg):
    if reg.tsk_eu_typ in (0, 1):
...
def sRQ_sDQ_converter(context: "SG2260Context", reg: sRQ_sDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
...
def sSG_converter(context: "SG2260Context", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSGL_converter(context: "SG2260Context", reg: sSGL_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
def SYS_TR_ACC_converter(context: "SG2260Context", reg):
    def int2bin(x, width):
...
def SYS_converter(context: "SG2260Context", reg):
    des_imm = reg.imm
...
def DMA_matrix_converter(context: "SG2260Context", reg: DMA_matrix_reg):
    res0, attr, opd0 = dma_reg_fmt_base(reg)
...
def DMA_masked_select_converter(context: "SG2260Context", reg: DMA_masked_select_reg):
    shape = tuple(reg[f"src_{d}size"] for d in "nchw")
    opd0 = dict(
...
def DMA_cw_transpose_converter(context: "SG2260Context", reg: DMA_cw_transpose_reg):
    lane_mask = reg.localmem_mask_h32 * 2**32 + reg.localmem_mask_l32
...
def DMA_nonzero_converter(context: "SG2260Context", reg: DMA_nonzero_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
...
def DMA_scatter_converter(context: "SG2260Context", reg: DMA_scatter_reg):
...
def DMA_reverse_converter(context: "SG2260Context", reg: DMA_reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
...
def DMA_compress_converter(context: "SG2260Context", reg: DMA_compress_reg):
    return [] * 3


@opparam_converter_regitstry("DMA_decompress ")
def DMA_decompress_converter(context: "SG2260Context", reg: DMA_decompress__reg):
    return [] * 3


@opparam_converter_regitstry("sDMA_sys")
def sDMA_sys_converter(context: "SG2260Context", reg: sDMA_sys_reg):
