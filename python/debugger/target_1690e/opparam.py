...
if TYPE_CHECKING:
    from .context import BM1690EContext
...
def sCONV_converter(context: "BM1690EContext", reg: sCONV_reg):
    INT8 = 0
    FP16 = 1
    FP32 = 2
    INT16 = 3
...
def sCONV_BW_converter(context: "BM1690EContext", reg: sCONV_BW_reg):
    INT8 = 0
    FP16 = 1
    FP32 = 2
    INT16 = 3
...
def sMM_converter(context: "BM1690EContext", reg: sMM_reg):
    L_row = reg.opd0_n
...
def sMM2_converter(context: "BM1690EContext", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "BM1690EContext", reg: sCMP_reg):
...
def sSFU_converter(context: "BM1690EContext", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sLIN_converter(context: "BM1690EContext", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
...
def sVC_converter(context: "BM1690EContext", reg: sVC_reg):
...
def sAR_converter(context: "BM1690EContext", reg: sAR_reg):
...
@opparam_converter_regitstry("sAR_cvt")
def sAR_cvt_converter(context: "BM1690EContext", reg: sAR_cvt_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    # round mm
    opd0 = dict(
        address=reg.opd0_addr,
...
def sPorD_converter(context: "BM1690EContext", reg: sPorD_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    # round mm
    opd0 = dict(
        address=reg.opd0_addr,
...
@opparam_converter_regitstry("sPorD_ROI")
def sPorD_ROI_converter(context: "BM1690EContext", reg: sPorD_ROI_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    # round mm
    opd0 = dict(
        address=reg.opd0_addr,
...
def cw_trans_reg_format(context, reg: sBC_CWT_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def bc_reg_format(context, reg: sBC_CWT_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
@opparam_converter_regitstry("sBC&CWT")
def sBC_CWT_converter(context: "BM1690EContext", reg: sBC_CWT_reg):
    if reg.tsk_eu_typ in (0, 1):
...
@opparam_converter_regitstry("sRQDQ")
def sRQDQ_converter(context: "BM1690EContext", reg: sRQDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
...
def sSG_converter(context: "BM1690EContext", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSGL_converter(context: "BM1690EContext", reg: sSGL_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
def SYS_TR_WR_converter(context: "BM1690EContext", reg):
    ...
    des_imm1_field1 = int(des_imm1[48:56], 2)
    des_imm1_field2 = int(des_imm1[56:64], 2)
    tgcr.setter(reg.reg_idx1, des_imm1_field1)
    tgcr.setter(reg.reg_idx2, des_imm1_field2)
    attrs = dict(
...
def SYS_converter(context: "BM1690EContext", reg):
    des_imm = reg.imm
    msg_id = (des_imm & 0xff) | (((des_imm >> 32) & 0x3) << 8)
    cnt = (des_imm >> 8) & 0x7f
    attrs = {}
...
def DMA_tensor_converter(context: "BM1690EContext", reg: DMA_tensor_reg):
    NONE = 0
...
def DMA_matrix_converter(context: "BM1690EContext", reg: DMA_matrix_reg):
    res0, attr, opd0 = dma_reg_fmt_base(reg)
...
def DMA_Mask_Select_converter(context: "BM1690EContext", reg: DMA_Mask_Select_reg):
    shape = tuple(reg[f"src_{d}size"] for d in "nchw")
    opd0 = dict(
...
def DMA_general_converter(context: "BM1690EContext", reg: DMA_general_reg):
...
# def DMA_cw_transpose_converter(context: "BM1690EContext", reg: DMA_cw_transpose_reg):
#     lane_mask = reg.localmem_mask_h32 * 2**32 + reg.localmem_mask_l32
...
def DMA_Nonzero_converter(context: "BM1690EContext", reg: DMA_Nonzero_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
...
@opparam_converter_regitstry("DMA_gather_scatter")
def dma_gather_scatter_base(context, reg: DMA_gather_scatter_reg):
    lane_mask = reg.localmem_mask_h32 * 2**32 + reg.localmem_mask_l32
...
# def DMA_gather_converter(context: "BM1690EContext", reg: DMA_gather_reg):
#     return dma_gather_base(context, reg)

# @opparam_converter_regitstry("DMA_scatter")
# def DMA_scatter_converter(context: "BM1690EContext", reg: DMA_scatter_reg):
...
def DMA_reverse_converter(context: "BM1690EContext", reg: DMA_Reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
...
@opparam_converter_regitstry("DMA_sys")
def sDMA_sys_converter(context: "BM1690EContext", reg: DMA_sys_reg):
...
# def CDMA_send_converter(context: "BM1690EContext", reg: CDMA_send_reg):
#     rao = cdma_operand_base(context, reg)
#     rao[1]["psum_op"] = "rd+wr" if reg.psum_op else "wo"
#     return rao

# @opparam_converter_regitstry("CDMA_read")
# def CDMA_read_converter(context: "BM1690EContext", reg: CDMA_read_reg):
#     return cdma_base(context, reg)

# @opparam_converter_regitstry("CDMA_write")
# def CDMA_write_converter(context: "BM1690EContext", reg: CDMA_write_reg):
#     return cdma_base(context, reg)

# @opparam_converter_regitstry("CDMA_general")
# def CDMA_general_converter(context: "BM1690EContext", reg: CDMA_general_reg):
...
# def CDMA_receive_converter(context: "BM1690EContext", reg: CDMA_receive_reg):
...
def DMA_lossy_compress_converter(context: "BM1690EContext", reg: DMA_Lossy_compress_reg):
    rao = cdma_operand_base(context, reg)
    rao[1]["psum_op"] = "rd+wr" if reg.psum_op else "wo"
    return rao
...
# def CDMA_lossy_decompress_converter(context: "BM1690EContext", reg: CDMA_lossy_decompress_reg):
#     rao = cdma_operand_base(context, reg)
#     rao[1]["psum_op"] = "rd+wr" if reg.psum_op else "wo"
#     return rao

# @opparam_converter_regitstry("sCDMA_sys")
# def sCDMA_sys_converter(context: "BM1690EContext", reg: sCDMA_sys_reg):
#     constant_value_l32 = reg.constant_value_l32
#     attr = {}
#     if reg.cmd_special_function in (3, 4, 5, 6):
#         msg_id = constant_value_l32 & 0x3FF
#         cnt = (constant_value_l32 >> 42) & 0x7F
#         attr = dict(msg_id=msg_id, cnt=cnt)
...
# def CDMA_tcp_send_converter(context: "BM1690EContext", reg: CDMA_tcp_send_reg):
#     return [] * 3

# @opparam_converter_regitstry("CDMA_tcp_rcv ")
# def CDMA_tcp_rcv_converter(context: "BM1690EContext", reg: CDMA_tcp_rcv_reg):
#     return [] * 3
