...
class BM1690ERunner(CModelRunner):
    lib_name = "libtpuv7.1_emulator.so"
    ...
    # ENGINE code, must same as defined in /nntoolchain/TPU1686/sg2260e/spec/include/engine_type.h
    ENGINE_GDMA = 1

    def __init__(self, memory_size, base_addr: Tuple[int, int]):
        # always init with 8 cores
        super().__init__()
        self.base_addr = base_addr  # a list contains several base_addr, each one is manully set in BM1690EContext, which represent the base_addr of 2 tags {TAG_WEIGHT,TAG_ACTIVATION} above.
        self.core_num = 4
        ...
        self.lib = lib
        self.init_memory(memory_size, L2M_size=0x1000000)
...
class Memory(CModelMemory):
    ...
    def clear_memory(self):
        ...
        lut = np.array(BM1690ERunner.gen_lookup_table(), np.uint32).view(np.uint8)
    ...
    def get_data(self, value_ref: ValueRef):
        ...
        # so the BM1690E backend config the same absolute address for LMEM on different cores, since a dma cmd would transfer
