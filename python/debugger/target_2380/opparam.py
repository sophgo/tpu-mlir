...
def sCONV_converter(context: "SG2380Context", reg: sCONV_reg):
    FP16 = 1
    FP32 = 2
    BF16 = 5
    INT4 = 6
...
def sMM_converter(context: "SG2380Context", reg: sMM_reg):
    ...
    add_lmem_addr_tag([opd0, opd1, opd2])
    add_lmem_addr_tag(res0)
...
def sMM2_converter(context: "SG2380Context", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "SG2380Context", reg: sCMP_reg):
    ...
    if reg.tsk_eu_typ in (23, 24, 26):
        ...
        rets = [res0]
    add_lmem_addr_tag([opd0, opd1, opd2, opd3])
    add_lmem_addr_tag(rets)
...
def sSFU_converter(context: "SG2380Context", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
    ...
    if reg.tsk_eu_typ == 17:
        ...
        opd_num = 2
    add_lmem_addr_tag([x for x in (opd0, opd1)[:opd_num]])
    add_lmem_addr_tag(res0)
...
def sLIN_converter(context: "SG2380Context", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
    ...
    if reg.tsk_eu_typ == 1:
        opd_num = 3
    add_lmem_addr_tag([x for x in (opd0, opd1, opd2)[:opd_num]])
    add_lmem_addr_tag(res0)
...
def sVC_converter(context: "SG2380Context", reg: sVC_reg):
...
def sAR_converter(context: "SG2380Context", reg: sAR_reg):
...
def sPorD_converter(context: "SG2380Context", reg: sPorD_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    # round mm
    opd0 = dict(
        address=reg.opd0_addr,
...
def sCW_sBC_converter(context: "SG2380Context", reg: sCW_sBC_reg):
    if reg.tsk_eu_typ in (0, 1):
...
def sRQ_sDQ_converter(context: "SG2380Context", reg: sRQ_sDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    gsize = 0
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSG_converter(context: "SG2380Context", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSGL_converter(context: "SG2380Context", reg: sSGL_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
def SYS_converter(context: "SG2380Context", reg):
    """
    0: intr barrier
...
def DMA_tensor_0x000__converter(context: "SG2380Context", reg: DMA_tensor_0x000__reg):
    NONE = 0
...
def DMA_matrix_converter(context: "SG2380Context", reg: DMA_matrix_reg):
    res0, attr, opd0 = dma_reg_fmt_base(reg)
...
def DMA_masked_select_converter(context: "SG2380Context", reg: DMA_masked_select_reg):
    shape = tuple(reg[f"src_{d}size"] for d in "nchw")
    opd0 = dict(
...
def DMA_general_converter(context: "SG2380Context", reg: DMA_general_reg):
...
def DMA_cw_transpose_converter(context: "SG2380Context", reg: DMA_cw_transpose_reg):
    lane_mask = reg.localmem_mask_h32 * 2**32 + reg.localmem_mask_l32
...
def DMA_nonzero_converter(context: "SG2380Context", reg: DMA_nonzero_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
...
def DMA_gather_converter(context: "SG2380Context", reg: DMA_gather_reg):
    return dma_gather_base(context, reg)


@opparam_converter_regitstry("DMA_scatter ")
def DMA_scatter_converter(context: "SG2380Context", reg: DMA_scatter_reg):
...
def DMA_reverse_converter(context: "SG2380Context", reg: DMA_reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
...
def DMA_compress_converter(context: "SG2380Context", reg: DMA_compress_reg):
    return [] * 3


@opparam_converter_regitstry("DMA_decompress ")
def DMA_decompress_converter(context: "SG2380Context", reg: DMA_decompress__reg):
    return [] * 3


@opparam_converter_regitstry("sDMA_sys")
def sDMA_sys_converter(context: "SG2380Context", reg: sDMA_sys_reg):
    """
    000: chain end +
    001: nop
    010: sys_tr_wr
