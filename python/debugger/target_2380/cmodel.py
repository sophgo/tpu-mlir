...
class SG2380Runner(CModelRunner):
    lib_name = "libcmodel_sg2380.so"
    # tag, range from 0 to 31, set as defined in /nntoolchain/TPU1686/sg2380/firmware_base/src/fullnet/nodechip_multi_fullnet.c
    TAG_WEIGHT = 1  # coeff
    TAG_ACTIVATION = 2  # neuron
    TAG_LMEM = 3  # lmem

    # ENGINE code, must same as defined in /nntoolchain/TPU1686/sg2380/spec/include/engine_type.h
    ENGINE_GDMA = 1
    ENGINE_HAU = 2

    def __init__(self, memory_size, base_addr: Tuple[int, int]):
        # always init with 4 cores
        super().__init__()
    ...
    def init_memory(self, memory_size: int):
        ...
        for i in range(self.core_num):
            ...
            LMEM.append(
                c_array_to_ndarray(self.lib.get_local_mem(i).contents.raw_ptr, (32, 16, 1024 * 8)))
...
class Memory(CModelMemory):
    ...
    def _local_mem_to_numpy(self, memref: MemRef, core_id):
        ...
        # referece: TPU1686/sg2380/cmodel/src/cmodel_common.cpp
    ...
    def clear_memory(self):
        ...
        lut = np.array(SG2380Runner.gen_lookup_table(), np.uint32).view(np.uint8)
