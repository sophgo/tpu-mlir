...
class BM1684XEContext(BModelContext):
    MemRef = MemRef
    device = Target.BM1684XE
    ...
    def get_runner(self, memory_size: int) -> CModelRunner:
        if self.using_cmodel:
            from .cmodel import BM1684XERunner as BM1684XECModel
            if self._runner is None:
                self._runner = BM1684XECModel(memory_size)
            runner = self._runner
        elif self.using_soc:
            from .soc import BM1684XERunner as BM1684XESoc
            if self._runner is None:
                self._runner = BM1684XESoc(memory_size)
            runner = self._runner
        else:
            from .pcie import BM1684XERunner as BM1684XEPcie
            if self._runner is None:
                self._runner = BM1684XEPcie(memory_size)
            runner = self._runner
