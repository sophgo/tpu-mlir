...
class BM1684X2Info:
    ...
    def __init__(self) -> None:
        self.lib_name = "libbackend_bm1684x2.so"
        self._lib = None
    ...
    def EU_NUM(self, dtype: DType) -> int:
        ...
        TYPED_EU_NUM = {
            ...
            DType.f8: BASE_EU_NUM,
            DType.f8e5m2: BASE_EU_NUM,
            DType.f8e4m3: BASE_EU_NUM,
    ...
    def CUBE_NUM(self, dtype: DType) -> int:
        ...
        TYPED_CUBE_NUM = {
            ...
            DType.f8: BASE_CUBE_NUM,
            DType.f8e5m2: BASE_CUBE_NUM,
            DType.f8e4m3: BASE_CUBE_NUM,
...
# /TPU1686/bm1684x2/spec/include/memmap.h
memmap = LazyMemmap()
...
class MemRef(MemRefBase):
    ...
    @functools.lru_cache()
    def r_addr(self):
        if self.mtype in [MType.UNKNOWN, MType.G, MType.L, MType.R]:
            return (self.context.fix_addr(self.address) - self.context.memmap[self.mtype][0])

        r_addr = self.address & 0xFFFFFF  # remain 24 bit (16 Mb) as local offset
        return r_addr
...
ExecutionUnitNumber = 16 * 256 // 2 // 8  # NPU_NUM=16 for BM1684X2
