...
if TYPE_CHECKING:
    from .context import BM1684X2Context
...
def sCONV_converter(context: "BM1684X2Context", reg: sCONV_reg):
    INT8 = 0
    FP16 = 1
    FP32 = 2
    INT16 = 3
...
@opparam_converter_regitstry("sCONV_DW")
def sCONV_DW_converter(context: "BM1684X2Context", reg: sCONV_DW_reg):
    INT8 = 0
    FP16 = 1
    FP32 = 2
    INT16 = 3
...
def sMM2_converter(context: "BM1684X2Context", reg: sMM2_reg):
    L_row = reg.res0_c
    L_col = reg.opd1_c
...
def sCMP_converter(context: "BM1684X2Context", reg: sCMP_reg):
...
def sSFU_converter(context: "BM1684X2Context", reg: sSFU_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sLIN_converter(context: "BM1684X2Context", reg: sLIN_reg):
    shape = tuple(reg[f"res0_{d}"] for d in "nchw")
    c = shape[1]
    opd0 = dict(
        address=reg.opd0_addr,
...
def sVC_converter(context: "BM1684X2Context", reg: sVC_reg):
...
def sAR_converter(context: "BM1684X2Context", reg: sAR_reg):
    res0_n, res0_c, res0_h, res0_w = reg.res0_n, reg.res0_c, reg.res0_h, reg.res0_w

    # Build result
    res0 = dict(
        address=reg.res0_addr,
        dtype=(reg.opt_res0_prec, reg.opt_res0_sign),
        shape=(res0_n, res0_c, res0_h, res0_w),
        layout=reg.short_res0_str,
    )

    # Add stride if available (stride fields are 18 bits, so check if non-zero)
    if reg.res0_n_str > 0 or reg.res0_c_str > 0 or reg.res0_h_str > 0 or reg.res0_w_str > 0:
        res0["stride"] = (
            reg.res0_n_str,
            reg.res0_c_str,
            reg.res0_h_str,
    ...
    # opd0 - always present for arithmetic operations
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
        shape=(res0_n, res0_c, res0_h, res0_w),
    ...
    if reg.opd0_n_str > 0 or reg.opd0_c_str > 0 or reg.opd0_h_str > 0 or reg.opd0_w_str > 0:
        opd0["stride"] = (
            reg.opd0_n_str,
            reg.opd0_c_str,
            reg.opd0_h_str,
    ...
    operands.append(get_value(context, **opd0))

    # opd1 - present if tsk_opd_num >= 2
    if reg.tsk_opd_num >= 2:
        ...
        if reg.opd1_n_str > 0 or reg.opd1_c_str > 0 or reg.opd1_h_str > 0 or reg.opd1_w_str > 0:
            opd1["stride"] = (
                reg.opd1_n_str,
                reg.opd1_c_str,
                reg.opd1_h_str,
        ...
        operands.append(get_value(context, **opd1))

    # opd2 - present if tsk_opd_num >= 3
    if reg.tsk_opd_num >= 3:
        opd2 = dict(
            address=reg.opd2_addr,
            dtype=(reg.opt_opd2_prec, reg.opt_opd2_sign),
            shape=(res0_n, res0_c, res0_h, res0_w),
            layout=reg.short_opd0_str,  # opd2 uses same layout as opd0 typically
            is_const=reg.opt_opd2_const,
        )
        operands.append(get_value(context, **opd2))

    attr = dict(op=reg.tsk_eu_typ)
    results = [get_value(context, **res0)]

    return (results, attr, operands)


@opparam_converter_regitstry("sPRNG")
def sPRNG_converter(context: "BM1684X2Context", reg: sPRNG_reg):
    res0 = dict(
        address=reg.res0_addr,
        dtype=(reg.opt_res0_prec, 0),
        shape=(1, reg.res0_c, 1, reg.res0_w),
        layout=Layout.alignEU,
    )
    opd0 = dict(
    ...
    attr = dict(
        opt_res_add=bool(reg.opt_res_add),
        tsk_lane_num=reg.tsk_lane_num,
    )
    operands = [get_value(context, **opd0)]
    results = [get_value(context, **res0), get_value(context, **res1)]
    return (results, attr, operands)


@opparam_converter_regitstry("sREDUCE")
def sREDUCE_converter(context: "BM1684X2Context", reg: sREDUCE_reg):
    n, c, h, w = reg.res0_n, reg.res0_c, reg.res0_h, reg.res0_w
    opd0_n, opd0_c, opd0_h, opd0_w = reg.opd0_n, reg.opd0_c, reg.opd0_h, reg.opd0_w
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
        shape=(opd0_n, opd0_c, opd0_h, opd0_w),
        layout=reg.short_opd0_str,
    )
    res0 = dict(
        address=reg.res0_addr,
...
def sPorD_converter(context: "BM1684X2Context", reg: sPorD_reg):
...
def sCW_sBC_converter(context: "BM1684X2Context", reg: sCW_sBC_reg):
    if reg.tsk_eu_typ in (0, 1):
...
def sRQ_sDQ_converter(context: "BM1684X2Context", reg: sRQ_sDQ_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=(reg.opt_opd0_prec, reg.opt_opd0_sign),
...
def sSG_converter(context: "BM1684X2Context", reg: sSG_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
...
def sSGL_converter(context: "BM1684X2Context", reg: sSGL_reg):
    n, c, h, w = (reg[f"res0_{d}"] for d in "nchw")
    opd0 = dict(
        address=reg.opd0_addr,
        dtype=DType(reg.opt_res0_prec),
...
@opparam_converter_regitstry("SYS_TR_WR")
def SYS_TR_WR_converter(context: "BM1684X2Context", reg: SYS_TR_WR_reg):
...
def SYS_converter(context: "BM1684X2Context", reg):
    des_imm = reg.imm
...
def dma_reg_fmt_base(reg: Union[DMA_tensor_reg, DMA_matrix_reg]):
    if isinstance(reg, DMA_tensor_reg):
        addr = [
    ...
    lane_mask = reg.default_lanemask1 * 2**32 + reg.default_lanemask2
    opd0 = dict(
...
def DMA_tensor_converter(context: "BM1684X2Context", reg: DMA_tensor_reg):
    NONE = 0
...
def DMA_matrix_converter(context: "BM1684X2Context", reg: DMA_matrix_reg):
    res0, attr, opd0 = dma_reg_fmt_base(reg)
...
def DMA_masked_select_converter(context: "BM1684X2Context", reg: DMA_masked_select_reg):
    # src_hsize and src_wsize are split into low and high parts
    src_hsize = reg.src_hsize | (reg.src_hsize_h16 << 16)
    src_wsize = reg.src_wsize | (reg.src_wsize_h16 << 16)
    shape = (reg.src_nsize, reg.src_csize, src_hsize, src_wsize)
    opd0 = dict(
...
def DMA_general_converter(context: "BM1684X2Context", reg: DMA_general_reg):
    # Calculate copy length from src_wsize (combining l16 and h16 parts)
    copy_len = reg.src_wsize_l16 | (reg.src_wsize_h16 << 16)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
        dtype=DType(reg.src_data_format),
        shape=(copy_len, ),
...
# DMA_cw_transpose converter removed - register class not in regdef.py


@opparam_converter_regitstry("DMA_nonzero")
def DMA_nonzero_converter(context: "BM1684X2Context", reg: DMA_nonzero_reg):
...
def dma_gather_base(context, reg: DMA_gather_reg):
    lane_mask = reg.default_lanemask1 * 2**32 + reg.default_lanemask2
    # src_csize is split into l16 and h16 parts
    # src_csize = src_csize_l16 | (src_csize_h16 << 16)
    # where src_csize_h16 is in the upper 16 bits of src_hstride_or_src_csize_h16
    c = reg.src_csize_or_src_csize_l16 | (reg.src_hstride_or_src_csize_h16 & 0xFFFF0000)
    h = reg.src_hsize
    w = reg.src_wsize
    d_h = reg.dst_hsize
    if reg.nchw_copy:
        d_h = h
    stride = (c * h * w, h * w, w, 1)
    # src_hstride is in the lower 16 bits of src_hstride_or_src_csize_h16
    src_hstride = reg.src_hstride_or_src_csize_h16 & 0xFFFF
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
        dtype=DType(reg.src_data_format),
        shape=(1, c, h, w),
        stride=(0, reg.src_cstride, src_hstride, 1),
        layout=Layout.stride,
    )
    # dst_csize is split into l16 and h16 parts
    # dst_csize = dst_csize_l16 | (dst_csize_h16 << 16)
    # where dst_csize_h16 is in the upper 16 bits of dst_hstride_or_dst_csize_h16
    dst_c = reg.dst_csize_or_dst_csize_l16 | (reg.dst_hstride_or_dst_csize_h16 & 0xFFFF0000)
    # dst_hstride is in the lower 16 bits of dst_hstride_or_dst_csize_h16
    dst_hstride = reg.dst_hstride_or_dst_csize_h16 & 0xFFFF
    # index_csize is split into l16 and h16 parts
    index_c = reg.index_csize_l16 | (reg.index_csize_h16 << 16)
    res0 = dict(
        address=dma_addr(reg.dst_start_addr_h13, reg.dst_start_addr_l32),
        dtype=DType(reg.src_data_format),
        shape=(1, max(c, index_c), d_h, w),
        stride=(0, reg.dst_cstride, dst_hstride, 1),
        layout=Layout.DMAstride(lane_mask),
    )
    opd1 = dict(
        address=dma_addr(reg.index_start_addr_h13, reg.index_start_addr_l32),
        dtype=DType.ui32,
        shape=(1, index_c, d_h, 1),
...
def DMA_gather_converter(context: "BM1684X2Context", reg: DMA_gather_reg):
    return dma_gather_base(context, reg)


@opparam_converter_regitstry("DMA_scatter")
def DMA_scatter_converter(context: "BM1684X2Context", reg: DMA_scatter_reg):
...
def DMA_reverse_converter(context: "BM1684X2Context", reg: DMA_reverse_reg):
    n, c, h, w = (reg[f"src_{d}size"] for d in "nchw")
    stride = (c * h * w, h * w, w, 1)
    opd0 = dict(
        address=dma_addr(reg.src_start_addr_h13, reg.src_start_addr_l32),
...
# DMA_compress and DMA_decompress converters removed - register classes not in regdef.py


@opparam_converter_regitstry("sDMA_sys")
def sDMA_sys_converter(context: "BM1684X2Context", reg: sDMA_sys_reg):
...
# copied from BM1690/opparam.py, need to verify
...
def CDMA_send_converter(context: "BM1684X2Context", reg: CDMA_send_reg):
...
def CDMA_receive_converter(context: "BM1684X2Context", reg: CDMA_receive_reg):
...
def CDMA_read_converter(context: "BM1684X2Context", reg: CDMA_read_reg):
    return cdma_base(context, reg)


@opparam_converter_regitstry("CDMA_write")
def CDMA_write_converter(context: "BM1684X2Context", reg: CDMA_write_reg):
    return cdma_base(context, reg)


@opparam_converter_regitstry("CDMA_general")
def CDMA_general_converter(context: "BM1684X2Context", reg: CDMA_general_reg):
...
def sCDMA_sys_converter(context: "BM1684X2Context", reg: sCDMA_sys_reg):
