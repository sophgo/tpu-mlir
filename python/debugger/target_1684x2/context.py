...
from .cmodel import BM1684X2Runner
...
def GET_LMEM_START_ADDR(core_id):
    return 0x24000000 + core_id * info.LMEM_SIZE


def GET_SMEM_START_ADDR(core_id):
    return 0x24400000 + core_id * 0x800000


class BM1684X2Context(BModelContext):
    ...
    base_addr = [0, 2**29, GET_LMEM_START_ADDR]  # align with runtime.
    ...
    def merge_instruction(cls, tiu: List[BaseTpuCmd], dma: List[BaseTpuCmd]) -> List[BaseTpuCmd]:
        ...
        def fix_tgcr_cmd_id_dp(tiu_cmd: List[BaseTpuCmd]):
            for i, v in enumerate(tiu_cmd):
                if isinstance(v.reg, SYS_TR_WR_reg):
    ...
    def get_runner(self, memory_size: int) -> CModelRunner:
        ...
        if self._runner is None:
            self._runner = BM1684X2Runner(memory_size, self.base_addr)
        return self._runner
