...
class InternVL3Converter(LlmConverter):
    ...
    def vision_block(self, vit_mlir, idx: int, in_op):
        ...
        def vision_attention(in_op):
            ...
            norm1_op = top.LayerNormOp(T([1, 1025, self.vit_hidden_size]),
                                       ...
                                       loc=L(norm1),
                                       ip=ip).output
            weight_op2 = vit_mlir.create_weight_op(attn_qkv + ".weight", [self.vit_hidden_size, self.vit_hidden_size * 3])
            weight_op3 = vit_mlir.create_weight_op(attn_qkv + ".bias", [1, 1, self.vit_hidden_size * 3])
            ...
            qk_softmax_op = top.SoftmaxOp(T([1, self.vit_num_heads, 1025, 1025]),
                                         ...
                                         ip=ip).output
            v_permute_op = top.PermuteOp(T([1, self.vit_num_heads, 1025, self.vit_head_dim]), v_squeeze_op, order=[0,2,1,3], loc=L(attn_qkv+".v.permute"), ip=ip).output
            qkv_mm_op = top.MatMulOp(T([1, self.vit_num_heads, 1025, self.vit_head_dim]),
                                    ...
                                    ip=ip).output
            o_permute_op = top.PermuteOp(T([1, 1025, self.vit_num_heads, self.vit_head_dim]), qkv_mm_op, order=[0,2,1,3], loc=L(attn_proj+".o.permute"), ip=ip).output
            o_reshape_op = top.ReshapeOp(T([1, 1025, self.vit_hidden_size]), o_permute_op, shape=[1,1025,self.vit_hidden_size], loc=L(attn_proj+".o.reshape"), ip=ip).output
            weight_op4 = vit_mlir.create_weight_op(attn_proj + ".weight", [self.vit_hidden_size, self.vit_hidden_size])
            weight_op5 = vit_mlir.create_weight_op(attn_proj + ".bias", [1, 1, self.vit_hidden_size])
            ...
            o_mul_op = top.MulOp(T([1, 1025, self.vit_hidden_size]), [o_mm_op, weight_op6], loc=L(attn_proj+".o.mul"), ip=ip).output
            new_op = top.AddOp(T([1, 1025, self.vit_hidden_size]), [in_op, o_mul_op], loc=L(attn_proj+".add"),
                               ip=ip).output
            return new_op
        ...
        def vision_mlp(in_op):
            ...
            norm2_op = top.LayerNormOp(T([1, 1025, self.vit_hidden_size]),
                                       ...
                                       loc=L(norm2),
                                       ip=ip).output
            weight_op2 = vit_mlir.create_weight_op(mlp + "1.weight", [self.vit_hidden_size, self.vit_intermediate_size])
            weight_op3 = vit_mlir.create_weight_op(mlp + "1.bias", [1, 1, self.vit_intermediate_size])
            mlp_up_op = top.MatMulOp(T([1, 1025, self.vit_intermediate_size]),
                                    ...
                                    loc=L(mlp + "1"),
                                    ip=ip).output
            active_op = top.GELUOp(T([1, 1025, self.vit_intermediate_size]), mlp_up_op, loc=L(mlp + ".gelu"),
                            ip=ip).output
            weight_op4 = vit_mlir.create_weight_op(mlp + "2.weight", [self.vit_intermediate_size, self.vit_hidden_size])
            ...
            mul_op = top.MulOp(T([1, 1025, self.vit_hidden_size]), [mlp_down_op, weight_op6], loc=L(mlp + ".mul"), ip=ip).output
            new_op = top.AddOp(T([1, 1025, self.vit_hidden_size]), [in_op, mul_op], loc=L(mlp + ".add"),
                               ip=ip).output
            return new_op
    ...
    def gen_vit_mlir(self):
        ...
        # embedding
        weight_op0 = vit_mlir.create_weight_op(patch_embed + ".patch_embedding.weight",
        ...
        concat_op = top.ConcatOp(T([1, 1025, self.vit_hidden_size]),
        ...
        add_op = top.AddOp(T([1, 1025, self.vit_hidden_size]),
                           [concat_op, weight_op3],
        ...
        reshape_op2 = top.ReshapeOp(T([1, self.num_image_token, self.vit_intermediate_size]),
                                    ...
                                    ip=ip).output
        weight_op4 = vit_mlir.create_weight_op(merger + ".0.weight", [1, 1, self.vit_intermediate_size])
        weight_op5 = vit_mlir.create_weight_op(merger + ".0.bias", [1, 1, self.vit_intermediate_size])
        norm_op = top.LayerNormOp(T([1, self.num_image_token, self.vit_intermediate_size]),
                                    ...
                                    loc=L(merger + ".norm"),
                                    ip=ip).output
        weight_op6 = vit_mlir.create_weight_op(merger + ".1.weight", [self.vit_intermediate_size, self.hidden_size])
        ...
        mm_op0 = top.MatMulOp(T([1, self.num_image_token, self.hidden_size]),
                              ...
                              loc=L(merger + ".1"),
                              ip=ip).output
        active_op = top.GELUOp(T([1, self.num_image_token, self.hidden_size]), mm_op0, loc=L(merger + ".gelu"),
                               ip=ip).output
        weight_op8 = vit_mlir.create_weight_op(merger + ".3.weight", [self.hidden_size, self.hidden_size])
