...
class Qwen2VLConverter(LlmConverter):
    ...
    def vision_block(self, vit_mlir, id: int, in_op, cos_op, sin_op, mask_op):
        ...
        def vision_attention(in_op):
            new_weight = vit_mlir.create_weight_op(norm1 + ".weight", [1, self.embed_dim])
            norm1_op = top.RMSNormOp(T([self.num_patches, self.embed_dim]),
                                     in_op,
                                     new_weight,
        ...
        def vision_mlp(in_op):
            ...
            weight_op = vit_mlir.create_weight_op(norm2 + ".weight", [1, self.embed_dim])
            new_op = top.RMSNormOp(T(in_shape),
                                   in_op,
                                   weight_op,
                                   eps=self.rms_norm_eps,
                                   loc=self.get_loc(norm2, vit_mlir),
                                   ip=ip).output
    ...
    def gen_vit_mlir(self):
        ...
        new_op = top.RMSNormOp(T([self.num_patches, self.embed_dim]),
                               new_op,
                               new_weight,
                               eps=self.rms_norm_eps,
                               loc=L(merger_ln_q),
