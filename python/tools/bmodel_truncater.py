    ...
    for func_id, function in enumerate(MlirModel.functions):
        for reg_id, region in enumerate(function.regions):
            for block_id, block in enumerate(region.blocks):
                ...
                if block.subnet.run_mode == block.subnet.run_mode.TPU_STATIC:
                    if core_num > 1:
                        ...
                        for core_id, core_cmds in enumerate(block.cores_cmds):
                            if core_id == int(target_core_id):
                                for msgcore_id, msgcore in enumerate(
                                    core_cmds.msgcores
                                ):
                                    ...
                                    if start_id <= int(target_cmd_id) <= end_id:
                                        ...
                                        # print(f"boundary_type:{boundary_type}, start_id:{start_id}, end_id:{end_id}")
                                        break
                        ...
                        for core_id, core_cmds in enumerate(block.cores_cmds):
                            ...
                            if boundary_type == 0:
                                ...
                                new_all = sum(
                                    core_cmds.msgcores[:truncate_msgcore_id], []
                                )
                                if core_id == 0:
                            ...
                            # print(len(tiu_buf),len(dma_buf))
                            tiu_buf = padding_to_128_align(tiu_buf)
                            dma_buf = padding_to_128_align(dma_buf)
                            # print(len(tiu_buf),len(dma_buf))
                            # exit()

                            # print(bmodel.net[func_id].parameter[reg_id].sub_net[block_id].core_commands[core_id].gdma_tiu_commands)
                            # print(bmodel.net[func_id].parameter[reg_id].sub_net[block_id].core_commands[core_id].gdma_tiu_commands[0])
                            # print(bmodel.net[func_id].parameter[reg_id].sub_net[block_id].core_commands[core_id].gdma_tiu_commands[0].tiu_cmd)
                            # print(bmodel.net[func_id].parameter[reg_id].sub_net[block_id].core_commands[core_id].gdma_tiu_commands[0].tiu_num)
                            # print(len(bmodel.net[func_id].parameter[reg_id].sub_net[block_id].core_commands[core_id].gdma_tiu_commands))
                        ...
                        for cmd in block.cmds[0].all:
                            ...
                            if cmd.cmd_type == get_cmd_type(
                                targt_cmd_type
                            ) and cmd.cmd_id == int(target_cmd_id):
                                break
