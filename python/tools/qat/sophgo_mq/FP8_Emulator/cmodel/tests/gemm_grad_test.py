...
print('Forward :L2 distance output: ',
      torch.dist(z2.to(dtype=torch.float64, copy=True), z, 2).item())

(z2[0, 0] + z2[0, 1]).sum().backward()
(z[0, 0] + z[0, 1]).sum().backward()

print('Backward : L2 distance a_grad: ',
      torch.dist(a.grad.to(dtype=torch.float64, copy=True), a64.grad, 2).item())
print('Backward : L2 distance b_grad: ',
      torch.dist(b.grad.to(dtype=torch.float64, copy=True), b64.grad, 2).item())
print('Backward : L2 distance c_grad: ',
      torch.dist(c.grad.to(dtype=torch.float64, copy=True), c64.grad, 2).item())
