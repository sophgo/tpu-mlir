    ...
    def forward(ctx,
        ...
        if mode == 'NONE':
            ...
            if input.is_sparse:
                ...
                if inplace == True:
                    outputs = fpemu_cuda.forward(input._values().contiguous(), mode, size, inplace,
                ...
                size = input.nelement()
                outputs = fpemu_cuda.forward(input.contiguous(), mode, size, inplace, scale,
                                             blocknorm, blocksize)
                output = outputs[0]
...
def _forward_per_channel(ctx,
    ...
    if mode == 'NONE':
        ...
        if input.is_sparse:
            ...
            # input = input.cpu()
            size = input.nelement()
            outputs = fpemu_cuda.forward(input.contiguous(), mode, size, inplace, scale, blocknorm,
                                         blocksize)
            output = outputs[0]
