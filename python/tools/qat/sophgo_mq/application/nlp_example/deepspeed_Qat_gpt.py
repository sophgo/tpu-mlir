...
def train(model, epochs, traindataset, validation_dataloader):
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(trainloader):
            ...
            outputs = model_engine(b_input_ids, attention_mask=b_masks, labels=b_labels)
            loss = outputs[0]
            ...
            if (step + 1) % args.log_interval == 0:
                print("GPUs:%d:epoch:%d,loss:%.3f,device:%d\n" %
                      (torch.cuda.device_count(), epoch_i, running_loss /
                       (step + 1), model_engine.local_rank))
        # Measure how long this epoch took.
        print("GPUs:%d:epoch:%d,loss:%.3f,device:%d\n" %
              (torch.cuda.device_count(), epoch_i, running_loss / len(trainloader),
        ...
        with torch.no_grad():
            ...
            print("GPUs:%d:epoch:%d,loss:%.3f,device:%d\n" %
                  (torch.cuda.device_count(), epoch_i, running_loss1 / len(validation_dataloader),
                   model_engine.local_rank))
