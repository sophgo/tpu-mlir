...
def main(config_path):
    ...
    export_inputs['token_type_ids'] = torch.tensor(eval_datasets[0]['token_type_ids']).unsqueeze(
        0)  #.cuda()
    export_inputs['attention_mask'] = torch.tensor(eval_datasets[0]['attention_mask']).unsqueeze(
        0)  #.cuda()
    convert_deploy(
        model,
