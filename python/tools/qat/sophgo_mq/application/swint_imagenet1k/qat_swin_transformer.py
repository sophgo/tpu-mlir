...
def main(config_path):
    ...
    cali_batch = 20
    cali_batch_size = 10
    cali_dataset = torch.utils.data.Subset(train_dataset,
                                           indices=torch.arange(cali_batch_size * cali_batch))
    ...
    for t in range(epoch_num):
        ...
        total_loss = train_loop(train_loader, model_copy, loss_fn, optimizer, lr_scheduler, t + 1,
                                total_loss)
    ...
    for t in range(epoch_num):
        print(f"Epoch {t+1}/{epoch_num}\n-------------------------------")
        total_loss = train_loop(train_loader, model_prepared, loss_fn, optimizer1, lr_scheduler1,
                                t + 1, total_loss)
