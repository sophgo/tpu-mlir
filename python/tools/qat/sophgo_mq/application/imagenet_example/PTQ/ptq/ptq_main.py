...
parser.add_argument('--data_path', metavar='DIR', help='path to dataset', required=True)
...
parser.add_argument('--chip',
                    type=str,
                    choices=['BM1688', 'BM1684X', 'BM1690', 'academic'],
                    default='BM1690')
parser.add_argument('--quantmode',
                    type=str,
                    choices=['weight_activation', 'weight_only'],
                    default='weight_activation')
parser.add_argument('--cali_batch_num',
                    default=16,
                    type=int,
                    metavar='N',
                    help='set calibration batch num (default: 16)')
...
parser.add_argument('--quantize_type',
                    metavar='DIR',
                    help='set quantize_type',
                    type=str,
                    default='naive_ptq')
parser.add_argument('--deploy', action='store_true')
parser.add_argument('--cpu', action='store_true', help='use cpu to quant')
...
def deploy(model, args):
    net_type = 'CNN'
    output_path = './' if not hasattr(args, 'output_path') else args.output_path
    model_name = args.arch
    deploy_to_qlinear = False if not hasattr(args, 'deploy_to_qlinear') else args.deploy_to_qlinear

    convert_deploy(model,
                   net_type, {'input': [1, 3, 224, 224]},
...
def main():
    ...
    if args.quantize_type == 'advanced_ptq':
        ...
        if hasattr(args, 'reconstruction'):
            model = ptq_reconstruction(model, cali_data, args.reconstruction)
