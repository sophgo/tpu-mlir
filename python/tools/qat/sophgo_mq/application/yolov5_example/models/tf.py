...
from models.common import (C3, SPP, SPPF, Bottleneck, BottleneckCSP, C3x, Concat, Conv, CrossConv,
                           DWConv, DWConvTranspose2d, Focus, autopad)
...
class TFConv(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, w=None):
        ...
        conv = keras.layers.Conv2D(filters=c2,
                                   kernel_size=k,
                                   strides=s,
                                   ...
                                   bias_initializer='zeros' if hasattr(w, 'bn') else
                                   keras.initializers.Constant(w.conv.bias.numpy()))
...
class TFDWConv(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, act=True, w=None):
        ...
        conv = keras.layers.DepthwiseConv2D(kernel_size=k,
...
class TFDWConvTranspose2d(keras.layers.Layer):
    ...
    def call(self, inputs):
        return tf.concat([m(x) for m, x in zip(self.conv, tf.split(inputs, self.c1, 3))],
                         3)[:, 1:-1, 1:-1]
...
class TFFocus(keras.layers.Layer):
    ...
    def call(self, inputs):  # x(b,w,h,c) -> y(b,w/2,h/2,4c)
        ...
        inputs = [
            inputs[:, ::2, ::2, :], inputs[:, 1::2, ::2, :], inputs[:, ::2, 1::2, :],
            inputs[:, 1::2, 1::2, :]
        ]
...
class TFBottleneck(keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 g=1,
                 e=0.5,
                 w=None):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
...
class TFBottleneckCSP(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, w=None):
        ...
        self.m = keras.Sequential(
            [TFBottleneck(c_, c_, shortcut, g, e=1.0, w=w.m[j]) for j in range(n)])
...
class TFUpsample(keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 scale_factor,
                 mode,
                 w=None):  # warning: all arguments needed including 'w'
        super().__init__()
...
def parse_model(d, ch, model, imgsz):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [
                nn.Conv2d, Conv, DWConv, DWConvTranspose2d, Bottleneck, SPP, SPPF, MixConv2d, Focus,
                CrossConv, BottleneckCSP, C3, C3x
        ]:
            c1, c2 = ch[f], args[0]
