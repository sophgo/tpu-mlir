...
class Albumentations:
    ...
    def __init__(self):
        ...
        try:
            ...
            LOGGER.info(prefix + ', '.join(f'{x}'.replace('always_apply=False, ', '')
                                           for x in T if x.p))
    ...
    def __call__(self, im, labels, p=1.0):
        if self.transform and random.random() < p:
            ...
            im, labels = new['image'], np.array([[c, *b]
...
def hist_equalize(im, clahe=True, bgr=False):
    ...
    return cv2.cvtColor(yuv,
                        cv2.COLOR_YUV2BGR if bgr else cv2.COLOR_YUV2RGB)  # convert YUV image to RGB
    ...
    if p and n:
        ...
        for j in random.sample(range(n), k=round(p * n)):
            ...
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                ...
                cv2.drawContours(im_new, [segments[j].astype(np.int32)], -1, (255, 255, 255),
                                 cv2.FILLED)
    ...
    try:
        ...
        if augment:  # Resize and crop
            ...
            if auto_aug:
                ...
                if jitter > 0:
                    color_jitter = (float(
                        jitter), ) * 3  # repeat value for brightness, contrast, satuaration, 0 hue
