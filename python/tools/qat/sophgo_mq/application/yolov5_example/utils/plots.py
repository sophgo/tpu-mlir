...
from utils.general import (CONFIG_DIR, FONT, LOGGER, Timeout, check_font, check_requirements,
                           clip_coords, increment_path, is_ascii, threaded, try_except, xywh2xyxy,
                           xyxy2xywh)
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            blocks = torch.chunk(x[0].cpu(), channels,
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    try:  # color histogram bars by class
        [y[2].patches[i].set_color([x / 255 for x in colors(i)])
         for i in range(nc)]  # known issue #3195
    except Exception:
        pass
