...
def reshape_classifier_output(model, n=1000):
    ...
    name, m = list(
        (model.model if hasattr(model, 'model') else model).named_children())[-1]  # last module
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                ...
                s_in, s_out = (tuple(x.shape) if isinstance(x, torch.Tensor) else 'list'
                               for x in (x, y))  # shapes
                p = sum(x.numel()
                        for x in m.parameters()) if isinstance(m, nn.Module) else 0  # parameters
                print(
...
def model_info(model, verbose=False, imgsz=640):
    ...
    try:  # FLOPs
        ...
        im = torch.zeros((1, p.shape[1], stride, stride),
                         device=p.device)  # input image in BCHW format
        flops = thop.profile(deepcopy(model), inputs=(im, ),
...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
    ...
    if name == 'Adam':
        optimizer = torch.optim.Adam(g[2], lr=lr,
...
class EarlyStopping:
    ...
    def __init__(self, patience=30):
        ...
        self.patience = patience or float(
            'inf')  # epochs to wait after fitness stops improving to stop
