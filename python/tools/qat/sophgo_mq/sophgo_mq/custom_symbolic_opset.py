...
def _fake_quantize_learnable_per_tensor_affine(g, x, scale, zero_point, quant_min, quant_max,
                                               grad_factor):
    output = g.op("Sophgo_custom::LearnablePerTensorAffine", x, scale, zero_point, quant_min,
                  quant_max)
...
def fake_quantize_per_channel_affine(g, x, scale, zero_point, ch_axis, quant_min, quant_max):
    output = g.op("Sophgo_custom::FixedPerChannelAffine", x, scale, zero_point, ch_axis, quant_min,
                  quant_max)
    input_shape = symbolic_helper._get_tensor_sizes(x)
