...
def convert_qnniqat_linear(model, fused_node):
    ...
    linear = torch.nn.Linear(fused_module.in_features, fused_module.out_features, fused_module.bias
                             is not None)
...
def convert_qnnqat_conv2d(model, fused_node):
    ...
    conv = torch.nn.Conv2d(fused_module.in_channels, fused_module.out_channels,
                           fused_module.kernel_size, fused_module.stride, fused_module.padding,
                           fused_module.dilation, fused_module.groups, fused_module.bias
                           is not None, fused_module.padding_mode)
...
def convert_qnnqat_deconv2d(model, fused_node):
    modules = dict(model.named_modules())
    fused_module = modules[fused_node.target]
    deconv = torch.nn.ConvTranspose2d(fused_module.in_channels,
                                      fused_module.out_channels,
                                      fused_module.kernel_size,
                                      stride=fused_module.stride,
                                      padding=fused_module.padding,
                                      output_padding=fused_module.output_padding,
                                      groups=fused_module.groups,
                                      bias=fused_module.bias is not None,
...
def linearert_qnniqat_linearrelu(model, fused_node):
    ...
    with graph.inserting_after(fused_node):
        relu_node_name = relu_name if linear_parent_name == "" else "{}.{}".format(
            linear_parent_name, relu_name)
...
def convert_nniqat_convbn(model, fused_node):
    ...
    fused_module = modules[fused_node.target]
    # Create a Conv2d from FusedModule.
    conv = fused_module_class_map[type(fused_module)](fused_module.in_channels,
                                                      fused_module.out_channels,
                                                      fused_module.kernel_size, fused_module.stride,
                                                      fused_module.padding, fused_module.dilation,
                                                      fused_module.groups, fused_module.bias
                                                      is not None, fused_module.padding_mode)
    conv.weight = fused_module.weight
    if fused_module.bias is not None:
        conv.bias = fused_module.bias
...
def convert_nniqat_convbnrelu(model, fused_node):
    ...
    nodes = list(model.graph.nodes)
    with graph.inserting_after(fused_node):
        relu_node_name = relu_name if conv_parent_name == "" else "{}.{}".format(
            conv_parent_name, relu_name)
        assert relu_node_name in modules and isinstance(modules[relu_node_name], torch.nn.ReLU)
        inserted_node = graph.create_node("call_module", relu_node_name, (fused_node, ), {})
        for _node in nodes:
            for i, _arg in enumerate(_node.args):
...
def convert_qnni_deconvbn(model, fused_node):
    ...
    deconv = torch.nn.ConvTranspose2d(fused_module_deconv.in_channels,
                                      fused_module_deconv.out_channels,
                                      fused_module_deconv.kernel_size,
                                      stride=fused_module_deconv.stride,
                                      padding=fused_module_deconv.padding,
                                      output_padding=fused_module_deconv.output_padding,
                                      groups=fused_module_deconv.groups,
                                      bias=fused_module_deconv.bias is not None,
...
def convert_qnni_deconvbnrelu(model, fused_node):
    ...
    if not hasattr(modules[deconv_parent_name], relu_name):
        setattr(modules[deconv_parent_name], relu_name,
                torch.nn.ReLU(inplace=True).train(fused_module.training))
    # Update modules.
    modules = dict(model.named_modules())
    graph = model.graph
    nodes = list(model.graph.nodes)
    with graph.inserting_after(fused_node):
        relu_node_name = relu_name if deconv_parent_name == "" else "{}.{}".format(
            deconv_parent_name, relu_name)
        assert relu_node_name in modules and isinstance(modules[relu_node_name], torch.nn.ReLU)
        inserted_node = graph.create_node("call_module", relu_node_name, (fused_node, ), {})
        for _node in nodes:
            for i, _arg in enumerate(_node.args):
