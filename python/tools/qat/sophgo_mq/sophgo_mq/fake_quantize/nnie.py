...
class NNIEFakeQuantize(QuantizeBase):
    ...
    def forward(self, X):
        with no_jit_trace():
            ...
            data_max = torch.max(-self.activation_post_process.min_val,
...
class NNIEQuantizeFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, data_max):
        ...
        x[zero_idx] = 0
        x[pos_idx] = 2**((torch.clamp(torch.round(16 * torch.log2(x[pos_idx]) - z), 0, 127) + z) /
                         16)
        x[neg_idx] = -2**(
            (torch.clamp(torch.round(16 * torch.log2(-x[neg_idx]) - z), 1, 127) + z) / 16)
        x = x.float()
        return x
