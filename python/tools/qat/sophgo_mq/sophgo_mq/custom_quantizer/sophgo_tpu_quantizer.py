...
from sophgo_mq.fake_quantize import (LearnableFakeQuantize, Fp16FakeQuantize, BF16FakeQuantize)
...
class SophgoTpuQuantizer(ModelQuantizer):
    ...
    def _passed_func_type(self):
        return (torch.nn.functional.relu, torch.nn.functional.relu6, torch.flatten)
    ...
    def _insert_fake_quantizer(self,
                               model,
                               graph,
                               modules,
        ...
        if len(next_layers) > 0:
            ...
            with graph.inserting_after(node):
                ...
                for next_layer in next_layers:
                    next_layer.args = self._fix_succ_recursivly(next_layer.args, node,
                                                                inserted_node)
    ...
    def _insert_fake_quantize_for_act_quant_for_mix_precsion_train(self, model: GraphModule,
                                                                   qconfig: Any):
        graph = model.graph
        nodes = list(model.graph.nodes)
        ...
        for node in nodes:
            ...
            logger.info("Insert out act quant {}, type:{}".format(quantizer_name,
                                                                  type(fake_quantizer)))
        ...
        support_fuse_relu_module_type = (
            ...
            # Upsample
            torch.nn.Upsample) + self._layers_need_scale_form_input_fake_quantizer
        support_fuse_relu_function_type = (torch.matmul, operator.sub, torch.sub, operator.mul,
                                           torch.mul, operator.add, torch.add, torch.cat)
        deleted_node = []
        for node in nodes:
    ...
    def _insert_fake_quantize_for_act_quant(self, model: GraphModule, qconfig: Any):
        ...
        node_to_quantize_output.extend(node for node in self.node_to_quantize_output_transformer
                                       if node not in node_to_quantize_output)
                ...
                if (node.op == "call_module" and isinstance(modules[node.target], self.module_type_to_quant_input)) or \
                    ...
                    for next_node in node.users:
                        if ((next_node.op == 'call_function'
                             and next_node.target in self._passed_func_type) or
                            (next_node.op == 'call_module'
                             and isinstance(modules[next_node.target], self._passed_module_type))):
            ...
            if node.op == "placeholder" and (node.name not in self.exclude_node_name):
                if 'tensor_meta' in node.meta:
                    if len(node.meta['tensor_meta'].shape) > 1:
                        print(
                            f'add placeholder {node.target} to node_need_to_quantize_output by tensor_meta'
                        )
                        node_need_to_quantize_output.append(node)
                        print(">>>>> append node ", node)
                else:
                    print(
                        f'no tensor_meta, add placeholder {node.target} to node_need_to_quantize_output'
                    )
                    node_need_to_quantize_output.append(node)
                    print(">>>>> append node ", node)
    ...
    def _set_fake_quantizer_to_next_weight_layer(self, model: GraphModule):
        ...
        self.quantizer_prefix = "_input_act_fake_quantizer"
        for node in nodes:
            if node.target in modules and (self.quantizer_prefix in node.target
                                           or "_post_act_fake_quantizer" in node.target):
                fake_quantizer = getattr(model, node.target)
                for user in node.users:
                    if (user.op == "call_module"
                            and isinstance(modules[user.target],
                                           self._layers_need_scale_form_input_fake_quantizer)):
                        setattr(modules[user.target], "input_fake_quantizer", fake_quantizer)
                        print('wlog:', user.target, '\'type is:', type(modules[user.target]),
                              "add input_fake_quantizer")
                    if user.target in modules and type(
                            modules[user.target]) in self.exclude_module_name:
                        for user2 in user.users:
                            if (user2.op == "call_module" and isinstance(
                                    modules[user2.target],
                                    self._layers_need_scale_form_input_fake_quantizer)):
                                setattr(modules[user2.target], "input_fake_quantizer",
                                        fake_quantizer)
                                print('wlog:', user2.target, '\'type is:',
                                      type(modules[user2.target]), "add input_fake_quantizer")
    ...
    def _set_module_only_enable_observer(self, model: GraphModule):
        if self.strategy == "Transformer":
            ...
            for name, submodule in model.named_modules(remove_duplicate=False):
                if isinstance(
                        submodule,
                        torch.quantization.FakeQuantizeBase) and "_post_act_fake_quantizer" in name:
        ...
        for name, submodule in model.named_modules(remove_duplicate=False):
            if isinstance(submodule, torch.quantization.FakeQuantizeBase) and (
                    name in self.module_only_enable_observer):
                logger.info('Only enable observer : {}'.format(name))
