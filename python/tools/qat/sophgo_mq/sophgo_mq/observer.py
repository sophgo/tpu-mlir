...
class ObserverBase(_ObserverBase):
    ...
    @torch.jit.export
    def extra_repr(self):
        return "min_val={}, max_val={} ch_axis={} pot={}".format(
            self.min_val if self.ch_axis == -1 else 'List',
            self.max_val if self.ch_axis == -1 else 'List', self.ch_axis, self.pot_scale)
...
class MinMaxFloorObserver(ObserverBase):
    ...
    def __init__(self,
                 ...
                 pot_scale=False,
                 factory_kwargs=None):
        super(MinMaxFloorObserver, self).__init__(dtype, qscheme, reduce_range, quant_min,
                                                  quant_max, ch_axis, pot_scale, factory_kwargs)
        '''
    ...
    def calculate_qparams(self):
        ...
        scale, zero_point = self._calculate_qparams(self.min_val, self.max_val)
        scale.data = scale.data * 0 + max(self.min_val / self.quant_min,
                                          self.max_val / self.quant_max)
        if scale < 2**-15:
            max_scale = 0
        else:
            max_scale = 1 / scale
        ...
        for s in range(max_scale, max_scale + scale_range):
            ...
            if mth == 3:
                ...
                new_x = torch.where((new_x < 0) & (new_x - new_x.floor() == 0.5), new_x.ceil(),
                                    new_x.round())
                new_x *= _s
...
class PoTModeObserver(ObserverBase):
    ...
    def __init__(self,
                 ...
                 pot_scale=False,
                 factory_kwargs=None):
        super(PoTModeObserver, self).__init__(dtype, qscheme, reduce_range, quant_min, quant_max,
                                              ch_axis, pot_scale, factory_kwargs)
        self.quant_type = None
        self.counter = [0] * 20
...
class EMAQuantileObserver(ObserverBase):
    ...
    def __init__(self,
                 ...
                 bins=2048,
                 factory_kwargs=None):
        super(EMAQuantileObserver, self).__init__(dtype, qscheme, reduce_range, quant_min,
                                                  quant_max, ch_axis, pot_scale, factory_kwargs)
    ...
    def forward(self, x_orig):
        ...
        if self.max_val.numel() <= 1 and self.max_val.isinf():
            ...
            self.min_val = self.min_val * self.ema_ratio + max(min_val_cur,
                                                               -clip_value) * (1.0 - self.ema_ratio)
            self.max_val = self.max_val * self.ema_ratio + min(max_val_cur,
                                                               clip_value) * (1.0 - self.ema_ratio)
        return x
...
class ClipStdObserver(ObserverBase):
    ...
    def __init__(self,
                 ...
                 std_scale=2.6,
                 factory_kwargs=None):
        super(ClipStdObserver, self).__init__(dtype,
                                              qscheme,
                                              reduce_range,
                                              quant_min,
                                              quant_max,
...
class MSEObserver(ObserverBase):
    ...
    def lp_loss(self, pred, tgt, dim=None):
        ...
        """
        return (pred - tgt).abs().pow(self.p).mean(dim) if dim else (pred -
                                                                     tgt).abs().pow(self.p).mean()

    def mse(self, x: torch.Tensor, x_min: torch.Tensor, x_max: torch.Tensor, iter=80):
        best_score = 1e+10
        best_min, best_max = torch.tensor([1.0], dtype=torch.float), torch.tensor([1.0],
                                                                                  dtype=torch.float)
        ...
        for i in range(iter):
            ...
            x_q = torch.fake_quantize_per_tensor_affine(x, scale.item(), int(zero_point.item()),
    ...
    def mse_perchannel(self,
        ...
        for i in range(iter):
            ...
            x_q = torch.fake_quantize_per_channel_affine(
                x, scale,
                zero_point.long() if _version_under_1100 else zero_point, ch_axis, self.quant_min,
                self.quant_max)
    ...
    def forward(self, x_orig):
        ...
        if self.ch_axis == -1:
            ...
            min_val_cur, max_val_cur = torch._aminmax(y, 1)
            min_val_cur, max_val_cur = self.mse_perchannel(x,
                                                           min_val_cur,
                                                           max_val_cur,
                                                           iter=80,
...
class KLDObserver(ObserverBase):
    ...
    def __init__(self,
        ...
        super(KLDObserver, self).__init__(dtype, qscheme, reduce_range, quant_min, quant_max,
                                          ch_axis, pot_scale, factory_kwargs)
        self.histobserver = torch.ao.quantization.observer.HistogramObserver(
            dtype=dtype,
            qscheme=qscheme,
