...
class MaskRCNN_Tester_Basic(object):
    ...
    def __init__(self, debug: bool = True, path_custom_dataset: str = None):
        ...
        if path_custom_dataset is not None:
            self.print_debug(
                "[Warning] path_default_MaskRCNN_dataset is changed from default path: {} to new path: {}"
                .format(self.path_default_MaskRCNN_dataset, path_custom_dataset))
...
class MaskRCNN_InputPreprocessor(MaskRCNN_Tester_Basic):
    ...
    def deal_path_save_preprocessed(self):
        if self.path_preprocessed_npz is not None:
            ...
            path_save_preprocessed_lv0 = self.path_input_image.replace(
                ".npy", "").split(".npz")[0] + "_{}.npz".format(self.SAVE_suffix)
    ...
    def __init__(self,
        ...
        assert len((torch.tensor(basic_scalar_factor) == 1).nonzero(
            as_tuple=True)[0]) == 0, "[Warning] scalar_factor usually not all 1s"
        ...
        if self.mode_input_generator == self.Mode_Complete:
           ...
           "##################################################"
            ...
            assert 0, "[Error] Such Mode-{} is not supported!".format(self.mode_input_generator)
    ...
    def preprocess_YAML(self):
        ...
        self.C_getBboxB = self.dict_content_yaml["MAX_PER_IMG"] * self.dict_content_yaml[
            "NUM_CLASSES_GetBboxB"]
    ...
    def process_PPL_ConstOp(self, num_constOp: int = 3):
        ...
        self.basic_max_shape_inverse = self.gen_basic_tenor_and_repeat_twice(
            self.basic_max_shape_inverse)
        ...
        if num_constOp >= 2:
            max_shape_getBboxB = np.repeat(self.basic_max_shape_inverse, self.C_getBboxB,
                                           axis=2).reshape([1, 1, -1, self.num_indexes])
            self.processed_input_data[str(start_idx + 1)] = max_shape_getBboxB.astype(np.float32)

            #4-scale-factor
            self.basic_scalar_factor = self.gen_basic_tenor_and_repeat_twice(
                self.basic_scalar_factor)
            scale_factor_getBboxB = np.repeat(self.basic_scalar_factor, self.C_getBboxB,
                                              axis=2).reshape([1, 1, -1, self.num_indexes])
            self.processed_input_data[str(start_idx + 2)] = scale_factor_getBboxB.astype(np.float32)

            scale_factor_MaskPOoler = np.repeat(self.basic_scalar_factor,
                                                self.roi_slice_maskpooler,
                                                axis=2).reshape([1, 1, -1, self.num_indexes])
            self.processed_input_data[str(start_idx + 3)] = scale_factor_MaskPOoler.astype(
                np.float32)

        self.print_debug(
            "-----------------Statiscal Info for save_path: {}--------------------".format(
                self.path_save_preprocessed))
        for idx, per_key in enumerate(self.processed_input_data.keys()):
            self.print_debug("[Key]-{}  shape-{}".format(per_key,
                                                         self.processed_input_data[per_key].shape))
        self.print_debug("-------------------------------------------------------------------")
        np.savez(self.path_save_preprocessed, **self.processed_input_data)

        recheck_data = np.load(self.path_save_preprocessed)
        assert np.sum(
            np.abs(recheck_data[recheck_data.files[start_idx + 0]] -
                   self.processed_input_data[recheck_data.files[start_idx + 0]])) < 1e-6
        assert np.sum(np.abs(recheck_data[recheck_data.files[start_idx + 0]] -
                             max_shape_RPN)) < 1e-6
        if num_constOp >= 2:
            assert np.sum(
                np.abs(recheck_data[recheck_data.files[start_idx + 1]] - max_shape_getBboxB)) < 1e-6
            ...
            assert np.sum(
                np.abs(recheck_data[recheck_data.files[start_idx + 2]] - scale_factor_getBboxB)
            ) / scale_factor_getBboxB.flatten().shape[0] < 1e-6, np.sum(
                np.abs(recheck_data[recheck_data.files[start_idx + 2]] - scale_factor_getBboxB))
        self.print_debug("[CMP]All stored {}-data Checked&Passed!".format(
            len(self.processed_input_data.keys())))
    ...
    def gen_model_transform_cmd(self):
        ...
        test_reference_path = "Ref_Output_" + self.path_save_preprocessed.split("/")[-1].replace(
            "input", "").replace(self.SAVE_suffix, "")
        if self.path_preprocessed_npz is None:
            context_info += "--test_reference  {} #using output npz of your last block \ ".format(
                test_reference_path)
        else:
            context_info += "--test_reference  {} #using output npz of your last block \ ".format(
                self.path_preprocessed_npz)
...
if __name__ == "__main__":
    ...
    Superior_MaskRCNN_InputParser = MaskRCNN_InputPreprocessor(args.path_yaml,
                                                               ...
                                                               args.basic_scalar_factor, args.debug,
