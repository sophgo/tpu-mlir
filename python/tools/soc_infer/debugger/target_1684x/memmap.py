...
memmap = {
    MType.R.value: (0x8000000, 0x9000000),  # lmen_base 16M
    MType.S.value: (0x9000000, 0x9004000),  # static memory 16KB
    MType.L.value: (0x10000000, 0x10200000),  # L2 SRAM  2M
    MType.G.value: (0x100000000, 0x300000000),  # global memory
}
...
class MemRef(MemRefBase):
    ...
    def r_addr(self):
        if self.mtype == MType.UNKNOWN:
            return self.address
        # print(memmap, type(memmap))
        # print(MType.R.name,self.mtype.name)
        # print(MType.R.value, self.mtype.value, type(self.mtype.value),type(MType.R.value))
        r_addr = self.address - memmap[self.mtype.value][0]
        return r_addr
