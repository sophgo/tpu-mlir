...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(in_channels=out_channel,
                               ...
                               stride=stride,
                               bias=False,
                               padding=1)  # H/2，W/2。C不变
        self.bn2 = nn.BatchNorm2d(num_features=out_channel)
        self.conv3 = nn.Conv2d(in_channels=out_channel,
                               out_channels=out_channel * self.expansion,
                               kernel_size=1,
                               stride=1,
                               bias=False)  # H,W不变。C: out_channel -> 4*out_channel
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, block_num, num_classes=1000):
        ...
        self.layer2 = self._make_layer(
            block=block, channel=128, block_num=block_num[1],
            stride=2)  # H/2, W/2。downsample控制的shortcut，out_channel=128x4=512
        self.layer3 = self._make_layer(
            block=block, channel=256, block_num=block_num[2],
            stride=2)  # H/2, W/2。downsample控制的shortcut，out_channel=256x4=1024
        self.layer4 = self._make_layer(
            block=block, channel=512, block_num=block_num[3],
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        layers.append(
            block(in_channel=self.in_channel,
                  out_channel=channel,
