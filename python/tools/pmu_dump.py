        ...
        for line in f.readlines():
            if line[0:2] in "[0][1][2][3][4][5][6][7]":
                ...
                if last_core_no != core_no and last_core_no != -1:
                    profile_info[last_core_no] = [
                        profile_gdma_info.copy(),
                        profile_tpu_info.copy(),
                        profile_sdma_info.copy()
                    ]
            ...
            if line.startswith("---> gdma"):
                ...
                profile_gdma_info.append([
                    item_map['inst_id'], item_map['thread_id'], cycle, start_time, end_time,
                    interval
                ])
                ...
                profile_tpu_info.append([
                    item_map['inst_id'], item_map['thread_id'], cycle, start_time, end_time,
                    interval, bank_conflict
                ])
...
def write_csv_file(cmds_dict: dict, pmu_profile_info: dict, save_file: str):
            ...
            if (type == EngineType.GDMA):
                if pmu_gdma_inst_id >= pmu_gdma_count:
                    ...
                    if (str(gdma_inst_id) == inst_id):
                        item = [
                            str(i), inst_id, "GDMA", thread_id, gdma_item[2], gdma_item[3],
                            gdma_item[4], gdma_item[5], ""
                        ]
                        item += cmd_info
                        pmu_gdma_inst_id += 1
                ...
                if pmu_tpu_inst_id >= pmu_tpu_count:
                    ...
                    if (str(tpu_inst_id) == inst_id):
                        item = [
                            str(i), inst_id, "TPU", thread_id, tpu_item[2], tpu_item[3],
                            tpu_item[4], tpu_item[5], tpu_item[6]
                        ]
                        item += cmd_info
                        pmu_tpu_inst_id += 1
                ...
                if pmu_sdma_inst_id >= pmu_sdma_count:
                    ...
                    if (str(sdma_inst_id) == inst_id):
                        item = [
                            str(i), inst_id, "SDMA", thread_id, sdma_item[2], sdma_item[3],
                            sdma_item[4], sdma_item[5], ""
                        ]
                        item += cmd_info
                        pmu_sdma_inst_id += 1
    ...
    with open(save_file, 'w', newline='') as csvfile:
        ...
        writer.writerow([
            'No', 'inst_id', 'cmd_type', 'thread_id', 'cycle', 'start(us)', 'end(us)',
            'interval(us)', 'bank_conflict', 'cmd_name', 'id_dep_info', 'res0_info', 'opd0_info',
            'opd1_info', 'opd2_info', 'opd3_info', 'opd4_info', 'opd5_info'
        ])
...
def argparser():
    ...
    optparser.add_argument('-f', '--cmd_file', action='store', dest='cmd_file', required=True)
    optparser.add_argument('-p', '--pmu_file', action='store', dest='pmu_file', required=True)
    optparser.add_argument('-o', '--out_file', action='store', dest='out_file', required=True)
    optparser.add_argument('-c',
                           '--chip',
                           action='store',
                           dest='chip',
                           default='BM1690',
                           help='only support bm1690')
