...
def graph_layout_dropdown(id):
    return dbc.Select(id=id,
                      ...
                      options=[{
                          'label': x.upper(),
                          'value': x
                      } for x in ('dagre', 'klay', 'cola', 'euler', 'grid', 'circle', 'concentric',
                                  'breadthfirst')])
...
def mini_menu(*ids):
    ...
    return dbc.DropdownMenu([
        ...
        dbc.DropdownMenuItem("hide toolbox", id=id_toolbox, n_clicks=1, disabled=True),
...
def layer_info(layer):
    ...
    tmp_attrs = {
        x: layer.attrs[x]
        for x in layer.attrs if x != "multipliers" and x != "rshifts" and x != "multiplier"
        and x != "rshift" and x != "quant_mode" and x!= "out_f8_scales"
    }
    ...
    return html.Div(dbc.Table([
        html.Thead([
            html.Tr([
                html.Th("Layer Name", style=table_style),
                html.Th(layer.name, style=table_style),
                html.Th("Layer Type", style=table_style),
                html.Th(layer.type, style=table_style)
            ],
                    style=table_style),
        ]),
        html.Tbody([
            html.Tr([
                html.Td('Inputs', style=table_style),
                html.Td(f'{in_}', style=table_style),
                html.Td('Outputs', style=table_style),
                html.Td(f'{out_}', style=table_style)
            ]) for in_, out_ in zip(ins, outs)
        ]),
        html.Tbody([
            html.Tr([
                html.Td(f'{x[0]}', style=table_style),
                html.Td(f'{x[1]}', style=table_style),
                html.Td(f'{x[2]}', style=table_style),
                html.Td(f'{x[3]}', style=table_style)
            ]) for x in list(att)
        ]),
    ],
                              style=table_style),
...
def info_tab(*ids):
    ...
    return html.Div([
        dcc.Tabs(id=ids[0],
                 value='tab0',
                 children=[
                     dcc.Tab(label='Layer/Tensor Info',
                             value='tab0',
                             id=ids[1],
                             children=[
                                 html.Div([
                                     tensor_figure('figure-graph', 'figure-sample',
                                                   'figure-sample-display', 'figure-store'),
                                 ],
                                          style={
                                              'width': '100%',
                                              'height': '45vh'
                                          }),
                                 html.Div(dbc.Table(id='layer-info-card0', ),
                                          style={
                                              'position': 'absolute',
                                              'top': '51vh',
                                              'height': '100%',
                     ...
                     dcc.Tab(label='Distribution Info',
                             value='tab1',
                             id=ids[2],
                             children=[
                                 html.Div(dist_figure('dist-graph', 'dist-store'),
                                          style={'height': '100vh'}),
                             ],
                             style=tab_style,
                             selected_style=tab_selected_style),
                     dcc.Tab(label='Weight Info',
                             value='tab2',
                             id=ids[3],
                             children=[
                                 html.Div(tensor_figure('weight-graph', 'weight-sample',
                                                        'weight-sample-display', 'weight-store'),
                                          style={'height': '80vh'}),
                             ],
                             style=tab_style,
                             selected_style=tab_selected_style),
                     dcc.Tab(label='Summary Info',
                             value='tab3',
                             id=ids[4],
                             children=[
                                 html.Div(info_tabulator('info-tabulator'),
                                          style={'height': '100vh'}),
                             ],
                             style=tab_style,
                             selected_style=tab_selected_style),
...
def tensor_figure(*ids):
    ...
    return html.Div([
        dbc.Row([
            dbc.Col(dbc.Input(
                id=id_fig_sample, type='range', value=1000, min=1000, max=20000, step=100),
                    width=10),
            dbc.Col(html.Div(
                "1000",
