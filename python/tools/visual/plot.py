...
def linear_plot(data, name=None, max_sampling=1000, bias=None):
    ...
    fig = plt.plot_float_vs_fixpoint(index, (blob_fp.flatten()[::step], blob_int.flatten()[::step]),
                                     subplot_titles=(name + '[{:.2f}/{:.2f}]'.format(kur, ske),
                                                     "float - int8\t "))
    fig.update_layout(
        legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.98, bgcolor="rgba(0,0,0,0)"))
    fig.update_layout(margin=dict(l=10, r=10, t=20, b=20), hovermode='x unified')
    return fig
...
def param_plot(data, name=None, max_sampling=1000, bias=None):
    ...
    if bias is not None:
        fig = plt.plot_weight_and_transposed(
            indexw,
            indexb, (weight.flatten()[::stepw], weight.transpose().flatten()[::stepw],
                     bias.flatten()[::stepb]),
            subplot_titles=(name, name + "-transposed", name + "-bias"))
    else:
        fig = plt.plot_weight_and_transposed(
            indexw,
            indexb, (weight.flatten()[::stepw], weight.transpose().flatten()[::stepw], None),
            subplot_titles=(name, name + "-transposed", name + "-bias"))
    fig.update_layout(
        legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.98, bgcolor="rgba(0,0,0,0)"))
    fig.update_layout(margin=dict(l=10, r=10, t=20, b=20), hovermode='x unified')
...
def dist_plot(data, name=None, scale=1.0, dtype='I8'):
    ...
    if dtype == 'I8':
        ...
        hist = np.histogram(blob_fp, bins=bins, range=(-min_bin * scale, max_bin * scale))
        hist_fp = hist[0]
        hist = np.histogram(blob_int, bins=bins, range=(-min_bin * scale, max_bin * scale))
        hist_quant = hist[0]
        q_range = np.zeros(bins)
        q_range[min_bin - 128:min_bin +
                127] = np.ones(255) * np.maximum(np.max(hist_fp), np.max(hist_quant))
        ...
        hist_quant = hist[0]
        q_range = np.zeros(bins)
        q_range[min_bin:min_bin +
                255] = np.ones(255) * np.maximum(np.max(hist_fp), np.max(hist_quant))
        index = np.arange(bins)
        fig = plt.plot_dist_fp_fixpoint(fig,
                                        index, (hist_fp, hist_quant, q_range),
        ...
        index = np.arange(255)
        hist = np.histogram(blob_fp, bins=255, range=(np.min(blob_fp), np.max(blob_fp)))
        hist_fp = hist[0]
        fig = plt.plot_dist_fp_fixpoint(fig,
                                        index, (hist_fp, [], []),
        ...
        xticklabels[int(np.abs(np.min(blob_fp)) / (np.max(blob_fp) - np.min(blob_fp) + 1e-4) *
                        255)] = '0.0'

    fig.update_xaxes(tickmode='array',
                     tickvals=np.arange(bins),
...
def metrics_plot(tensor_info):
    ...
    fig.update_xaxes(tickmode='array',
                     tickvals=np.arange(len(x_name) - 1),
                     ticktext=x_name,
                     range=[-0.5, len(x_name)],
                     type='linear')
