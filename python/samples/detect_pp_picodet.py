...
def postprocess(outputs, image, scale_factor):
    multiclass_nms3_0_tmp_0 = outputs['multiclass_nms3_0.tmp_0']
    multiclass_nms3_0_tmp_2 = outputs['multiclass_nms3_0.tmp_2']
    
    # 获取图像尺寸
    h, w, _ = image.shape

    # 调整输出尺寸
    resized_h, resized_w = h * scale_factor[0][0], w * scale_factor[0][1]

    # 计算缩放比例
    scale_h, scale_w = h / resized_h, w / resized_w

    # 更新multiclass_nms3_0.tmp_0的值
    multiclass_nms3_0_tmp_0[:, 0] = multiclass_nms3_0_tmp_0[:, 0] * scale_w
    multiclass_nms3_0_tmp_0[:, 1] = multiclass_nms3_0_tmp_0[:, 1] * scale_h
    multiclass_nms3_0_tmp_0[:, 2] = multiclass_nms3_0_tmp_0[:, 2] * scale_w
    multiclass_nms3_0_tmp_0[:, 3] = multiclass_nms3_0_tmp_0[:, 3] * scale_h

    # 获取预测结果
    pred_boxes = multiclass_nms3_0_tmp_0[:, :4]
    pred_scores = multiclass_nms3_0_tmp_0[:, 4:]
    pred_label = multiclass_nms3_0_tmp_2[0]

    # 将预测结果组合为元组列表
    results = []
    for i in range(len(pred_boxes)):
        box = pred_boxes[i]
        score = np.max(pred_scores[i])
        label = np.argmax(pred_scores[i])
        
        if label == int(pred_label):
...
def postproc_pico(outputs, input_shape, anchors=ANCHORS):
    outputs = list(outputs.values())
    num_outs = 4
    cls_scores, bbox_preds = outputs[:num_outs], outputs[num_outs:]
    det_bboxes, det_conf, det_classid = get_bboxes_single(cls_scores, bbox_preds, 1, input_shape, rescale=False)
...
def get_bboxes_single(cls_scores, bbox_preds, scale_factor, input_shape, rescale=False):
    ...
    for i in range(len(strides)):
        anchors = make_grid_v2((math.ceil(input_shape[0] / strides[i]), math.ceil(input_shape[1] / strides[i])), strides[i])
...
def main():
    ...
    out = postproc_pico(output, input_shape)
