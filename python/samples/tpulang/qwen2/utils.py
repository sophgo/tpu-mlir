...
class TPULANG_CONVERTER(object):
    ...
    def compile_and_check(self, model_name, inputs, outputs):
        ...
        tpul.compile_f32(model_name, inputs, outputs, cmp=True, mode=self.mode, no_save=False, top_mlir_inference=True, tpu_mlir_inference=True, log_level='normal')
...
def read_weights(model_weights):
    # matmul
    projections = {
        "q_proj": {"prefix": "self_attn.q_proj", "has_bias": True},
        "k_proj": {"prefix": "self_attn.k_proj", "has_bias": True},
        "v_proj": {"prefix": "self_attn.v_proj", "has_bias": True},
        "o_proj": {"prefix": "self_attn.o_proj", "has_bias": False},
        "gate_proj": {"prefix": "mlp.gate_proj", "has_bias": False},
        "up_proj": {"prefix": "mlp.up_proj", "has_bias": False},
        "down_proj": {"prefix": "mlp.down_proj", "has_bias": False},
    }
    ...
    for name, cfg in projections.items():
        prefix = cfg["prefix"]
        tensors[name] = {
            "weights": to_tpul_tensor(f"{prefix}.qweight", model_weights, use_float=False, dtype="int32"),
            "scales":  to_tpul_tensor(f"{prefix}.scales",  model_weights, use_float=True,  dtype="float32"),
            "zeros":   to_tpul_tensor(f"{prefix}.qzeros",  model_weights, use_float=False, dtype="int32"),
            "bias":    to_tpul_tensor(f"{prefix}.bias", model_weights, use_float=True,  dtype="float32") if cfg["has_bias"] else None,
        }

    # ln
    input_layernorm_weight = model_weights['input_layernorm.weight'].float().numpy()
    post_attention_layernorm_weight = model_weights['post_attention_layernorm.weight'].float().numpy()
    input_layernorm_weight = tpul.Tensor(dtype="float32",
            shape=list(input_layernorm_weight.shape), data=input_layernorm_weight, ttype="coeff")
    post_attention_layernorm_weight = tpul.Tensor(dtype="float32",
            shape=list(post_attention_layernorm_weight.shape), data=post_attention_layernorm_weight, ttype="coeff")
