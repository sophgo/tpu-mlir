...
def preprocess(img, input_shape):
    ...
    mean_vec = np.array([127, 127, 127]).astype('float32')
...
def parse_args():
    ...
    parser.add_argument("--net_input_dims", type=str, default="512,512", help="(h,w) of net input")
...
def postprocess_cls(output, img, category_file, top_k=5):
    prob = output[list(output.keys())[-1]]
...
def postprocess_seg(output, img):
    prob = output[list(output.keys())[-1]]
...
def get_output_node_name(model_def):
    import onnx
    model = onnx.load(model_def)
    output_node_name = []
...
def main():
    ...
    data = {'x': img}  # input name from model
    output = dict()
    if args.model_def.endswith('.prototxt') and args.model_data.endswith('.caffemodel'):
        ...
        output = onnx_inference(data, args.model_def, False)
    else:
        raise RuntimeError("not support modle file:{}".format(args.model_def))
    fix_img = postprocess_seg(output, origin_img)
