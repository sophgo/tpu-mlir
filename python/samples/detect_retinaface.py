...
def postprocess(image,
    ...
    for stride, scores, bbox_deltas, landmark_deltas in zip(strides, cls_heads, box_heads,
                                                            landmark_heads):
        ...
        proposals = clip_boxes(proposals, image_shape[:2])
...
def preprocess(img, input_shape):
    ...
    for i in range(3):
        im_tensor[0, :, :,
                  i] = (img[:, :, 2 - i] / pixel_scale - pixel_means[2 - i]) / pixel_stds[2 - i]
    im_tensor = im_tensor.transpose([0, 3, 1, 2])
    return im_tensor, img.shape[0:2], im_scale


def parse_args():
    parser = argparse.ArgumentParser(description='Inference retinaface network.')
    parser.add_argument("--model", type=str, required=True, help="Model definition file")
    parser.add_argument("--net_input_dims", type=str, default="240,320", help="(h,w) of net input")
...
def main():
    ...
    image_data, image_shape, image_scale = preprocess(origin_image, input_shape)
    data = {"data": image_data}  # input name from model

    output = dict()

    cls_names = [
        'face_rpn_cls_prob_reshape_stride32', 'face_rpn_cls_prob_reshape_stride16',
        'face_rpn_cls_prob_reshape_stride8'
    ]
    bbx_names = [
        'face_rpn_bbox_pred_stride32', 'face_rpn_bbox_pred_stride16', 'face_rpn_bbox_pred_stride8'
    ]
    landmark_names = [
        'face_rpn_landmark_pred_stride32', 'face_rpn_landmark_pred_stride16',
    ...
    probs, boxes, landmarks = postprocess(image_data, cls_heads, bbx_heads, landmark_heads,
