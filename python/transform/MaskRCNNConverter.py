...
class MaskRCNNHelper(MaskRCNN_Tester_Basic):
    ...
    def parser_MASKRCNN_CONFIG_YAML(self):
        ...
        self.DELTA2BBOX_1st_MAX_SHAPE_H = self.dict_content_yaml[
            "DELTA2BBOX_1st_MAX_SHAPE_H"]  #value:800.0
        self.DELTA2BBOX_1st_MAX_SHAPE_W = self.dict_content_yaml[
            "DELTA2BBOX_1st_MAX_SHAPE_W"]  #value:1216.0
        ...
        self.DELTA2BBOX_2nd_MAX_SHAPE_H = self.dict_content_yaml[
            "DELTA2BBOX_2nd_MAX_SHAPE_H"]  #value:800.0
        self.DELTA2BBOX_2nd_MAX_SHAPE_W = self.dict_content_yaml[
            "DELTA2BBOX_2nd_MAX_SHAPE_W"]  #value:1216.0
        ...
        self.Global_strides = self.dict_content_yaml[
            "Global_strides_DQ"]  #value:[(4, 4), (8, 8), (16, 16), (32, 32), (64, 64)]
        ...
        self.MASK_POOLER_FAKE_DYNAMIC_LENGTH = self.dict_content_yaml[
            "MASK_POOLER_FAKE_DYNAMIC_LENGTH"]  #value:100
        self.MAX_LENGTH_STATIC_STRECHED = self.dict_content_yaml[
            "MAX_LENGTH_STATIC_STRECHED"]  #value:4741
...
class MaskRCNNConverter(TorchConverter, MaskRCNNHelper):
    ...
    def __init__(self,
        ...
        assert self.params_model_transform_MaskRCNN in self.dict_content_yaml.keys(
        ), "[MaskRCNN-Error] your config yaml: {} must contain necessary external maskrcnn parameters:{}".format(
            path_yaml)
        io_map = dict()
        ...
        maskrcnn_num_output = int(
            self.dict_content_yaml[self.params_model_transform_MaskRCNN]['maskrcnn_num_output'])
        list_encoded_block = self.dict_content_yaml[
            self.params_model_transform_MaskRCNN]['list_encoded_block']
        list_name_PPLop = self.dict_content_yaml[
            self.params_model_transform_MaskRCNN]['list_name_PPLop']
        nums_PPLOp_Inputs = self.dict_content_yaml[
            self.params_model_transform_MaskRCNN]['nums_PPLOp_Inputs']
        ...
        list_encoded_block, multi_torch_files, nums_PPLOp_Inputs, input_shapes_AllBlock, list_name_PPLop, io_map, maskrcnn_num_output, num_InputOperand_BlockTorch_1st = self.Gen_Utest_Torch_pt(
            list_encoded_block, multi_torch_files, nums_PPLOp_Inputs, input_shapes_AllBlock,
            list_name_PPLop, io_map, maskrcnn_num_output)
        if isinstance(multi_torch_files, str):
            multi_torch_files = [multi_torch_files]
        TorchConverter.__init__(self, model_name, multi_torch_files[0],
                                input_shapes_AllBlock[0:num_InputOperand_BlockTorch_1st],
                                input_types, output_names, preprocess_args)
        ...
        assert len(list_name_PPLop) + len(
            multi_torch_files) == self.num_sub_blocks_without_TOPINOUT, (
                len(list_name_PPLop), len(multi_torch_files), self.num_sub_blocks_without_TOPINOUT)
        ...
        self.ppl_factory = {
            "ppl::RPN_get_bboxes":
            lambda input_list, op_type, id_PPLOp: self.convert_RPN_get_bboxes_op(
                input_list, op_type, id_PPLOp),
            "ppl::Bbox_Pooler":
            lambda input_list, op_type, id_PPLOp: self.convert_BBox_Pooler_op(
                input_list, op_type, id_PPLOp),
            "ppl::get_bboxes_B":
            lambda input_list, op_type, id_PPLOp: self.convert_get_bboxes_B_op(
                input_list, op_type, id_PPLOp),
            "ppl::Mask_Pooler":
            lambda input_list, op_type, id_PPLOp: self.convert_Mask_Pooler_op(
                input_list, op_type, id_PPLOp)
        }
        ...
        self.process_model_info(multi_torch_files, input_types, output_names)
    ...
    def Gen_Utest_Torch_pt(self, maskrcnn_structure: list[int], model_def: list[str],
                           numPPLOp_InWithoutWeight_MaskRCNN: list[int], input_shapes: list,
                           maskrcnn_ppl_op: list[int], maskrcnn_io_map: dict,
        ...
        if is_FirstOp_PPLOp:
            ...
            path_modified_fake_torch = os.path.abspath(os.getcwd()) + "/{}".format(
                self.name_Utest_Pt)
            if model_def is not None and (not is_OnlyOneOp):
                for idx_model, each_exist_model in enumerate(model_def):
                    if (idx_model > 0):
                        assert self.name_Utest_Pt not in each_exist_model, "[Error]{} will be added, but it's conflict with your existed model_def:{}!".format(
                            each_exist_model)
                    else:
                        assert self.name_Utest_Pt in each_exist_model, "[Error]now {} must be the first name in your model_def!".format(
                            each_exist_model)
                model_def = [path_modified_fake_torch] + model_def[1:]
            elif model_def is not None and is_OnlyOneOp:
                assert len(
                    model_def
                ) == 1, "[Error] MaskRCNN-Utest Mode Enable, model_def contains only 1 .pt file, but got: {}, which len is {}!".format(
                    model_def, len(model_def))
                assert model_def[
                    0] == self.name_Utest_Pt, "[Error] MaskRCNN-Utest Mode Enable, model_def must be {}, as there's only 1 Op in maskrcnn_structure and must be 0==PPLOp".format(
                        self.name_Utest_Pt)
                model_def = [path_modified_fake_torch]
            else:
                model_def = [path_modified_fake_torch]
            num_InputOperands_block_1st = int(numPPLOp_InWithoutWeight_MaskRCNN[0])
            assert num_InputOperands_block_1st <= len(
                input_shapes
            ), "[Error][Case:ppl-op first]: nums of Top_Input {} must be compatible with nums of input_shapes {}".format(
                int(numPPLOp_InWithoutWeight_MaskRCNN[0]), len(input_shapes))
            ...
            example_forward_input = tuple(
                [torch.rand(input_shapes[i]) for i in range(num_InputOperands_block_1st)])
            ...
            traced_model_fake = torch.jit.load(path_modified_fake_torch,
            ...
            assert np.sum([
                isinstance(j, int) for j in list_id_dst_block
            ]) == len(list_id_dst_block), "[Utest-PPL] only allow (str:int)  for (dst/src:id)] now!"
    ...
    def reorganize_sub_block_info(self):
        ...
        for _, block_id in enumerate(self.list_encoded_block):
            if block_id == self.Symbol_PPLSubBlock:
                ...
                self.list_block_discontinuous_id_OrderbyStructure_IndexbyBlockType.append(
                    idx_TorchBlock)
                idx_TorchBlock += 1
            else:
                assert 0, (block_id, self.Symbol_TorchSubBlock, self.Symbol_PPLSubBlock)
    ...
    def deal_with_preprocess(self, preprocess_args):
        ...
        assert len(self.preprocess_args) == len(self.input_names), (len(self.preprocess_args), "vs",
    ...
    def detect_idle_io_map(self, init_io_map):
        ...
        if len(init_top_idx) != len(self.input_shapes_AllBlock):
            assert 0, "[Error] Some Inputs-([Warning]is the TOP_INPUT name!) are  not used, which will lead to dynamic_ir parser overflow error! Check revised_io_map_{}.svg!".format(
                self.model_name)
        '''
        ...
        for _, dst_per in enumerate(init_io_map.keys()):
            src_per = init_io_map[dst_per]
            assert src_per[0] != self.TOP_OUTPUT, " src can't be TOP_OUT({})".format(
                self.TOP_OUTPUT)
        ...
        assert self.maskrcnn_num_output == sum_simulated_num_output, "[Error] io_map requires {}-outputs but {} is given from model-transform!".format(
            sum_simulated_num_output, self.maskrcnn_num_output)
        ...
        for _, dst_per in enumerate(init_io_map.keys()):
            ...
            if block_id in dict_replicated_dst_per.keys():
                if operand_id in dict_replicated_dst_per[block_id]:
                    name_sub_block = self.list_all_block_str_names_order[
                        block_id] if block_id != -2 else "TOP_OUT"
                    assert 0, "{} Subblock-{}-operand_id_{} has illegal multi-incomes, it forced to be single!".format(
                        name_sub_block, block_id, operand_id)
            else:
        ...
        assert self.list_encoded_block[
            0] == self.Symbol_TorchSubBlock, "[Error]first input sub-block must be torch.pt!"
        ...
        for _, block_id_withUtest in enumerate(dict_static_iomap_num_input.keys()):
            if block_id_withUtest != self.TOP_OUTPUT:
                if self.list_encoded_block[block_id_withUtest] == self.Symbol_PPLSubBlock:
                    ...
                    num_input_PPLOp_atDst_fromIomap = dict_static_iomap_num_input[
                        block_id_withUtest]
                    if num_input_PPLOp_from_user != num_input_PPLOp_atDst_fromIomap:
                        PPLOP_name = self.list_name_PPLop[ppl_op_count_from_iomap]
                        assert 0, "[Error] name-{} nums of input from_iomap is {},but user gives {}".format(
                            PPLOP_name, num_input_PPLOp_atDst_fromIomap, num_input_PPLOp_from_user)
                    ppl_op_count_from_iomap += 1
        assert ppl_op_count_from_iomap == len(
            self.list_name_PPLop
        ), "[Error] nums of pplop in io_map is {}, but user gives {}!".format(
            ppl_op_count_from_iomap, len(self.list_name_PPLop))
    ...
    def MaskRCNN_IOMAP_Transformer(self, init_io_map):
        ...
        for block_dst in dst_keys:
            ...
            if new_block_dst not in new_io_map.keys():
                ...
                self.print_debug(
                    "[Warning] dst {} is conflict, exist/conflict: {}-----{} , I choose exist user_given io_map {} as first prioity"
                    .format(new_block_dst, exist_set, conflict_set, exist_set))
        ...
        if (io_style == self.IO_PATTERN_STRING):
            ...
            def name_checker(name_dst, name_src, source_list, hist_flag_torch_dst,
                             hist_flag_torch_src):
                flag_dst = 0
                flag_src = 0
            ...
            for set_block in dst_keys:
                ...
                hist_flag_torch_dst, hist_flag_torch_src = name_checker(
                    name_dst, name_src, self.multi_torch_files, 0, 0)
                flag_dst, flag_src = name_checker(name_dst, name_src, self.list_name_PPLop,
                                                  hist_flag_torch_dst, hist_flag_torch_src)
        ...
        for _, dst_per in enumerate(dst_set):
            ...
            def gen_absolute_offset_y(block_idx_dst, block_idx_src):
                ...
                if block_idx_src == self.TOP_INPUT:
                    ...
                    y_src = base + line_height + block_height + (block_idx_src) * (line_height +
                                                                                   block_height)
                if block_idx_dst == self.TOP_OUTPUT:
                    y_dst = base + line_height + (len(self.multi_torch_files) + len(
                        self.nums_PPLOp_Inputs)) * (line_height + block_height)
                else:
            ...
            if dst_per[0] != self.TOP_OUTPUT:
                ...
                block_width = width_sub_block[dst_per[
                    0]]  #if not self.list_encoded_block[dst_per[0]] else width_sub_block[dst_per[0]]
                ...
                if not self.list_encoded_block[dst_per[0]]:
                    idx_op = int(dst_per[0] -
                                 np.sum(np.array(self.list_encoded_block[0:dst_per[0]])))
                else:
                    idx_op = int(np.sum(np.array(self.list_encoded_block[0:dst_per[0]])))
                tag = self.multi_torch_files[idx_op].split("/")[-1] if self.list_encoded_block[
                    dst_per[0]] else self.list_name_PPLop[idx_op].split("/")[-1]
                plt.text(x=pointer_x_00 + block_width / 2,
                         y=pointer_y_00 + block_height / 2,
                         s="[{}]-{}".format(dst_per[0], tag))

            p_dst = [x_dst, y_dst]
            p_src = [x_src, y_src]
            self.print_debug("[Connect-Info]", p_src, "to", p_dst, "[origin]:",
                             [src_per[1], src_per[0]], "to", [dst_per[1], dst_per[0]])
            ...
            if np.sum(x) == 0 or np.sum(y) == 0.0 or np.isnan(x[0]) or np.isnan(y[0]):
                if point_x[0] == point_x[1]:
                    point_x = [
                        point_x[0], point_x[0] - 0.01329 * (point_y[1] - point_y[0]) / 2, point_x[1]
                    ]
                    point_y = [point_y[0], (point_y[0] + point_y[1]) / 2, point_y[1]]
                plt.plot(point_x, point_y, color="black")
            plt.text(x=p_dst[0] + 0.1,
                     y=p_dst[1] + 0.5,
                     s="{}".format(str(int(dst_per[1] - block_start_dict[dst_per[0]]))))
            plt.text(x=p_src[0] + 0.1,
                     y=p_src[1] + 0.5,
                     s="{}".format(str(int(src_per[1] - block_start_dict[src_per[0]]))))

        plt.axhline(y=max_dst_y, color='r', ls="--", label="TOP_OUT")
        plt.axhline(y=min_src_y, color='r', ls="--", label="TOP_IN")
        path_io_plt = os.path.abspath(os.getcwd()) + "/revised_io_map_{}.svg".format(
            self.model_name)
    ...
    def process_model_info(self, multi_torch_files: list[str], input_types: list,
                           output_names: list):
        ...
        if self.num_InputOperand_BlockTorch_1st < self.num_InputOperand_AllBlock:
            self.print_debug(
                "[Warning] Input Shapes exist other formats for later PPLOp: {}".format(
                    self.input_shapes_AllBlock[self.num_InputOperand_BlockTorch_1st:]))
        self.load_multi_torch_model_Superior_MaskRCNN(
            multi_torch_files, self.input_shapes_AllBlock[:self.num_InputOperand_BlockTorch_1st],
    ...
    def load_multi_torch_model_Superior_MaskRCNN(self, multi_torch_files: list[str],
                                                 input_shapes: list, input_types: list,
                                                 output_names: list):
        for idx, torch_file in enumerate(multi_torch_files):
            model = None
            model_name = self.get_submodel_prefix(idx)
            if isinstance(torch_file, str):
                model = torch.jit.load(torch_file, map_location=torch.device('cpu'))
            else:
                model = torch_file
            model.eval()
    ...
    def generate_mlir(self, mlir_file: str, save_in_mem: bool = False):
        ...
        for idx, _name in enumerate(self.input_names):
            ...
            input_ = self.mlir.create_input_op(self.get_loc(unified_name), idx,
                                               self.preprocess_args[idx])
        ...
        for idx, flag_isNotPPL in enumerate(self.list_encoded_block):
            self.print_debug("[GenMlirStart]{}-sub_structure: {}".format(
                idx, ".pt" if flag_isNotPPL else "PPL"))
            ...
            if flag_isNotPPL:
                ...
                for j, _name in enumerate(cur_input_names):
                    ...
                    output_name = output_name if len(re.findall(
                        prefix_isolated, output_name)) >= 1 else prefix_isolated + output_name
                    self.print_debug("prefix_isolated/output_name: {}/{},{}".format(
                        prefix_isolated, output_name, re.findall(prefix_isolated, output_name)))
                    op = self.getOperand(output_name)

                    prefix_unified = self.get_submodel_prefix(id_graph)
                    unified_name = _name if len(re.findall(
                        prefix_unified, output_name)) >= 1 else prefix_unified + _name

                    self.addOperand(unified_name, op)
                    self.print_debug("prefix_unified/unified_name: {}/{}".format(
                        prefix_unified, unified_name))
                ...
                for n in self.converted_nodes:
                    ...
                    n.inputs = [prefix_unified + each_name
                                for each_name in n.inputs] if len(n.inputs) >= 1 else []
            ...
            #PPLOp
                ...
                for j in range(nums_PPL_input_per):
                    ...
                    output_name = output_name if len(re.findall(
                        prefix_isolated, output_name)) >= 1 else prefix_isolated + output_name
                    self.print_debug("[{}-prefix_isolated/output_name]: {}, {}".format(
                        output_id, prefix_isolated, output_name))
                    op = self.getOperand(output_name)

                    _name = "{}_input_{}".format(ppl_op_type, j)
                    unified_name = ppl_op_name + _name
                    self.print_debug("[PPL-unified_name/ppl_op_name/name]: {}/{}/{}".format(
                        unified_name, ppl_op_name, _name))
                    self.print_debug("-------------------------------")
                    self.addOperand(unified_name, op)
                    ppl_op_input_names.append(unified_name)

                cur_output_names = self.ppl_factory.get(ppl_op_type, lambda x: NoneAndRaise(x))(
                    ppl_op_input_names, ppl_op_type, id_PPLOp)
                ...
                if idx == len(self.list_encoded_block) - 1:
                    assert len(self.output_names) == len(
                        cur_output_names), "[Output-Shape Error] last sub_block is PPLOp,\
                       which num_outputs is {} != given num_outputs={}".format(
                            len(cur_output_names), len(self.output_names))
        ...
        for i in range(self.maskrcnn_num_output):
            ...
            unified_prefix = "" if self.list_encoded_block[
                id_Block] == self.Symbol_PPLSubBlock else self.get_structure_prefix(id_Block)
    ...
    def convert_RPN_get_bboxes_op(self, input_list: list, op_type: str, id_PPLOp: int):
        ...
        def _gen_base_anchors():
            ...
            for i, base_size in enumerate(base_sizes):
                multi_level_base_anchors.append(
                    _gen_single_level_base_anchors(base_size, scales=scales, ratios=ratios))
        ...
        def _grid_anchors(featmap_sizes):
            ...
            for i in range(Global_num_levels):
                anchors = _single_level_grid_anchors(base_anchors[i], featmap_sizes[i],
                                                     Global_strides[i])
        ...
        dynamic_featmap_sizes = [[
            div_up_internal(self.H_RPN_DYN_MAX, 2**i),
            div_up_internal(self.W_RPN_DYN_MAX, 2**i)
