...
class OnnxConverter(BaseConverter):
            ...
            if n.op_type in ["Gather"]:
                ...
                for input in n.input:
                    input_shape[input] = self.get_shape_for_node(self.model.graph.input,
                                                                 ...
                                                                 self.model.graph.value_info, input)
                output_shape = dict()
                for output in n.output:
                    output_shape[output] = self.get_shape_for_node(self.model.graph.input,
                                                                   self.model.graph.output,
    ...
    def convert_resize_op(self, onnx_node):
        ...
        if (len(scale_factor) == 5 and not use_size and scale_factor[-3] > 1.0
                and scale_factor[-2] > 1.0 and scale_factor[-1] > 1.0
                and coord_mode == b'half_pixel' and mode == b'linear'):
            ...
            D_out, H_out, W_out = int(actual_output_shape[-3]), int(actual_output_shape[-2]), int(
    ...
    def convert_mean_std_scale_op(self, onnx_node):
        ...
        new_op = top.MeanStdScaleOp(
            ...
            customization_format=StringAttr.get(onnx_node.attrs.get('customization_format',
                                                                    'NCHW')),
            channel_order=StringAttr.get(onnx_node.attrs.get('channel_order', 'RGB')),
    ...
    def convert_qlinear_op(self, onnx_node):
        ...
        y_zero_point = np.array(self.getWeight(onnx_node.inputs[2])).astype(np.int32).reshape(
            [-1]).tolist()
        axis = None
    ...
    def convert_deqlinear_op(self, onnx_node):
        ...
        x_zero_point = np.array(self.getWeight(onnx_node.inputs[2])).astype(np.int32).reshape(
            [-1]).tolist()
        axis = None
        if hasattr(onnx_node, 'attrs'):
            axis = onnx_node.attrs.get('axis', None)
