...
class ONNX_IR_TESTER(object):
    ...
    def __init__(self,
        ...
        self.test_cases = {
            ...
            "AddBcast":     (self.test_AddBcast,      Y, Y, N),
            "AddBcast2":    (self.test_AddBcast2,     Y, Y, N),
            "AddBcast3":    (self.test_AddBcast3,     N, N, N),  # failed cases
            "Arg":          (self.test_Arg,           Y, Y, Y),
            "AddWeight":     (self.test_AddWeight,    Y, Y, Y),
            "AvgPool1d":    (self.test_AvgPool1d,     Y, Y, Y),
            "AvgPool2d":    (self.test_AvgPool2d,     Y, Y, Y),
            "AvgPool3d":    (self.test_AvgPool3d,     Y, Y, Y),
            "AvgPoolOdd":   (self.test_AvgPoolOdd,    Y, Y, Y),
            "PadAvgPool2d": (self.test_PadAvgPool2d,  Y, N, Y),
            "BatchMatMul":  (self.test_BatchMatMul,   Y, N, Y),
            "BCastAdd":     (self.test_BCastAdd,      Y, Y, Y),
            "BCastMul":     (self.test_BCastMul,      Y, Y, Y),
            "BCastMulCst":  (self.test_BCastMulCst,   Y, Y, Y),
            ...
            "ConvStride":   (self.test_ConvStride,    Y, Y, Y),
            "ConvDw":       (self.test_ConvDw,        Y, Y, Y),
            "ConvTrans":    (self.test_ConvTrans,     Y, N, Y),
            "ConvTrans2":   (self.test_ConvTrans2,    Y, N, Y),  #no pad
            "Clip":         (self.test_Clip,          Y, Y, Y),
            "DepthToSpace": (self.test_DepthToSpace,  Y, Y, Y),
            "Deconv":       (self.test_Deconv,        Y, N, N),
            "Div":          (self.test_Div,           Y, Y, Y),
            "DivBcast":     (self.test_DivBcast,      Y, Y, N),
            "DivBcast2":    (self.test_DivBcast2,     Y, Y, N),
            "Einsum":       (self.test_Einsum,        Y, Y, Y),
            "Einsum2":      (self.test_Einsum2,       Y, Y, Y),
            "Elu":          (self.test_Elu,           Y, Y, N),
            "Erf":          (self.test_Erf,           Y, Y, N),
            "Exp":          (self.test_Exp,           Y, Y, Y),
            "Expand":       (self.test_Expand,        Y, Y, Y),
            "Expand2":      (self.test_Expand2,       Y, Y, Y),
            "Floor":        (self.test_floor,         Y, Y, N),
            "Gather":       (self.test_Gather,        Y, N, Y),
            # "Gather2":      (self.test_Gather2,     Y, N, Y),
            "Gemm":         (self.test_Gemm,          Y, Y, Y),
            "GroupFC":      (self.test_GroupFC,       Y, Y, Y),
            "GRU":          (self.test_GRU,           Y, Y, Y),  # test gru output Y
            "GRU2":         (self.test_GRU2,          Y, Y, Y),  # test gru output Yh
            "GRU3":         (self.test_GRU3,          Y, Y, Y),  # test gru output Y and Yh
            "LeakyRelu":    (self.test_LeakyRelu,     Y, Y, Y),
            "Log":          (self.test_Log,           Y, Y, Y),
            "LogSoftmax":   (self.test_LogSoftmax,    Y, Y, Y),
            "LRN":          (self.test_LRN,           Y, Y, Y),
            "LSTM":         (self.test_LSTM,          Y, Y, Y),  # output_y
            "LSTM2":        (self.test_LSTM2,         Y, Y, Y),  # output all
            "LSTM3":        (self.test_LSTM3,         Y, Y, Y),  # output_yh and output_yc
            "MaxPool1d":    (self.test_MaxPool1d,     Y, Y, Y),
            "MaxPool2d":    (self.test_MaxPool2d,     Y, Y, Y),
            "MaxPool3d":    (self.test_MaxPool3d,     N, Y, Y),
            ...
            "MinBcast":     (self.test_MinBcast,      Y, Y, N),
            "MulConst":     (self.test_MulConst,      Y, Y, Y),
            "Neg":          (self.test_Neg,           Y, Y, Y),
            "Pad":          (self.test_Pad,           Y, N, Y),  # zero pad
            "Pad1":         (self.test_Pad1,          Y, N, Y),  # pad val
            "PadEdge":      (self.test_PadEdge,       Y, Y, Y),
            "PadReflect":   (self.test_PadReflect,    Y, Y, Y),
            "Pow1":         (self.test_Pow1,          Y, Y, Y),  # y = x ^ n
            "Pow2":         (self.test_Pow2,          N, N, N),  # y = n ^ x
            "PRelu":        (self.test_PRelu,         Y, Y, Y),
            # "Range":        (self.test_Range,       Y, Y, N),
            "Resize":       (self.test_Resize,        Y, Y, Y),
            "Resize2":      (self.test_Resize2,       Y, N, Y),
            "Reshape":      (self.test_Reshape,       Y, Y, N),
            "Reduce":       (self.test_Reduce,        Y, Y, Y),
            "Reduce2":      (self.test_Reduce2,       Y, Y, Y),
            "ReduceL2":     (self.test_ReduceL2,      Y, Y, Y),
            "ReduceMean":   (self.test_ReduceMean,    Y, Y, Y),
            "ReduceSum":    (self.test_ReduceSum,     Y, Y, Y),
            "ReduceProd":   (self.test_ReduceProd,    Y, Y, N),
            "Reciprocal":   (self.test_Reciprocal,    Y, Y, Y),
            "Relu":         (self.test_Relu,          Y, Y, Y),
            "PermuteMove":  (self.test_PermuteMove,   Y, Y, Y),
            "ScatterND":    (self.test_ScatterND,     Y, N, N),
            "Shape":        (self.test_Shape,         Y, Y, N),
            "SiLU":         (self.test_SiLU,          Y, Y, Y),
            "Softmax":      (self.test_Softmax,       Y, Y, Y),
            "Softplus":     (self.test_Softplus,      Y, Y, Y),
            "Squeeze":      (self.test_Squeeze,       Y, Y, Y),
            "Sigmoid":      (self.test_Sigmoid,       Y, Y, Y),
            "Slice":        (self.test_Slice,         Y, Y, Y),
            "Slice2":       (self.test_Slice2,        Y, Y, Y),
            "Slice3":       (self.test_Slice3,        Y, Y, Y),
            "Split":        (self.test_Split,         Y, Y, Y),
            "Scale":        (self.test_Scale,         Y, Y, Y),
            "Sqrt":         (self.test_Sqrt,          Y, Y, Y),
            "Sub":          (self.test_Sub,           Y, Y, Y),
            "Sub2":         (self.test_Sub2,          Y, Y, Y),
            "SubBcast":     (self.test_SubBcast,      Y, Y, N),
            "SubBcast2":    (self.test_SubBcast2,     Y, Y, N),
            "SubConst":     (self.test_SubConst,      Y, Y, N),
            "SubConst2":    (self.test_SubConst2,     Y, Y, Y),
            "Sum":          (self.test_Sum,           Y, Y, Y),
            "Tanh":         (self.test_Tanh,          Y, Y, Y),
            "Tile":         (self.test_Tile,          Y, Y, Y),
            "Transpose":    (self.test_Transpose,     Y, Y, Y),
            "Transpose2":   (self.test_Transpose2,    Y, Y, Y),
            "TopK":         (self.test_TopK,          Y, Y, N),
            "Upsample":     (self.test_Upsample,      Y, Y, N),
            "Where":        (self.test_Where,         Y, Y, Y),
            #####################################
            # Torch Test Case, Alphabetically
            #####################################
            # case: (test, bm1684x_support, bm1686_support, cv183x_support)
            "TorchActivation":      (self.test_TorchActivation,     N, Y, Y),
            "TorchArgmax":          (self.test_TorchArgmax,         N, N, N),
            "TorchChannelShuffle":  (self.test_TorchChannelShuffle, N, N, N),
            "TorchChunk":           (self.test_TorchChunk,          Y, Y, Y),
            "TorchConv2d":          (self.test_TorchConv2d,         N, N, Y),
            "TorchConv3dTrans":     (self.test_TorchConv3dTrans,    Y, Y, Y),
            "TorchHardSwish":       (self.test_TorchHardSwish,      Y, Y, Y),
            "TorchHardSigmoid":     (self.test_TorchHardSigmoid,    Y, Y, Y),
            "TorchGelu":            (self.test_TorchGelu,           Y, Y, Y),
            "TorchGroupNorm":       (self.test_TorchGroupNorm,      Y, Y, N),
            "TorchGroupNorm2":      (self.test_TorchGroupNorm2,     Y, Y, N),
            "TorchGRU":             (self.test_TorchGRU,            Y, Y, Y),
            "TorchIdentity":        (self.test_TorchIdentity,       Y, Y, Y),
            "TorchIndexCopy":       (self.test_TorchIndexCopy,      N, N, N),
            "TorchInstanceNorm":    (self.test_TorchInstanceNorm,   Y, Y, N),
            "TorchInstanceNorm2":   (self.test_TorchInstanceNorm2,  Y, Y, N),
            "TorchLayerGroup":      (self.test_TorchLayerGroup,     Y, Y, Y),
            "TorchLayerNorm":       (self.test_TorchLayerNorm,      Y, Y, Y),
            "TorchLayerNorm2":      (self.test_TorchLayerNorm2,     Y, Y, Y),
            "TorchLogSoftmax":      (self.test_TorchLogSoftmax,     Y, Y, Y),
            "TorchLSTM":            (self.test_TorchLSTM,           Y, Y, Y),
            "TorchMaskedFill":      (self.test_TorchMaskedFill,     Y, Y, N),
            "TorchNonZero":         (self.test_TorchNonZero,        Y, Y, N),
            "TorchReflectionPad":   (self.test_TorchReflectionPad,  Y, Y, Y),
            "TorchRoiAlign":        (self.test_TorchRoiAlign,       Y, Y, N),
            "TorchSize":            (self.test_TorchSize,           Y, Y, Y),
            "TorchStd":             (self.test_TorchStd,            Y, Y, Y),
            "TorchWhere":           (self.test_TorchWhere,          Y, Y, N),
            "TorchZeroPad":         (self.test_TorchZeroPad,        Y, N, Y),
            #########################################
            # Special Pass test case, Alphabetically
            #########################################
            # case: (test, bm1684x_support, bm1686_support, cv183x_support)
            "ArgError":         (self.test_ArgError,        Y, Y, N),
            "ArgReducefull":    (self.test_ArgReducefull,   Y, Y, N),
            "ConcatFuse":       (self.test_ConcatFuse,      Y, Y, Y),
            "ConcatToSpace":    (self.test_ConcatToSpace,   Y, Y, N),
            "Conv3dTo2d":       (self.test_Conv3dTo2d,      Y, Y, Y),
            "Div2Mul":          (self.test_Div2Mul,         Y, Y, Y),
            "ConvSlice":        (self.test_ConvSlice,       Y, Y, N),
            "GaToSlice":        (self.test_GaToSlice,       Y, N, Y),
            "Mul2Scale":        (self.test_Mul2Scale,       Y, Y, Y),
            ...
            "PadConv2d":        (self.test_PadConv2d,       Y, Y, Y),
            "PadConv3d":        (self.test_PadConv3d,       N, N, N),
            "PadPool1d":        (self.test_PadPool1d,       N, Y, Y),
            ...
            "PermuteToReorg2":  (self.test_PermuteToReorg2, Y, Y, Y),
            "PermuteToReshape": (self.test_PermuteToReshape,Y, Y, N),
            "Permute5dSplit":   (self.test_Permute5dSplit,  Y, Y, Y),
            "PoolAfterRelu":    (self.test_PoolAfterRelu,   Y, Y, Y),
            "PoolSignError":    (self.test_PoolSignError,   Y, Y, Y),
            "ReshapeFuse":      (self.test_ReshapeFuse,     Y, Y, Y),
            "ReduceTranspose":  (self.test_ReduceTranspose, Y, Y, N),
            "ReduceFuse":       (self.test_ReduceFuse,      Y, Y, Y),
            "SwapDimInner":     (self.test_SwapDimInner,    Y, Y, N),
            "SliceToReverse":   (self.test_SliceToReverse,  Y, Y, N),
            "StaticDynMixed":   (self.test_StaticDynMixed,  Y, N, N),
            "TransposeArg":     (self.test_TransposeArg,    Y, Y, Y),
