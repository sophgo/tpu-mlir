...
#         case5 = [[1, 1, 100, 100], [1, 1, 9998, 9998], [1, 1, 99.98, 99.98], [4], 'linear',
#                  'align_corners']
#         case6 = [[1, 1, 736, 1280], [1, 1, 184, 320], [1, 1, 1 / 4, 1 / 4], [4], 'linear',
#                  'align_corners']
#         case7 = [[1, 128, 92, 160], [1, 128, 23, 40], [1, 1, 1 / 4, 1 / 4], [4], 'nearest',
#                  'asymmetric']
#         case8 = [[1, 1, 544, 960], [1, 1, 272, 480], [1, 1, 0.5, 0.5], [4], 'nearest', 'asymmetric']
#         cases = (case0, case1, case2, case3, case4, case5, case6, case7,
#                  case8) if self.chip in ['bm1684x', 'bm1688', 'sg2380'] else (case0, )
#         if self.chip in ['cv184x']:
#             cases = (case0, case1, case2, case3, case4, case6, case7, case8)
...
#     def test_TopKTrans(self, case_name):
...
#                 k = 128
#                 axis = 1
#                 values, indices = torch.topk(x, k, dim=axis)
#                 return values, indices

#         x = torch.randn(1, 976, 1, 1).float()

#         self.torch_and_test(x, Model(), case_name, support_modes=["f32"])
...
class ONNX_IR_TESTER(object):
    ...
    def __init__(self,
        ...
        self.test_cases = {
            ...
            "DivConst":     (self.test_DivConst,      N, Y, Y, Y, Y, Y),
            ...
            "MaskedFill":   (self.test_MaskedFill,    N, Y, Y, N, Y, Y),
            "MaskRCNNBboxPooler": (self.test_MaskRCNNBboxPooler, N, Y, Y, N, Y, N),
            "MaskRCNNGetBboxB": (self.test_MaskRCNNGetBboxB, N, Y, Y, N, Y, N),
            "MaskRCNNMaskPooler": (self.test_MaskRCNNMaskPooler, N, Y, Y, N, Y, N),
            "MatchTemplate": (self.test_MatchTemplate,  N, Y, Y, N, Y, N),
            "MaxBcast":     (self.test_MaxBcast,      Y, Y, Y, N, Y, Y),
            "MaxConst":     (self.test_MaxConst,      N, Y, Y, Y, Y, Y),
            "MaxPoolingIndicesBwd": (self.test_MaxPoolingIndicesBwd, N, Y, Y, N, Y, N),
            "MaxPoolWithMask": (self.test_MaxPoolWithMask,  N, Y, Y, N, Y, N),
            "MaxUnpool": (self.test_MaxUnpool,  N, Y, Y, N, Y, N),
            "MeanRstd": (self.test_MeanRstd,  N, Y, Y, N, Y, N),
            "MeanStdScale": (self.test_MeanStdScale,  N, Y, Y, N, Y, N),
            "MeshGrid": (self.test_MeshGrid,  N, Y, Y, N, Y, N),
            "Mish": (self.test_Mish,  N, Y, Y, N, Y, N),
            "ScaleLut": (self.test_ScaleLut,  N, Y, Y, N, Y, N),
            "ShuffleChannel": (self.test_ShuffleChannel,  N, Y, Y, N, Y, N),
            "Sin":          (self.test_Sin,          N, Y, Y, N, Y, N),
            "Sinh":         (self.test_Sinh,         N, Y, Y, N, Y, N),
            "Tan":          (self.test_Tan,          N, Y, Y, N, Y, N),
            "SliceAxis":    (self.test_SliceAxis,    N, Y, Y, N, Y, N),
            "Softsign":     (self.test_Softsign,     N, Y, Y, N, Y, N),
            "Sort":         (self.test_Sort,         N, Y, Y, N, Y, N),
            "StridedSlice": (self.test_StridedSlice, N, Y, Y, N, Y, N),
            "SwapChannel":  (self.test_SwapChannel,  N, Y, Y, N, Y, N),
            "Swish":        (self.test_Swish,        N, Y, Y, N, Y, N),
            ...
            "MinConst":     (self.test_MinConst,      N, Y, Y, Y, Y, Y),
            ...
            "Pow":          (self.test_Pow1,          Y, Y, Y, Y, Y, Y), # y = x ^ n
            ...
            "QuantizeLinear": (self.test_QuantizeLinear, N, Y, Y, N, Y, N),
            ...
            "TopKTrans":    (self.test_TopKTrans,     N, Y, N, N, N, N),
            "Upsample":     (self.test_Upsample,      Y, Y, Y, N, Y, Y),
            "Pack":         (self.test_Pack,          Y, Y, Y, N, Y, Y),
            "Unpack":       (self.test_Unpack,        Y, Y, Y, N, Y, Y),
    ...
    def onnx_convert(self,
        ...
        try:
            ...
            print("Skip onnx inference (custom op), use top mlir as reference")
            onnx_outs = {}
        show_fake_cmd(input_npz, fp32_mlir, "top_out.npz")
        top_mlir_outs = mlir_inference(input_data, fp32_mlir, True)
        small_onnx_outs = None
    ...
    def onnx_and_test(self,
        ...
        if counter == 0:
            if onnx_outs:
                raise RuntimeError("No compare between onnx outs and mlir outts")
            print("Skip ONNX comparison (custom op), continue with bmodel test")
        else:
            print("Success: ONNX outs and Mlir outs are equal\n")
        if small_top_mlir_outs:
            for name in small_onnx_outs:
    ...
    def test_Pack(self, case_name):
        ...
        inp_infos = [helper.make_tensor_value_info(f'i{i}', TensorProto.FLOAT, shape)
                     for i in range(3)]
        out_info = helper.make_tensor_value_info('o', TensorProto.FLOAT, [1, 3, 3, 32, 32])
        node_def = helper.make_node(
            "Pack", inputs=[f'i{i}' for i in range(3)], outputs=["o"],
            domain="tpu_mlir", axis=1)
        graph_def = helper.make_graph([node_def], case_name, inp_infos, [out_info])
        self.onnx_and_test(graph_def)

    def test_Unpack(self, case_name):
        shape = [1, 4, 32, 32]
        inp_info = helper.make_tensor_value_info('input', TensorProto.FLOAT, shape)
        out_infos = [helper.make_tensor_value_info(f'o{i}', TensorProto.FLOAT, [1, 1, 32, 32])
                     for i in range(4)]
        node_def = helper.make_node(
            "Unpack", inputs=["input"],
            ...
            domain="tpu_mlir", axis=1)
        graph_def = helper.make_graph(
            [node_def], case_name, [inp_info], out_infos)
        self.onnx_and_test(graph_def)
    ...
    def test_Resize(self, case_name):
        ...
        cases = (case0, case1, case2, case3, case4, case5, case6,
                 case7) if self.chip in ['bm1684x', 'bm1688', 'sg2380'] else (case0, )
    ...
    def test_MinConst(self, case_name):
        input_shape = [1, 3, 27, 27]
        output_shape = [1, 3, 27, 27]
        ...
        graph_txt = """
            ...
            <float const_min = {-2.0}>
            {
                output = Min(input, const_min)
            }
            """ % (case_name, input_shape, output_shape)
        graph_def = onnx.parser.parse_graph(graph_txt)
        self.onnx_and_test(graph_def)

    def test_MaxConst(self, case_name):
        input_shape = [1, 3, 27, 27]
        output_shape = [1, 3, 27, 27]
    ...
    def test_Mish(self, case_name):
        ...
        inp_info = helper.make_tensor_value_info('input', TensorProto.FLOAT, shape)
        out_info = helper.make_tensor_value_info('output', TensorProto.FLOAT, shape)

        node_def = helper.make_node(
            "Mish", inputs=["input"], outputs=["output"], domain="tpu_mlir")
    ...
    def test_ShuffleChannel(self, case_name):
        ...
        node_def = helper.make_node(
            "ShuffleChannel", inputs=["input"], outputs=["out"],
            domain="tpu_mlir", group=4)
    ...
    def test_SliceAxis(self, case_name):
        ...
        inp_info = helper.make_tensor_value_info('input', TensorProto.FLOAT, shape)
        out_info = helper.make_tensor_value_info('out', TensorProto.FLOAT, [1, 3, 16, 32])

        node_def = helper.make_node(
            "SliceAxis",
            inputs=["input", "axis", "start", "end", "step"],
            outputs=["out"],
            domain="tpu_mlir")
        ...
        graph_def = helper.make_graph(
            [node_def], case_name, [inp_info], [out_info])
        graph_def.initializer.extend([
            helper.make_tensor('axis', TensorProto.FLOAT, [1], [2.0]),
            helper.make_tensor('start', TensorProto.FLOAT, [1], [0.0]),
            helper.make_tensor('end', TensorProto.FLOAT, [1], [16.0]),
            helper.make_tensor('step', TensorProto.FLOAT, [1], [2.0]),
        ])
        self.onnx_and_test(graph_def)
    ...
    def test_StridedSlice(self, case_name):
        ...
        out_info = helper.make_tensor_value_info('y', TensorProto.FLOAT, [1, 3, 8, 16])

        node_def = helper.make_node(
            "StridedSlice", inputs=["x", "starts", "ends", "strides"], outputs=["y"],
            domain="tpu_mlir", begin_mask=0, end_mask=0)

        graph_def = helper.make_graph(
            [node_def], case_name, [inp_info], [out_info])
        graph_def.initializer.extend([
            helper.make_tensor('starts', TensorProto.FLOAT, [4], [0, 0, 0, 0]),
            helper.make_tensor('ends', TensorProto.FLOAT, [4], [1, 3, 16, 32]),
            helper.make_tensor('strides', TensorProto.FLOAT, [4], [1, 1, 2, 2]),
        ])
        self.onnx_and_test(graph_def)
    ...
    def test_SwapChannel(self, case_name):
        ...
        node_def = helper.make_node(
            "SwapChannel", inputs=["x"], outputs=["y"],
            domain="tpu_mlir", channel_order=[2, 1, 0])
    ...
    def test_Swish(self, case_name):
        ...
        node_def = helper.make_node(
            "Swish", inputs=["x"], outputs=["y"],
            domain="tpu_mlir", beta=1.0)
    ...
    def test_QuantizeLinear(self, case_name):
        ...
        zp = helper.make_tensor('zp', TensorProto.INT8, [1], [0])
