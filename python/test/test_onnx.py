...
class ONNX_IR_TESTER(object):
    ...
    def inference_and_compare(self,
        ...
        npz_compare([tpu_npz, model_npz, "--tolerance", "0.95,0.80", "-vv"])
    ...
    def test_AdaptiveAvgPool(self, case_name):
        ...
        for idx, (input_shape, output_size) in enumerate(cases):
            ...
            onnx_model = onnx.load(onnx_file)
            onnx_outs, top_mlir_outs, _, _, _, node_name_mapping = self.onnx_convert(
                input_data, None, onnx_model.graph, model_name, ["f32"])
            counter = 0
            for name in onnx_outs:
                if name in top_mlir_outs:
                    print("Compare onnx and mlir:{}\n".format(name))
                    self.compare(onnx_outs[name].ravel(), top_mlir_outs[name].ravel())
                    counter += 1
                elif name in node_name_mapping:
                    mapped_name = node_name_mapping[name]
                    if mapped_name in top_mlir_outs:
                        print("Compare onnx and mlir:{}\n".format(mapped_name))
                        self.compare(onnx_outs[name].ravel(), top_mlir_outs[mapped_name].ravel())
                        counter += 1
            if counter == 0:
            ...
            if self.test_cuda:
                ...
                output_type = "tensor<{}xf32>".format("x".join(map(str, output_shape)))
                origin_mlir = model_name + "_origin.mlir"
                adaptive_mlir = model_name + "_adaptive_origin.mlir"
                with open(origin_mlir, "r") as f:
                    mlir_text = f.read()
                with open(adaptive_mlir, "w") as f:
                    f.write(mlir_text.replace("tensor<*xf32>", output_type))
                file_mark(adaptive_mlir)
                cuda_npz = model_name + "_top_cuda_out.npz"
                cuda_outs = mlir_inference(input_data, adaptive_mlir, dump_all=True, use_cuda=True)
                np.savez(cuda_npz, **cuda_outs)
                file_mark(cuda_npz)
                npz_compare([cuda_npz, self.ref_npz, "--tolerance", "0.9999,0.9999", "-v"])
    ...
    def test_GRU(self, case_name):
        ...
        onnx_outs, top_mlir_outs, _, _, _, node_name_mapping = self.onnx_convert(
            input_data, None, graph_def, case_name, ["f32"])
        counter = 0
        for name in onnx_outs:
            if name in top_mlir_outs:
                print("Compare onnx and mlir:{}\n".format(name))
        ...
        if self.test_cuda:
            cuda_npz = case_name + "_top_cuda_out.npz"
            cuda_outs = mlir_inference(input_data,
                                       case_name + ".mlir",
                                       dump_all=True,
                                       use_cuda=True)
            np.savez(cuda_npz, **cuda_outs)
    ...
    def test_Resize(self, case_name):
        ...
        cases = (case0, case1, case2, case3, case4, case5, case6, case7,
                 case8) if self.chip in ['bm1684x', 'bm1688', 'sg2380', 'bm1684x2'] else (case0, )
    ...
    def test_DivConst(self, case_name):
        ...
        input_data = {"in_0": input_.data.numpy().astype(np.float32)}
        model = Model()
        ...
        torch.onnx.export(model,
                          ...
                          opset_version=14,
                          input_names=["in_0"])
        self.torch_and_onnx_compare(input_data, onnx_file, model(input_))

        onnx_model = onnx.load(onnx_file)
        onnx_outs, top_mlir_outs, _, _, _, node_name_mapping = self.onnx_convert(
            input_data, None, onnx_model.graph, case_name, ["f32"])
        counter = 0
        for name in onnx_outs:
            if name in top_mlir_outs:
                print("Compare onnx and mlir:{}\n".format(name))
    ...
    def test_AffineMap(self, case_name, n=1):
        ...
        class Model3_SliceConvMulAdd(nn.Module):
            ...
            def forward(self, x):
                ...
                x = self.conv(x)
                x = x.permute(1, 0, 2, 3)  # (2, 8, 24, 32) -> (8, 2, 24, 32)
                x = x * 2.0
                x = x + 1.0
                return x
        ...
        class Model5_PermReshapeSliceConvMulAdd(nn.Module):
            ...
            def forward(self, x):
                ...
                x = x.reshape(16, 8, 4, 32)
                x = x[:, :, :, 4:28]  # slice on last dimension
                x = self.conv(x)
                x = x * 2.0
                x = x + 1.0
                return x
        ...
        class Model6_ConvMulAddPermReshapeSlice(nn.Module):
            ...
            def forward(self, x):
                ...
                x = x * 2.0
                x = x + 1.0
                x = x.permute(0, 2, 1, 3)  # (2, 8, 32, 32) -> (2, 32, 8, 32)
                x = x.reshape(2, 32, 8, 32)
                x = x[:, 4:28, :, :]  # slice on second dimension
                return x
        ...
        class Model7_ConvMulAddPermReshapeSlicePad(nn.Module):
            ...
            def forward(self, x):
                ...
                x = x * 2.0
                x = x + 1.0
                x = x.permute(1, 2, 0, 3)  # (2, 8, 32, 32) -> (8, 32, 2, 32)
                x = x.reshape(8, 32, 2, 32)
                x = x[4:8, :, :, :]  # slice on first dimension
                x = F.pad(x, (1, 1, 0, 0, 0, 0, 0, 0), mode='constant', value=0)
                return x
    ...
    def test_GatherND(self, case_name):
        ...
        for i in range(len(input_datas)):
            ...
            graph_def.initializer.extend([indices, add_const])
            model_name = "{}_{}".format(case_name, i)
            onnx_outs, top_mlir_outs, _, _, _, node_name_mapping = self.onnx_convert(
                input_data, None, graph_def, model_name, ["f32"])
            counter = 0
            for name in onnx_outs:
                if name in top_mlir_outs:
                    print("Compare onnx and mlir:{}\n".format(name))
    ...
    def test_TorchRoiExtract(self, case_name):
        ...
        def gen_rois(N, H, W, n):
            bi = torch.randint(0, N, (n, ), dtype=torch.int32).float()
            xl, xh = torch.rand(n) * (W - 1), torch.rand(n) * (W - 1)
            yl, yh = torch.rand(n) * (H - 1), torch.rand(n) * (H - 1)
            for i in range(n):
                if xl[i] > xh[i]: xl[i], xh[i] = xh[i], xl[i]
                if yl[i] > yh[i]: yl[i], yh[i] = yh[i], yl[i]
                ...
                for oc in range(C_out):
                    ...
                    for ic in range(C_in):
                        ...
                        for kh in range(Kh):
                            cols = []
                            for kw in range(Kw):
                                window = padded[:, ic, kh:kh + H_out:stride, kw:kw + W_out:stride]
