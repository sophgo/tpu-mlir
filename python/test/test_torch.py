...
class TORCH_IR_TESTER(object):
    ...
    def __init__(self,
        ...
        self.test_cases = {
            ...
            "Abs":              (self.test_Abs,               Y, Y, N),
            "Activation":       (self.test_Activation,        Y, Y, N),
            "AdaptiveAvgPool2d":(self.test_AdaptiveAvgPool2d, Y, Y, N),
            "Add":              (self.test_Add,               Y, Y, Y),
            "Addmm":            (self.test_Addmm,             Y, Y, N),
            "Arange":           (self.test_Arange,            Y, Y, N),
            "Attention":        (self.test_Attention,         Y, Y, N),
            "AvgPool1d":        (self.test_AvgPool1d,         Y, Y, N),
            "AvgPool2d":        (self.test_AvgPool2d,         Y, Y, Y),
            "AvgPool3d":        (self.test_AvgPool3d,         Y, Y, N),
            "BatchNorm":        (self.test_BatchNorm,         Y, Y, Y),
            "BMM":              (self.test_BatchMatMul,       Y, Y, Y),
            "ChannelShuffle":   (self.test_ChannelShuffle,    Y, Y, N),
            "Chunk":            (self.test_Chunk,             Y, Y, N),
            "Compare":          (self.test_Compare,           Y, Y, N),
            "Concat":           (self.test_Concat,            Y, Y, N),
            ...
            "LayerNorm":        (self.test_LayerNorm,         Y, Y, N),
            "LeakyRelu":        (self.test_LeakyRelu,         Y, Y, N),
            "Linear":           (self.test_Linear,            Y, Y, N),
            "LogSoftmax":       (self.test_LogSoftmax,        Y, Y, N),
            "LSTM":             (self.test_LSTM,              Y, Y, N),
            "Math":             (self.test_Math,              Y, Y, N),
            "MatMul":           (self.test_MatMul,            Y, Y, N),
            "MaxPool1d":        (self.test_MaxPool1d,         Y, Y, N),
            "MaxPool2d":        (self.test_MaxPool2d,         Y, Y, N),
            "MaxPool3d":        (self.test_MaxPool3d,         Y, Y, N),
            "MM":               (self.test_MM,                Y, Y, N),
            "Mul":              (self.test_Mul,               Y, Y, Y),
            "NewZeros":         (self.test_NewZeros,          Y, Y, N),
            "Reduce":           (self.test_Reduce,            Y, Y, N),
            "Repeat":           (self.test_Repeat,            Y, Y, N),
            "Reshape":          (self.test_Reshape,           Y, Y, N),
            "PixelShuffle":     (self.test_PixelShuffle,      Y, Y, N),
            "PRelu":            (self.test_PRelu,             Y, Y, N),
            "Permute":          (self.test_Permute,           Y, Y, N),
            "Pad1d":            (self.test_Pad1d,             Y, Y, N),
            "Pad2d":            (self.test_Pad2d,             Y, Y, N),
            "Pow":              (self.test_Pow,               Y, Y, N),
            "Scatter":          (self.test_Scatter,           N, N, N),
            "Select":           (self.test_Select,            Y, Y, N),
            "Slice":            (self.test_Slice,             Y, Y, N),
            "Softmax":          (self.test_Softmax,           Y, N, N),
            "Softmin":          (self.test_Softmin,           Y, Y, N),
            "Split":            (self.test_Split,             Y, Y, N),
            "Squeeze":          (self.test_Squeeze,           Y, Y, N),
            "Stack":            (self.test_Stack,             Y, Y, N),
            "Sub":              (self.test_Sub,               Y, Y, Y),
            "T":                (self.test_T,                 Y, Y, N),
            "Tile":             (self.test_Tile,              Y, Y, N),
            "To":               (self.test_To,                N, N, N),
            "Type_as":          (self.test_Type_as,           N, N, N),
            "Transpose":        (self.test_Transpose,         Y, Y, N),
            "Upsample":         (self.test_Upsample,          Y, N, N),
            "Unary":            (self.test_Unary,             Y, Y, N),
            "Unsqueeze":        (self.test_Unsqueeze,         Y, Y, N),
            "View":             (self.test_View,              Y, Y, N),
            "Where":            (self.test_Where,             Y, Y, N),
            ## Special Case
            "SplitReshape":     (self.test_SplitReshape,      Y, Y, N),
        }
        # yapf: enable
