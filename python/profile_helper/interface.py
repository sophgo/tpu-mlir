...
def bmprofile_parse_perfAI(input_dir: str, output_dir: str, mark_str, arch="A2", debug=False):
    ...
    if not debug:
        subprocess.run([
            f"python  $PROJECT_ROOT/python/PerfAI/PerfAI.doc/run_doc.py {os.path.abspath(output_dir)} 2 --style 1"
        ],
                       shell=True)
        subprocess.run([
            f"python  $PROJECT_ROOT/python/PerfAI/PerfAI.web/run_web.py {os.path.abspath(output_dir)} --name PerfAI_web"
        ],
                       shell=True)


def bmprofile_parse_layergroup_log(log_file: str, perfdoc, bmodel, *args):
    BMProfileLLayerGroup(log_file, perfdoc, bmodel).parse()
