...
NB_MODULE(pymlir, m) {
  m.doc() = "nanobind for TPU-MLIR";
  m.attr("__version__") = mlir_version;
  m.attr("support_cuda") = support_cuda;
  m.def("debug", &debug, nb::arg("enable") = true,
  ...
  nb::class_<quant_brief_info>(m, "q_info", "simple tensor quant info")
      .def_ro("dtype", &quant_brief_info::dtype)
      .def_ro("shape", &quant_brief_info::shape)
      .def_ro("scale", &quant_brief_info::scale)
      .def_ro("zp", &quant_brief_info::zp);
  // clang-format off
  nb::class_<py_module>(m, "module", "MLIR Module")
      .def(nb::init<>())
      ...
      .def("invoke", &py_module::invoke, nb::arg("fixed_to_float")=true)
      ...
      .def_ro("input_names", &py_module::input_names)
      .def_ro("output_names", &py_module::output_names)
      .def_ro("all_tensor_names", &py_module::all_tensor_names)
      .def_ro("all_weight_names", &py_module::all_weight_names);
#ifdef USE_CUDA
  nb::class_<py_cuda>(m, "cuda", "MLIR Cuda")
      .def(nb::init<>())
      .def("load", &py_cuda::load, "load mlir to cuda")
      .def("set_tensor", &py_cuda::set_tensor)
      .def("invoke", &py_cuda::invoke, nb::arg("dump_all")=false)
      .def("get_tensor", &py_cuda::get_tensor, "get one tensor data")
      .def("get_all_tensor", &py_cuda::get_all_tensor, "dump all tensor data")
      .def_ro("input_names", &py_cuda::input_names)
      .def_ro("output_names", &py_cuda::output_names);
#endif
  // clang-format on
