...
void py_cuda::cudaScatterNDOp(top::ScatterNDOp op) {
  ...
  std::vector<float> idx_h(updates_elems * index_depth);
  std::vector<float> upd_h(upd_num);
  CHECK_CUDA(cudaMemcpy(idx_h.data(), getCudaData(op.getIndices()),
                        idx_h.size() * sizeof(float), cudaMemcpyDeviceToHost));
...
void py_cuda::cudaScatterNDOp(tpu::ScatterNDOp op) {
  auto in_shape = module::getShape(op.getInputData());
  auto idx_shape = module::getShape(op.getIndices());
  int in_num = module::getNumElements(op.getInputData());
  int upd_num = module::getNumElements(op.getUpdates());
  ...
  if (stype.isF32()) {
    ...
    auto output_f32 = cuda_malloc(in_num * sizeof(float));
    auto input_f32 = cuda_malloc(in_num * sizeof(float));
    cuda::convertType(getCudaData(op.getInputData()), input_f32.get(), in_num,
                      getCudaType(op.getInputData()), cuda::DT_F32);
    CHECK_CUDA(cudaMemcpy(output_f32.get(), input_f32.get(),
                          in_num * sizeof(float), cudaMemcpyDeviceToDevice));
  }
  ...
  if (stype.isF32()) {
    ...
    auto updates_f32 = cuda_malloc(upd_num * sizeof(float));
    cuda::convertType(getCudaData(op.getUpdates()), updates_f32.get(), upd_num,
                      getCudaType(op.getUpdates()), cuda::DT_F32);
    CHECK_CUDA(cudaMemcpy(upd_h.data(), updates_f32.get(),
                          upd_num * sizeof(float), cudaMemcpyDeviceToHost));
  }
  ...
  if (stype.isF32()) {
    ...
    auto output_f32 = cuda_malloc(in_num * sizeof(float));
    auto updates_f32 = cuda_malloc(upd_num * sizeof(float));
    cuda::convertType(getCudaData(op.getInputData()), output_f32.get(), in_num,
                      getCudaType(op.getInputData()), cuda::DT_F32);
    cuda::convertType(getCudaData(op.getUpdates()), updates_f32.get(), upd_num,
                      getCudaType(op.getUpdates()), cuda::DT_F32);
    cuda::scatterND(output_f32.get(), updates_f32.get(), fi_d.get(), upd_num, add);
    cuda::convertType(output_f32.get(), getCudaData(op.getOutput()), in_num,
