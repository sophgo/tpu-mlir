...
__global__ void g_adaptiveAvgPool2D(float *input, float *output,
  ...
  int nc      = blockIdx.z;                      // N x C flattened
...
__global__ void g_gatherND(float *input, float *indices, float *output,
  ...
  // Decode out_idx into multi-dim coordinates of indices (excluding the last coord dim)
  // Dims 0 to batch_dims-1 of indices are batch, batch_dims to second-to-last are middle
  // out_idx iterates over all flat combinations of batch + middles
  int batch_flat = out_idx;
  // Decode the batch part coordinates
  int in_base = 0;
  ...
  // Read indices: flat index out_idx corresponds to indices[out_idx, :]
...
// 1 thread handles all inner_dim elements of 1 group
__global__ void g_groupNorm(float *input, float *output,
  ...
  // affine: weight * y + bias, indexed by channel
...
// Additionally output mean and rstd
...
// all spatial elements of each (n, c)
...
// c, update cell state and hidden state
...
__global__ void g_pad5D(void *input, void *output, int n, int c,int d,int h, int w,
  ...
  // Decode back into 5D coordinates (outer to inner)
  int idx_w = idx % ow;
  int idx_h = (idx / ow)         % oh;
  ...
  // Linear index of the output position
  ...
  if (inside)
    ...
    {
      d_setValue(output, out_idx, tbytes, 0.0f);
    }
}
...
__global__ void g_requantInt16Perchannel_3d(int32_t *input, void *output,
  ...
  if (idx < total_elements) {
    // Compute channel index idx_c
    // For 4D (d==1): idx % (c*h*w) / (h*w) -> keeps the original logic
    // For 5D: compute the channel after skipping the D*H*W part
...
__global__ void g_requantInt8Perchannel_3d(int32_t *input, void *output,
  ...
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int total_elements = n * c * d * h * w;     // supports 5D (NCDHW)

  if (idx < total_elements) {

    // ====================== Compute channel index idx_c ======================
    // When d==1: behavior is fully consistent with the original
    // When d>1:  correctly handle the depth dimension of Conv3D
    int elements_per_channel = d * h * w;
    int idx_c = (idx % (c * elements_per_channel)) / elements_per_channel;
