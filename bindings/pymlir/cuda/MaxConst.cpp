//===----------------------------------------------------------------------===//
//
// Copyright (C) 2022 Sophgo Technologies Inc.  All rights reserved.
//
// TPU-MLIR is licensed under the 2-Clause BSD License except for the
// third-party components.
//
//===----------------------------------------------------------------------===//

#include "../pycuda.h"
#include "cuda_helper.h"

void py_cuda::cudaMaxConstOp(top::MaxConstOp op) {
  auto input = getCudaData(op.getInput());
  auto output = getCudaData(op.getOutput());
  float const_val = op.getConstVal().convertToDouble();
  int num = module::getNumElements(op.getOutput());
  cuda::bmMaxConst(input, output, const_val, num);
}

void py_cuda::cudaMaxConstOp(tpu::MaxConstOp op) {
  auto input = getCudaData(op.getInput());
  auto output = getCudaData(op.getOutput());
  float const_val = op.getConstVal().convertToDouble();
  int num = module::getNumElements(op.getOutput());

  auto stype = module::getStorageType(op.getOutput());
  if (stype.isF32()) {
    cuda::bmMaxConst(input, output, const_val, num);
  } else {
    auto input_f32 = newCudaData(input, num, getCudaType(op.getInput()), cuda::DT_F32);
    auto output_f32 = cuda_malloc(num * sizeof(float));
    cuda::bmMaxConst(input_f32.get(), output_f32.get(), const_val, num);
    cuda::convertType(output_f32.get(), output, num, cuda::DT_F32, getCudaType(op.getOutput()));
  }
}
