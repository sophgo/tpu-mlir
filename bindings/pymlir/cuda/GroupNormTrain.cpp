...
void py_cuda::cudaGroupNormTrainOp(top::GroupNormTrainOp op) {
  ...
  cuda::bmGroupNormTrain(getCudaData(op.getInput()), getCudaData(op.getOutput()),
                          getCudaData(op.getMean()), getCudaData(op.getRstd()),
                          w_ptr, b_ptr,
                          outer_dim, inner_dim, channel, channel_per_group, eps);
}

void py_cuda::cudaGroupNormTrainOp(tpu::GroupNormTrainOp op) {
  auto in_shape = module::getShape(op.getInput());
  int channel = in_shape[1];
  int num_groups = op.getNumGroups();
  int channel_per_group = channel / num_groups;
