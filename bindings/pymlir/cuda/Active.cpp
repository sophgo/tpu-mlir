...
void py_cuda::cudaActiveOp(tpu::ActiveOp op) {
  ...
  if (op.getMode() == tpu::ActiveMode::SIN) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSin(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSin(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32,
                        getCudaType(op.getOutput()));
      input_f32.reset();
  ...
  if (op.getMode() == tpu::ActiveMode::ERF) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmERF(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
  ...
  if (op.getMode() == tpu::ActiveMode::EXP) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmEXP(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmEXP(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
      input_f32.reset();
      output_f32.reset();
  ...
  if (op.getMode() == tpu::ActiveMode::MISH) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmMish(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmMish(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::SIGN) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSign(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSign(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::SINH) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSinh(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSinh(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::SOFT_PLUS) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSoftplus(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSoftplus(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::SQRT) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSqrt(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
  ...
  if (op.getMode() == tpu::ActiveMode::SOFT_SIGN) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSoftsign(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSoftsign(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::LN) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmLn(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmLn(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::TAN) {
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmTan(getCudaData(op.getInput()), getCudaData(op.getOutput()), num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmTan(input_f32.get(), output_f32.get(), num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  if (op.getMode() == tpu::ActiveMode::SWISH) {
    float beta = 1.0f;
    auto coeffs = op.getCoeffs();
    if (coeffs && !coeffs->empty())
      beta = coeffs->begin()->cast<mlir::FloatAttr>().getValueAsDouble();
    if (module::getStorageType(op.getInput()).isF32()) {
      cuda::bmSwish(getCudaData(op.getInput()), getCudaData(op.getOutput()),
                    beta, num_out);
    } else {
      auto input_f32 = newCudaData(op.getInput(), cuda::DT_F32);
      auto output_f32 = cuda_malloc(num_out * sizeof(float));
      cuda::bmSwish(input_f32.get(), output_f32.get(), beta, num_out);
      cuda::convertType(output_f32.get(), getCudaData(op.getOutput()),
                        num_out, cuda::DT_F32, getCudaType(op.getOutput()));
    }
    return;
  ...
  switch (op.getMode()) {
    ...
    case tpu::ActiveMode::ELU: {
      ...
      if (auto coeffs = op.getCoeffs()) {
        elu_alpha = coeffs->getValue()[0].cast<FloatAttr>().getValueAsDouble();
      }
      CHECK_CUDNN(cudnnSetActivationDescriptor(activation_desc,
...
void py_cuda::cudaEluOp(top::EluOp op) {
  ...
  cuda::bmELU(input, output, alpha, num);
}

void py_cuda::cudaErfOp(top::ErfOp op) {
  auto input = getCudaData(op.getInput());
  auto output = getCudaData(op.getOutput());
  auto num = module::getNumElements(op.getOutput());
  cuda::bmERF(input, output, num);
}

void py_cuda::cudaExpOp(top::ExpOp op) {
  auto input = getCudaData(op.getInput());
  auto output = getCudaData(op.getOutput());
  auto num = module::getNumElements(op.getOutput());
  cuda::bmEXP(input, output, num);
}
