...
void py_cuda::cudaSubConstOp(tpu::SubConstOp op) {
  ...
  if (module::isUniformQuantized(op.getInput())) {
    ...
    auto o_zp = o_qtype.getZeroPoint();
    ...
    bool out_signed = !module::getStorageType(op.getOutput()).isUnsignedInteger();
    cuda::subConst4DI8(getCudaData(op.getInput()), in_signed, static_cast<int>(const_v),
                  getCudaData(op.getOutput()), out_signed, op.getDoRelu(), is_reverse,
                  multi, shift, n0, c0, h0, w0, o_zp);
