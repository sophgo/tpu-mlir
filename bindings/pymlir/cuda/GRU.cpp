...
void py_cuda::cudaGRUOp(top::GRUOp op) {
  ...
  for (int d = 0; d < num_dir; d++) {
    ...
    float *b_iz = bias;
    float *b_ir = b_iz + hidden_size;
    float *b_ih = b_ir + hidden_size;
    float *b_hz = b_ih + hidden_size;
    float *b_hr = b_hz + hidden_size;
    float *b_hh = b_hr + hidden_size;
    ...
    for (int t = 0; t < seq_len; t++) {
      ...
      cuda::mmF32(x_t, W_z, x_gi, true, batch_size, input_size, hidden_size);
      cuda::mmF32(x_t, W_r, x_gr, true, batch_size, input_size, hidden_size);
      cuda::mmF32(x_t, W_h, x_gh, true, batch_size, input_size, hidden_size);

      cuda::mmF32(h_prev, R_z, h_gi, true, batch_size, hidden_size, hidden_size);
      cuda::mmF32(h_prev, R_r, h_gr, true, batch_size, hidden_size, hidden_size);
      cuda::mmF32(h_prev, R_h, h_gh, true, batch_size, hidden_size, hidden_size);

      if (bias_data != nullptr) {
        cuda::addAxis(x_gi, b_iz, x_gi, batch_size, hidden_size, 1, cuda::DT_F32);
        cuda::addAxis(x_gr, b_ir, x_gr, batch_size, hidden_size, 1, cuda::DT_F32);
        cuda::addAxis(x_gh, b_ih, x_gh, batch_size, hidden_size, 1, cuda::DT_F32);
        cuda::addAxis(h_gi, b_hz, h_gi, batch_size, hidden_size, 1, cuda::DT_F32);
        cuda::addAxis(h_gr, b_hr, h_gr, batch_size, hidden_size, 1, cuda::DT_F32);
        cuda::addAxis(h_gh, b_hh, h_gh, batch_size, hidden_size, 1, cuda::DT_F32);
      }
      ...
      if (has_Y && Y_data != nullptr) {
        ...
        if (batch_first) {
          ...
          y_offset = (d * seq_len + t_idx) * batch_size * hidden_size;
        }
