...
void py_cuda::cudaAddOp(tpu::AddOp op) {
  ...
  if (module::isUniformQuantized(op.getOutput())) {
    ...
    if (module::getStorageType(in0) != module::getStorageType(in1) ||
        module::getStorageType(in0) != module::getStorageType(out)) {
      UNREACHABLE_OP("Not supported", op);
    }
    if (module::getStorageType(in0).isF32()) {
      ...
      auto input1 = getCudaData(in1);
      auto output = getCudaData(out);
      cuda::add4DF32(input0, input1, output, op.getDoRelu(),
                     n0, c0, h0, w0,
                     n1, c1, h1, w1,
                     n2, c2, h2, w2);
      return;
      ...
      cuda::add4DF32(input0.get(), input1.get(), output.get(), op.getDoRelu(),
                     n0, c0, h0, w0,
                     n1, c1, h1, w1,
                     n2, c2, h2, w2);
