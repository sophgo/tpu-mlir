...
public:
  ...
  nb::ndarray<> get_tensor(std::string name);

  // Tip: not using copy in python, since independent mem
  nb::ndarray<> get_fp32_tensor(std::string name);
  ...
  nb::ndarray<> invoke_at(const std::string name);

  nb::ndarray<> backward_weight_at(
