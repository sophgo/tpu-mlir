...
#include <tpuDNN.h>
#include <vector>
#if defined(__tpu_6_0__) || defined(__tpul_6_0__) || defined(__tpul_8_1__) ||  \
    defined(__tpu_6_0_e__)
#include "bmlib_runtime.h"
#include "common_util.h"
#elif defined(__tpub_7_1__) || defined(__tpub_9_0__) || defined(__tpub_7_1_e__)
#include <tpu_fp16.h>
#else
#include <tpu_fp16.h>
#endif
...
#if defined(__tpul_6_0__)
#define MAX_TPU_CORE_NUM 2
#elif defined(__tpub_9_3__)
...
int DtypeBits(int dtype) {
  ...
  if (dtype == DT_INT8 || dtype == DT_UINT8)
    ...
    size = 4; // 4-bit types are packed, but for simplicity we use 1 byte per
              // element
#if defined(__tpub_9_0__) || defined(__tpub_9_3__)
  else if (dtype == DT_FP4)
    size = 4;
#endif
  return size;
}
...
int DtypeSize(int dtype) {
  ...
  if (dtype == DT_INT8 || dtype == DT_UINT8)
    ...
    assert(0 && "not support 4bit dtype\n");
  return size;
}
...
void convert_src_to_float(std::vector<float> &dst, void *data, size_t ele_size,
  ...
  for (size_t i = 0; i < ele_size; ++i) {
    if (dtype == DT_FP32)
      ...
      uint8_t val;
      if (i % 2 == 0) {
        val = reinterpret_cast<uint8_t *>(data)[i / 2] & 0x0F;
      } else {
        val = (reinterpret_cast<uint8_t *>(data)[i / 2] >> 4) & 0x0F;
      }
      if (dtype == DT_INT4) {
        int8_t int_val = static_cast<int8_t>(val);
        if (int_val > 7) {
          int_val -= 16;
        }
        dst[i] = static_cast<float>(int_val);
      } else {
...
void convert_float_to_dst(void *data, std::vector<float> &numbers,
  ...
  for (size_t i = 0; i < ele_size; ++i) {
    if (dtype == DT_FP32)
      ...
      value = std::max(static_cast<int8_t>(-8),
                       std::min(static_cast<int8_t>(7), value));
      int8_t &ori_val = reinterpret_cast<int8_t *>(data)[i / 2];
      if (i % 2) {
        value = value << 4;
        ori_val = (ori_val & 0x0F) | value;
      } else {
        ori_val = (ori_val & 0xF0) | value;
      }
    } else if (dtype == DT_UINT4) {
      uint8_t value = static_cast<uint8_t>(std::round(numbers[i]));
      value = std::max(static_cast<uint8_t>(0),
                       std::min(static_cast<uint8_t>(15), value));
      uint8_t &ori_val = reinterpret_cast<uint8_t *>(data)[i / 2];
      if (i % 2) {
        value = value << 4;
        ori_val = (ori_val & 0x0F) | value;
      } else {
...
void convert_data_type(void *dst_data, void *src_data, int ele_size,
  ...
  if (src_dtype == dst_dtype) {
    memcpy(dst_data, src_data, ele_size * DtypeBits(dst_dtype) / BYTE_SIZE);
  } else {
...
void load_file(std::string file_path, char *data, size_t data_size,
  ...
  size_t file_ele_size = file_size * BYTE_SIZE / DtypeBits(file_dtype);
...
void dump_data(std::string dir, std::string func_name, std::string pre,
               ...
               int dtype) {
  size_t element_size = data_size * BYTE_SIZE / DtypeBits(dtype);
      ...
      for (int h = 0; h < shape.h; ++h) {
        for (int w = 0; w < shape.w; ++w) {
          ...
          if (dtype == DT_FP32) {
            ...
            float rand_value = u(e);
            fp16 fp16_v = fp32_to_fp16(*(fp32 *)&rand_value);
            uint8_t fp4_v = fp16_to_fp4(fp16_v, ROUND_HALF_TO_EVEN);
            uint8_t &ori_val = reinterpret_cast<uint8_t *>(data)[index / 2];
            if (index % 2) {
              fp4_v = fp4_v << 4;
              ori_val = (ori_val & 0x0F) | fp4_v;
            } else {
              ori_val = (ori_val & 0xF0) | fp4_v;
            }
#endif
          } else if (dtype == DT_INT4 || dtype == DT_UINT4) {
...
#if defined(__tpu_6_0__) || defined(__tpul_6_0__) || defined(__tpul_8_1__) ||  \
    defined(__tpu_6_0_e__) || defined(__tpub_9_3__)
...
bm_handle_t handle;
#ifdef MAX_TPU_CORE_NUM
tpu_kernel_module_t tpu_module[MAX_TPU_CORE_NUM];
#else
...
static mem_t alloc_mem(dim4 &shape, int dtype) {
  ...
  size_t size = mem.ele_size * DtypeBits(dtype) / BYTE_SIZE;
  mem.data_size = size;
  mem.dtype = dtype;
  mem.shape = shape;
