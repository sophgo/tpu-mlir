...
__attribute__((constructor)) void tpu_kernel_register_##func() {   \
    ...
    __attribute__((weak)) int ap_debug(const char *format, ...);
    #endif
#endif
...
#define FP_MAX(dtype) \
    (dtype == DT_FP8E4M3 ? 0x7e : (dtype == DT_FP8E5M2? 0x7b : (dtype == DT_FP32 ? 0x7f7fffff : (dtype == DT_FP16 ? 0x7bff : 0x7f7f))))
...
void tpu_set_cdma_defered_barrier_msg_id(int);
...
void tpu_bdc_qt0_quant(
    local_addr_t     R_addr,
    local_addr_t     R1_addr,
    local_addr_t     A_addr,
    uint16_t         range_val,
    uint16_t         r_range_val,
    int              A_short_str,
    const dim4      *shape,
    data_type_t      dst_dtype,
    data_type_t      src_dtype);
...
void tpu_bdc_fp_dq2_mm2_nt(
    local_addr_t Y_addr,
    local_addr_t L_addr,
    local_addr_t R_addr,
    local_addr_t DQ2_addr,
    int L_row_num,
    int L_col_num,
    int R_col_num,
    int gsize,
    ...
    data_type_t L_dtype,
    data_type_t R_dtype,
    data_type_t DQ2_dtype, //8bit/16bit/32bit
    data_type_t Y_dtype,
    int L_fp8_type,
    int Y_fp8_type,
    int R_sign,
...
void tpu_bdc_fp_dq2_mm2_nt_requant(
    local_addr_t Y_addr,
    local_addr_t L_addr,
    local_addr_t R_addr,
    local_addr_t DQ2_addr,
    local_addr_t RQ_addr, //tensor's laddr or 32bit fp32 const val
    int L_row_num,
    int L_col_num,
    int R_col_num,
    int gsize,
...
void tpu_bdc_fp_dq2_mm2_tt_requant(
    local_addr_t Y_addr,
    local_addr_t L_addr,
    local_addr_t R_addr,
    local_addr_t DQ2_addr,
...
/*
  ...
  index_shape is (1, dst_C, 1, 1). Note:if index in lmem, it's actually stored as (1, 1, 1, dst_C)
  output_shape is (1, dst_C, H, W).
  H and W dim is compact store.
  C dim can support stride.
  If not stride_enable, gmem/smem compact store, lmem align store.
*/
void tpu_gdma_c_gather_S2L(
    local_addr_t   output_addr,
...
/*
  gdma_c_scatter
  input_shape is (1, src_C, H, W).
  index_shape is (1, src_C, 1, 1). Note:if index in lmem, it's actually stored as (1, 1, 1, src_C)
  output_shape is (1, dst_C, H, W).
  H and W dim is compact store.
  C dim can support stride.
  If not stride_enable, gmem/smem compact store, lmem align store.
  If do inplace_add, dst tensor must be in gmem!
*/
void tpu_gdma_c_scatter_S2L(
    local_addr_t   output_addr,
    system_addr_t  input_addr,
    addr_t         index_addr,
    bool           index_is_local,
...
void tpu_vsdma_cpy_reduce_S2L2(
    system_addr_t dst_addr,
    system_addr_t  src_addr,
    const dim4 *shape,
    const dim4 *dst_stride,
...
void tpu_bdc_fp_conv2d_for_deconv2d_without_rotate(
    local_addr_t      output_addr,
    local_addr_t      input_addr,
    local_addr_t      weight_addr,
    local_addr_t      bias_addr,
...
void tpu_bdc_fp_conv2d_with_ins(
    local_addr_t      output_addr,
    local_addr_t      input_addr,
    local_addr_t      weight_addr,
    local_addr_t      bias_addr,
...
void tpu_bdc_fp_hw_reduce_mean(
      local_addr_t output_addr,
      local_addr_t input_addr,
...
void tpu_bdc_int8_asym_quant_conv2d_for_deconv2d_without_rotate(
    local_addr_t      output_addr,
    local_addr_t      input_addr,
    local_addr_t      weight_addr,
    scalar_t          kzp_val,
...
void tpu_debug_cmd(u64 *p_cmd, int length, int command_type);
...
void tpu_bdc_fp_exp_for_neg_arg(
    local_addr_t  dst_addr,
    local_addr_t  src_addr,
    local_addr_t  work0_addr,
    local_addr_t  work1_addr,
...
void tpu_bdc_arithmetic_sequence_bcast_with_dtype(
    local_addr_t  dst_addr,
    int           npu_num,
    int           start,
    int           step,
...
void tpu_bdc_arithmetic_sequence_distribute_with_dtype(
    local_addr_t  dst_addr,
    int           start,
    int           step,
    int           num,
...
void tpu_bdc_arithmetic_sequence_distribute_aligned_with_dtype(
    local_addr_t  dst_addr,
    int           start,
    int           step,
    int           num,
...
void tpu_bdc_arithmetic_sequence_general_with_dtype(
    local_addr_t  dst_addr,
...
void tpu_bdc_load_fp_arctan_coeff(local_addr_t coeff_addr, data_type_t dtype);
...
void tpu_bdc_load_div_lut(local_addr_t table_addr);
...
void tpu_bdc_bf16_sigmoid_fast(
    local_addr_t  dst_addr,
    local_addr_t  src_addr,
    local_addr_t  work0_addr,
    local_addr_t  work1_addr,
...
// 0.5 * x * (1.0 + std::erf(x / std::sqrt(2.0)))
...
void tpu_bdc_fp32_gelu_fast(
    ...
    local_addr_t  work1_addr,
    local_addr_t  coeff_addr,
    local_addr_t  table_addr,  // NO_USE
    const dim4   *shape);
...
void tpu_sync_core();
void tpu_sync_all();
void tpu_sync_all_with_msg_id(int);
void tpu_sync_all_bdc();
void tpu_sync_all_gdma();
void tpu_sync_all_sdma();
void tpu_sync_all_vsdma(int msg_cnt);
...
void tpu_cdma_port_set_base_msg_id(int port, int msg_id);
...
void tpu_set_mem_guard(int id, system_addr_t addr, uint64_t size, mem_guard_attr_t attr);
...
// NaN check functions
int _tpu_mem_check_ddr_nan(global_addr_t buffer_addr, int num_elements, data_type_t dtype);
int _tpu_mem_check_local_nan(local_addr_t buffer_addr, dim4* shape, dim4* stride, data_type_t dtype);
#define tpu_mem_check_local_nan(buffer, ...) \
    do{ \
        TPUKERNEL_DBG("%d line check_local_nan: %s\n", __LINE__, #buffer); \
        int has_nan = _tpu_mem_check_local_nan(buffer, __VA_ARGS__); \
        (void)has_nan; \
        TPUKERNEL_DBG("_tpu_mem_check_local_nan: %s has %d NaN values\n", #buffer, has_nan); \
    } while (0)
#define tpu_mem_check_ddr_nan(buffer, ...) \
    do{ \
        TPUKERNEL_DBG("%d line check_ddr_nan: %s\n", __LINE__, #buffer); \
        int has_nan = _tpu_mem_check_ddr_nan(buffer, __VA_ARGS__); \
        (void)has_nan; \
        TPUKERNEL_DBG("_tpu_mem_check_ddr_nan: %s has %d NaN values\n", #buffer, has_nan); \
    } while (0)
