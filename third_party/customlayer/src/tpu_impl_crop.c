    ...
    data_type_t dtype)
{
    ...
    TPUKERNEL_ASSERT(0 <= hoffset && hoffset + hnew <= shape[0]);
    TPUKERNEL_ASSERT(0 <= woffset && woffset + wnew <= shape[1]);
    int data_size = tpu_data_type_size(dtype);
    dim4 old_shape = {.n=1, .c=1, .h=shape[0], .w=shape[1]};
    dim4 old_stride = {0};
    tpu_continuous_stride(&old_stride, &old_shape);
    dim4 new_shape = {.n=1, .c=1, .h=hnew, .w=wnew};
    tpu_gdma_cpy_S2S(
        output_global_addr,
