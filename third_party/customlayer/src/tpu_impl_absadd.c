...
void tpu_impl_absadd_global(global_addr_t input_global_addr,
  ...
  while (cur_idx[2] < length) {
    ...
    if (draning_idx < 1) {
      ...
      cur_rows[0] = MAX(1, cur_len / cur_cols[0]); // This calculates the number of rows for the current slice when assembled into a matrix.
The formed matrix shape is [cur_cols[0], cur_rows[0]].
    }
