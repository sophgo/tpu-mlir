...
static void upsample_split(
    ...
    uint32_t output_w, cvk_fmt_t fmt, int eu_align, uint32_t &c_step, uint32_t &h_step) {
  ...
  for (; h_step > 0; --h_step) {
    ...
    for (c_step = input_c; c_step >= (uint32_t)CVI_NPU_NUM ; --c_step) { //at least c = CVI_NPU_NUM
      ...
      uint32_t tiled_working_size =
          ctx.lmem_tensor_to_size(tiled_working_shape, fmt, eu_align);
  ...
  LLVM_DEBUG(llvm::dbgs() << "  upsample_split: c_step " << c_step << " ,   h_step " << h_step << "\n");
...
static void upsample_assign_lmem_layout(
    ...
    uint32_t input_h, uint32_t input_w, uint32_t output_c, uint32_t output_h,
    uint32_t output_w, cvk_fmt_t fmt, int eu_align, cvk_tl_t &tl_ifmap, cvk_tl_t &tl_working,
    cvk_tl_t &tl_ofmap) {

  uint32_t tl_offset = 0; // begin of local memory
  ctx.lmem_init_tensor(&tl_ifmap, {1, tiling_c, tiling_h, input_w}, fmt,
                       eu_align);
  tl_ifmap.start_address = tl_offset;
  tl_offset += ctx.lmem_tensor_to_size(tl_ifmap.shape, tl_ifmap.fmt,
                                       tl_ifmap.eu_align);

  ctx.lmem_init_tensor(&tl_working, {1, tiling_c, tiling_h, output_w}, fmt,
                       eu_align);
  tl_working.start_address = tl_offset;
  tl_offset += ctx.lmem_tensor_to_size(tl_working.shape, tl_working.fmt,
                                       tl_working.eu_align);

  uint32_t tiling_oh = tiling_h * (output_h/ input_h);
  ctx.lmem_init_tensor(&tl_ofmap, {1, tiling_c, tiling_oh, output_w}, fmt,
                       eu_align);
  tl_ofmap.start_address = tl_offset;
}

static void upsample_tensor_load(
    const CviBackendContext &ctx, uint32_t layer_id, gaddr_t ga_ifmap,
    cvk_tg_stride_t &ifmap_gstride, int n_pos, int c_pos, int h_pos, cvk_tl_t *tl_ifmap) {
...
*/
static void upsample_tiu_copy(
    const CviBackendContext &ctx, uint32_t layer_id, uint32_t h_factor,
    uint32_t w_factor, cvk_tl_t *tl_ifmap, cvk_tl_t *tl_working, cvk_tl_t *tl_ofmap) {

  LLVM_DEBUG(llvm::dbgs()
      << "  upsample_tiu_copy, layer_id " << layer_id
      << ", h_factor " << h_factor << ", w_factor " << w_factor << "\n"
      << "    ifmap addr " << tl_ifmap->start_address
      << ", shape(" << tl_ifmap->shape.n
      << ", " << tl_ifmap->shape.c << ", " << tl_ifmap->shape.h
      << ", " << tl_ifmap->shape.w
      << "), stride(" << tl_ifmap->stride.n
      << ", " << tl_ifmap->stride.c << ", " << tl_ifmap->stride.h
      << ", " << tl_ifmap->stride.w << ")\n"
      << "    working addr " << tl_working->start_address
      << ", shape(" << tl_working->shape.n
      << ", " << tl_working->shape.c << ", " << tl_working->shape.h
      << ", " << tl_working->shape.w
      << "), stride(" << tl_working->stride.n
      << ", " << tl_working->stride.c << ", " << tl_working->stride.h
      << ", " << tl_working->stride.w << ")\n"
      << "    ofmap address " << tl_ofmap->start_address
      << ", shape(" << tl_ofmap->shape.n
      << ", " << tl_ofmap->shape.c << ", " << tl_ofmap->shape.h
      << ", " << tl_ofmap->shape.w
      << "), stride(" << tl_ofmap->stride.n
      << ", " << tl_ofmap->stride.c << ", " << tl_ofmap->stride.h
      << ", " << tl_ofmap->stride.w << ")\n");
  ...
  cvk_tl_t tl_ifmap_fake = {0};
     ...
     tl_ifmap_fake.shape = {w_factor, tl_ifmap->shape.c, tl_ifmap->shape.h, tl_ifmap->shape.w};
  ...
  cvk_tl_stride_t tl_working_fake_stride = {
     ...
     tl_working->stride.h, tl_working->stride.w * w_factor};
  cvk_tl_t tl_working_fake = {0};
     tl_working_fake.start_address = tl_working->start_address;
     tl_working_fake.fmt = tl_working->fmt;
     tl_working_fake.shape = {w_factor, tl_ifmap->shape.c, tl_ifmap->shape.h, tl_ifmap->shape.w};
  ...
  cvk_tl_t tl_working_fake2 = {0};
     ...
     tl_working_fake2.shape = {h_factor, tl_ofmap->shape.c, tl_ifmap->shape.h, tl_ofmap->shape.w};
     tl_working_fake2.stride = tl_working_fake2_stride;
     tl_working_fake2.eu_align = tl_working->eu_align;

  cvk_tl_stride_t tl_ofmap_fake_stride = {tl_ofmap->stride.h, tl_ofmap->stride.c, tl_ofmap->stride.h * h_factor, tl_ofmap->stride.w};
  cvk_tl_t tl_ofmap_fake = {0};
     tl_ofmap_fake.start_address = tl_ofmap->start_address;
     tl_ofmap_fake.fmt = tl_ofmap->fmt;
     tl_ofmap_fake.shape =  {h_factor, tl_ofmap->shape.c, tl_ifmap->shape.h, tl_ofmap->shape.w};
...
static void upsample_tensor_store(
    const CviBackendContext &ctx, uint32_t layer_id, gaddr_t ga_ofmap,
    cvk_tl_t *tl_ofmap, cvk_tg_stride_t &ofmap_gstride, int n_pos, int c_pos, int oh_pos) {
...
void cvi_backend_tg_upsample_kernel(
  ...
  LLVM_DEBUG(llvm::dbgs() << llvm::format(
      ...
      ga_ifmap, ga_ofmap, input_n, input_c, input_h, input_w, h_factor,
      w_factor));
  ...
  uint32_t c_step = 0;
  uint32_t h_step = 0;
  upsample_split(ctx, input_n, input_c, input_h, input_w, output_c, output_h,
  ...
  for (uint32_t n_pos = 0; n_pos < input_n; n_pos += n_step) {
    for (uint32_t c_pos = 0; c_pos < input_c; c_pos += c_step) {
      ...
      for (uint32_t h_pos = 0; h_pos < input_h; h_pos += h_step) {
        ...
        upsample_assign_lmem_layout(ctx, tiling_c, tiling_h, input_c, input_h, input_w, output_c,
                                    output_h, output_w, fmt, eu_align, tl_ifmap, tl_working,
                                    tl_ofmap);
        // 2. tensor load
        upsample_tensor_load(ctx, layer_id, ga_ifmap, ifmap_gstride, n_pos, c_pos, h_pos,
                            &tl_ifmap);

        // 3. tiu copy
        upsample_tiu_copy(ctx, layer_id, h_factor, w_factor, &tl_ifmap, &tl_working, &tl_ofmap);

        // 4. tensor store
        uint32_t oh_pos = h_pos * h_factor;
        upsample_tensor_store(ctx, layer_id, ga_ofmap, &tl_ofmap, ofmap_gstride,
                              n_pos, c_pos, oh_pos);
      }
    } //for (uint32_t c_pos = 0; c_pos < input_c; c_pos += c_step) {
  } // for (uint32_t n_pos = 0; n_pos < input_n; n_pos += n_step)
