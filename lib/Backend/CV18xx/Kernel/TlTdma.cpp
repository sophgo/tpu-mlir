...
void cvi_backend_tl_load(
  ...
  tl_ifmap.fmt = fmt;
  tl_ifmap.shape = ctx.tl_shape_t4(n, ic, ih, iw);
...
void cvi_backend_tl_store(const CviBackendContext &ctx, uint32_t layer_id,
  ...
  tl_ofmap.fmt = fmt;
  tl_ofmap.shape = ctx.tl_shape_t4(n, oc, oh, ow);
                               ...
                               int n, int c, int h, int w)
{
  ...
  tl_src.fmt = fmt;
  tl_src.shape = ctx.tl_shape_t4(n, c, h, (sizeof(uint32_t)/sizeof(uint16_t)) * w);
  tl_src.stride = {stride.n, (uint32_t)EU_NUM, stride.h, stride.w};
