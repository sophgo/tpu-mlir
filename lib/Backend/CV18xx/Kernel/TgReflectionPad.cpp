...
void cvi_backend_tg_reflectionpad_kernel(const CviBackendContext &ctx,
  ...
  auto src_shape = ctx.tg_shape_t4(1, outer_size, ih, iw);
  auto dst_shape = ctx.tg_shape_t4(1, outer_size, oh, ow);
  ...
  // top
  src_shape = ctx.tg_shape_t4(1, outer_size, 1, iw);
  ...
  do_reflection(ctx, layer_id, src_gaddr, ga_output, ga_left, out_gstride,
                out_gstride, outer_size, pads[0], fmt);
  // right
  src_gaddr = ga_output + (pads[0] + iw - pads[1] - 1) * fmt_size;
  dst_gaddr = ga_output + (ow - pads[1]) * fmt_size;
  do_reflection(ctx, layer_id, src_gaddr, dst_gaddr, ga_right, out_gstride,
