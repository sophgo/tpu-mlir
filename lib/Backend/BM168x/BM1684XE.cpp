...
#include "tpu_mlir/Backend/BM168x/BM1684XE.h"
...
uint32_t BM1684XE::get_bdc_len(int bdc_num, int group_id) {
  if (bdc_num == 0) {
    return 0;
  }
...
uint32_t BM1684XE::get_gdma_len(int gdma_num, int group_id) {
  if (gdma_num == 0) {
    return 0;
  }
  auto group_num = dl_backend_api_get_tpu_inst_group_number();
...
void BM1684XE::before_codegen() {
  dl_allow_store_cmd();
  BM168x::before_codegen();
  dl_backend_api_clear_tpu_inst_data();
}

void BM1684XE::after_codegen(int64_t flops) {
  BM168x::after_codegen(flops);
  dl_store_cmd_end();
  dl_forbid_store_cmd();
}

void BM1684XE::load_custom_functions() {
