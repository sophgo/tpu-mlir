//===----------------------------------------------------------------------===//
//
// Copyright (C) 2026 Sophgo Technologies Inc.  All rights reserved.
//
// TPU-MLIR is licensed under the 2-Clause BSD License except for the
// third-party components.
//
//===----------------------------------------------------------------------===//

#include "fattention_prefill.c"
#include "fattention_v1.c"
#include "fattention_v2.c"
#include "ppl_dyn_fw.h"

extern int tpu_core_num();

// global
void dynamic_glb_flash_attention_layer_ctrl(void *ctx, void *param,
                                            global_tensor_spec_t *input_spec,
                                            global_tensor_spec_t *output_spec) {
  output_spec->dtype = input_spec->dtype;
  output_spec->shape[0] = input_spec->shape[0];
  output_spec->shape[1] = input_spec->shape[1];

  if (input_spec->dtype == FW_DTYPE_FP16) {
    tpu_kernel_api_fattention_v1_f16_t *_param =
        (tpu_kernel_api_fattention_v1_f16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = (_param->has_mask != 0) ? input_spec[3].addr : 0ULL;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_v1_f16_entry(_param);
  } else if (input_spec->dtype == FW_DTYPE_BFP16) {
    tpu_kernel_api_fattention_v1_bf16_t *_param =
        (tpu_kernel_api_fattention_v1_bf16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = (_param->has_mask != 0) ? input_spec[3].addr : 0ULL;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_v1_bf16_entry(_param);
  }
}

void dynamic_glb_flash_attention_heigh_prec_layer_ctrl(
    void *ctx, void *param, global_tensor_spec_t *input_spec,
    global_tensor_spec_t *output_spec) {
  output_spec->dtype = input_spec->dtype;
  output_spec->shape[0] = input_spec->shape[0];
  output_spec->shape[1] = input_spec->shape[1];
  if (input_spec->dtype == FW_DTYPE_FP16) {
    tpu_kernel_api_fattention_v2_f16_t *_param =
        (tpu_kernel_api_fattention_v2_f16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = (_param->has_mask != 0) ? input_spec[3].addr : 0ULL;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_v2_f16_entry(_param);
  } else if (input_spec->dtype == FW_DTYPE_BFP16) {
    tpu_kernel_api_fattention_v2_bf16_t *_param =
        (tpu_kernel_api_fattention_v2_bf16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = (_param->has_mask != 0) ? input_spec[3].addr : 0ULL;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_v2_bf16_entry(_param);
  }
}

void dynamic_glb_fattention_prefill_layer_ctrl(
    void *ctx, void *param, global_tensor_spec_t *input_spec,
    global_tensor_spec_t *output_spec) {
  output_spec->dtype = input_spec->dtype;
  output_spec->shape[0] = input_spec->shape[0];
  output_spec->shape[1] = input_spec->shape[1];
  if (input_spec->dtype == FW_DTYPE_FP16) {
    tpu_kernel_api_fattention_prefill_f16_t *_param =
        (tpu_kernel_api_fattention_prefill_f16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = input_spec[3].addr;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_prefill_f16_entry(_param);
  } else if (input_spec->dtype == FW_DTYPE_BFP16) {
    tpu_kernel_api_fattention_prefill_bf16_t *_param =
        (tpu_kernel_api_fattention_prefill_bf16_t *)param;
    if (_param->keep_dim) {
      output_spec->dims = 4;
      output_spec->shape[2] = input_spec->shape[2];
      output_spec->shape[3] = input_spec->shape[3];
    } else {
      output_spec->dims = 3;
      output_spec->shape[2] = input_spec->shape[2] * input_spec->shape[3];
    }
    _param->ptr_out = output_spec->addr;
    _param->ptr_q = input_spec[0].addr;
    _param->ptr_k = input_spec[1].addr;
    _param->ptr_v = input_spec[2].addr;
    _param->ptr_mask = input_spec[3].addr;
    _param->b = input_spec->shape[0];
    _param->qm = input_spec->shape[1];
    _param->kvm = (input_spec + 1)->shape[1];
    if (_param->g_core_num > tpu_core_num()) {
      _param->g_core_num = tpu_core_num();
    }
    fattention_prefill_bf16_entry(_param);
  }
}

REGISTER_PPL_DYN_OP(PPL_FW_FLASH_ATTENTION,
                    dynamic_glb_flash_attention_layer_ctrl, 0);
REGISTER_PPL_DYN_OP(PPL_FW_FLASH_ATTENTION_HEIGH_PRECISION,
                    dynamic_glb_flash_attention_heigh_prec_layer_ctrl, 0);
REGISTER_PPL_DYN_OP(PPL_FW_FATTENTION_PREFILL,
                    dynamic_glb_fattention_prefill_layer_ctrl, 0);
