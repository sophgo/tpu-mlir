...
extern "C" {
#endif
extern int add_tiling(gaddr_t ptr_dst, gaddr_t ptr_src, float rhs, int N, int C, int H,
...
void api_add_const_fp_global(void *param, size_t param_size, void *input_spec,
  ...
  add_tiling(out_spec->addr, in_spec->addr, rhs, in_spec->shape[0], in_spec->shape[1], in_spec->shape[2],
             in_spec->shape[3], do_relu, in_spec->dtype);
}
