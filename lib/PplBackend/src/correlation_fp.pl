...
template <typename T>
void correlation(T *ptr_dst, T *ptr_lsrc, T *ptr_rsrc, const int N, const int C,
                 const int H, const int W, const int cut_nums) {
  ...
  for (int n_idx = 0; n_idx < N; n_idx++) {
    ...
    for (int h_idx = 0; h_idx < H; h_idx += block_h) {
      ...
      for (int slice_idx = 0; slice_idx < cut_nums; slice_idx += 1) {
        ppl::enable_pipeline();
        // left_feature(1,32,24,320) ->
        // left_feature(offset={0,0,0,slice_idx})(1,32,24,319) slice
        // right_feature(1,32,24,320) ->
        // right_feature(offset={0,0,0,0})(1,32,24,319) slice
        ...
        tiu::fmul(mul_tensor.sub_view(slice_shape, offsetl),
                  in_l_v.sub_view(slice_shape, offsetl),
                  in_r_v.sub_view(slice_shape, offsetr));
        // mul(1,32,24,319) -> reduce(dim=2)(1,32,1,319)
        tiu::fpool_avg(reduce_tensor, mul_tensor, &kernel, &padding, &stride,
                       &dilation, scale);
        // reduce(1,32,1,319)                                      ->
        // reduce(1,32,1,320)
        ...
        dma::store(out_view_gtensor.sub_view(reduce_real_shape, offset_gout),
                   reduce_tensor);
      }
    }
  }
...
__KERNEL__ void correlation_f32(float *ptr_dst, float *ptr_lsrc,
...
__KERNEL__ void correlation_fp16(fp16 *ptr_dst, fp16 *ptr_lsrc, fp16 *ptr_rsrc,
                                 ...
                                 const int W, const int cut_nums) {
  correlation(ptr_dst, ptr_lsrc, ptr_rsrc, N, C, H, W, cut_nums);
}
