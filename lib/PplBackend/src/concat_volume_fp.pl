 ...
 *
 * Build concatenated cost volume for stereo matching. Concatenate left and
 * right feature maps after shifting by disparity.
 *
 * Input:
...
template <typename T>
void concat_volume(T *ptr_dst, T *ptr_lsrc, T *ptr_rsrc, const int N,
                   const int C, const int H, const int W, const int max_disp) {
  // Input shape: (N, C, H, W)
  ...
  // Memory layout: [n][c][d][h][w] -> linear index = n*(2C*D*H*W) + c*(D*H*W) +
  // d*(H*W) + h*W + w Equivalent to 4D [n][c*D+d][h][w] -> linear index =
  // n*(2C*D*H*W) + (c*D+d)*(H*W) + h*W + w
...
__KERNEL__ void concat_volume_f32(float *ptr_dst, float *ptr_lsrc,
                                  float *ptr_rsrc, const int N, const int C,
                                  const int H, const int W,
                                  const int max_disp) {
  concat_volume(ptr_dst, ptr_lsrc, ptr_rsrc, N, C, H, W, max_disp);
}

__KERNEL__ void concat_volume_fp16(fp16 *ptr_dst, fp16 *ptr_lsrc,
                                   fp16 *ptr_rsrc, const int N, const int C,
                                   const int H, const int W,
                                   const int max_disp) {
  concat_volume(ptr_dst, ptr_lsrc, ptr_rsrc, N, C, H, W, max_disp);
}

__KERNEL__ void concat_volume_bf16(bf16 *ptr_dst, bf16 *ptr_lsrc,
                                   bf16 *ptr_rsrc, const int N, const int C,
                                   const int H, const int W,
                                   const int max_disp) {
  concat_volume(ptr_dst, ptr_lsrc, ptr_rsrc, N, C, H, W, max_disp);
}

__KERNEL__ void concat_volume_int8(int8 *ptr_dst, int8 *ptr_lsrc,
                                   int8 *ptr_rsrc, const int N, const int C,
                                   const int H, const int W,
                                   const int max_disp) {
  concat_volume(ptr_dst, ptr_lsrc, ptr_rsrc, N, C, H, W, max_disp);
}
