...
void flash_attention_fp16_v1(fp16 *ptr_out, fp16 *ptr_q, fp16 *ptr_k,
  ...
  auto q_global_tensor = gtensor<fp16>(qo_global_shape, GLOBAL, ptr_q);
  auto out_global_tensor = gtensor<fp16>(qo_global_shape, GLOBAL, ptr_out);

  dim4 kv_global_shape = {b, kvm, kv_head, cur_d};
  auto k_global_tensor = gtensor<fp16>(kv_global_shape, GLOBAL, ptr_k);
  auto v_global_tensor = gtensor<fp16>(kv_global_shape, GLOBAL, ptr_v);

  dim4 mask_global_shape = {b, qm, 1, kvm};
  auto mask_global_tensor = gtensor<fp16>(mask_global_shape, GLOBAL, ptr_mask);
  ...
  for (int _b = 0; _b < b; _b += 1) {
    ...
    for (int _h = q_head_start; _h < q_head_end; _h += block_h_iter) {
      ...
      for (int _m = 0; _m < qm; _m += block_m) {
        ...
        auto qi_tensor = make_tensor<fp16>(q_shape, qi_real_local_shape);
        dma::load_transpose_nc(
            qi_tensor, q_sub_global.sub_view(qi_real_global_shape, qi_offset));
        auto qi_tensor_scale = make_tensor<fp16>(q_shape, qi_real_local_shape);
        ...
        auto mi_sub_tensor = make_tensor<fp16>(mi_shape, mi_real_shape);
        auto li_sub_tensor = make_tensor<fp16>(li_shape, li_real_shape);
        auto acc_sub_tensor = make_tensor<fp16>(acc_shape, acc_real_shape);
        ...
        for (int _k = 0; _k < kvm; _k += block_k_iter) {
          ...
          auto ki_tensor = make_tensor<fp16>(kv_shape, kvi_real_local_shape);
          auto vi_tensor = make_tensor<fp16>(kv_shape, kvi_real_local_shape);
          dma::load_transpose_nc(
              ki_tensor,
          ...
          auto mask_tensor = make_tensor<fp16>(mask_shape, mask_real_shape);
          if (has_mask) {
          ...
          auto qk_sub_tensor = make_tensor<fp16>(qk_shape, qk_real_shape);
          ...
          auto max_out = make_tensor<fp16>(mi_shape, mi_real_shape);
          auto mi_new_tensor = make_tensor<fp16>(mi_shape, mi_real_shape);
          quick_pooling(max_out, qk_sub_tensor, &qk_shape, &qk_real_shape,
                        -15000, 0);

          tiu::fmax(mi_new_tensor, mi_sub_tensor, max_out);

          auto alpha = make_tensor<fp16>(mi_shape, mi_real_shape);
          auto sub_out = make_tensor<fp16>(mi_shape, mi_real_shape);
          auto li_tmp_tensor = make_tensor<fp16>(li_shape, li_real_shape);
          ...
          auto sub_out1 = make_tensor<fp16>(qk_shape, qk_real_shape);
          // broadcast sub (w)
          tiu::fsub(sub_out1, qk_sub_tensor, mi_new_tensor);

          auto p_T = make_tensor<fp16>(qk_shape, qk_real_shape);
          auto sum = make_tensor<fp16>(li_shape, li_real_shape);
          ...
          tensor<fp16> pv_tensor = make_tensor<fp16>(acc_shape, acc_real_shape);
        ...
        auto div_li_tensor_T = make_tensor<fp16>(li_shape, li_real_shape);
        auto qkvo_tensor = make_tensor<fp16>(acc_shape, acc_real_shape);
