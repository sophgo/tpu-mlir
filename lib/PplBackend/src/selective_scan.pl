...
void selective_scan(T *ptr_res, T *ptr_C, T *ptr_deltaA, T *ptr_deltaB_u,
  ...
  for (int index_w = 0; index_w < Batch; index_w += block_batch) {
    ...
    auto block_deltaA_up =
        make_tensor<T>(shape_delta_block, real_shape_delta_block);
    auto block_deltaB_u_up =
        make_tensor<T>(shape_delta_block, real_shape_delta_block);
    ...
    auto block_deltaA_down =
        make_tensor<T>(shape_delta_block, real_shape_delta_block);
    auto block_deltaB_u_down =
        make_tensor<T>(shape_delta_block, real_shape_delta_block);
    ...
    for (int index_c = 0; index_c < KC_dim; index_c += block_c) {
      ...
      dma::load(D_up_slice, D_gtensor.sub_view(shape_D_slice, D_up_offset));
      dma::load(D_down_slice, D_gtensor.sub_view(shape_D_slice, D_down_offset));
      ...
      for (int index_l = 0; index_l < L; index_l += block_l) {
        ...
        dim4 delta_down_offset = {0, index_c + KC_dim, L_block_offset_down,
                                  index_w};

        dim4 c_up_offset = {index_l, index_c, 0, index_w};
        dim4 c_down_offset = {L_block_offset_down, index_c + KC_dim, 0,
                              index_w};
        ...
        dma::load(block_deltaB_u_down,
                  deltaB_u_gtensor.sub_view(real_shape_delta_block,
                                            delta_down_offset));
        ...
        for (int _l = 0; _l < block_l; _l++) {
          ...
          // auto deltaA_up_block_slice = make_tensor<T>(shape_slice,
          // real_shape_slice); auto deltaBu_up_block_slice =
          // make_tensor<T>(shape_slice, real_shape_slice); auto
          // C_block_up_slice = make_tensor<T>(shape_slice, real_shape_slice);
          // auto deltaA_down_block_slice = make_tensor<T>(shape_slice,
          // real_shape_slice); auto deltaBu_down_block_slice =
          // make_tensor<T>(shape_slice, real_shape_slice); auto
          // C_block_down_slice = make_tensor<T>(shape_slice, real_shape_slice);

          // auto deltaA_up_block_slice =
          // block_deltaA_up.sub_view(real_shape_slice, delta_up_slice_offset);
          // auto deltaBu_up_block_slice =
          // block_deltaB_u_up.sub_view(real_shape_slice,
          // delta_up_slice_offset); auto C_block_up_slice =
          // block_C_up.sub_view(real_shape_slice, c_up_slice_offset);

          // auto deltaA_down_block_slice =
          // block_deltaA_down.sub_view(real_shape_slice,
          // delta_down_slice_offset); auto deltaBu_down_block_slice =
          // block_deltaB_u_down.sub_view(real_shape_slice,
          // delta_down_slice_offset); auto C_block_down_slice =
          ...
          auto deltaBu_up_block_slice_tmp = block_deltaB_u_up.sub_view(
              real_shape_slice, delta_up_slice_offset);
          auto deltaBu_up_block_slice =
          ...
          auto deltaA_down_block_slice_tmp = block_deltaA_down.sub_view(
              real_shape_slice, delta_down_slice_offset);
          auto deltaA_down_block_slice =
              deltaA_down_block_slice_tmp.view(real_shape_slice, slice_stride);
          auto deltaBu_down_block_slice_tmp = block_deltaB_u_down.sub_view(
              real_shape_slice, delta_down_slice_offset);
          ...
          dma::store(res_gtensor.sub_view(real_shape_slice, res_up_offset),
                     y_up_tensor);
          dma::store(res_gtensor.sub_view(real_shape_slice, res_down_offset),
                     y_down_tensor);
        }
      }
    }
