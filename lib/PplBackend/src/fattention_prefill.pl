...
void fattention_prefill(T *ptr_out, T *ptr_q, T *ptr_k, T *ptr_v,
                        ...
                        const int d, const int block_m, const int mask_size) {
  int head_rep = q_head / kv_head;
  int core_index = get_core_index();
  if (core_index >= core_num)
    return;
  // Process all Q/KV heads together so the `store_transpose_nc` destination
  // stays contiguous. With a partial head set the destination is strided and
  // the L2S transpose drops query rows at the small block_m this kernel needs.
  // Multi-core parallelism is over the query (M) dimension instead.
  ...
  dim4 q_shape = {q_head, block_m, 1, d};
  dim4 kv_shape = {kv_head, block_m, 1, d};
  dim4 qk_shape = {q_head, block_m, 1, block_m};
  dim4 mask_shape = {1, block_m, 1, block_m};

  dim4 mi_shape = {q_head, block_m, 1, 1};
  dim4 li_shape = {q_head, block_m, 1, 1};
  dim4 acc_shape = {q_head, block_m, 1, d};
  ...
  for (int _b = 0; _b < b; _b += 1) {
    ...
    // Parallelize across cores over the query (M) dimension: each core owns a
    // round-robin subset of the M tiles and computes full attention (all
    // heads, all keys) for them. Round-robin balances the causal workload,
    // since later tiles scan more keys.
    for (int _mt = core_index; _mt < m_tiles; _mt += core_num) {
      int _m = _mt * block_m;
      int real_m = min(block_m, qm - _m);
      dim4 qi_real_local_shape = {q_head, real_m, 1, d};
      dim4 qi_real_global_shape = {real_m, q_head, 1, d};
      dim4 qi_offset = {_m, 0, 0, 0};
      ...
      dim4 mi_real_shape = {q_head, real_m, 1, 1};
      dim4 li_real_shape = {q_head, real_m, 1, 1};
      dim4 acc_real_shape = {q_head, real_m, 1, d};
      ...
      for (int _k = 0; _k < K_end; _k += block_k_iter) {
        ...
        dim4 kvi_real_local_shape = {kv_head, real_k, 1, d};
        dim4 kvi_real_global_shape = {real_k, kv_head, 1, d};
        dim4 kvi_offset = {_k, 0, 0, 0};
        dim4 qk_real_shape = {q_head, real_m, 1, real_k};
        ...
        if (!block_no_mask) {
          dma::load(mask_tensor,
                    mask_global_tensor.sub_view(mask_real_shape, mask_offset));
        }
        ...
        for (int i = 0; i < q_head; i++) {
          ...
          auto p_tensor_batch = p_T_a16.sub_view(p_batch_shape, batch_p_offset);
...
__KERNEL__ void fattention_prefill_bf16(bf16 *ptr_out, bf16 *ptr_q, bf16 *ptr_k,
                                        bf16 *ptr_v, fp32 *ptr_mask, int b,
