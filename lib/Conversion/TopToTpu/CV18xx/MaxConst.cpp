...
namespace tpu_mlir {
namespace cv18xx {
void MaxConstLowering::LoweringINT8(PatternRewriter &rewriter, top::MaxConstOp op,
  ...
  auto weight_operand =
          top::WeightOp::create(op, "const_max", std::vector<int8_t>(nums, to_int8(const_val, ROUNDING_HALF_UP)), weight_type);
...
void MaxConstLowering::LoweringBF16(PatternRewriter &rewriter, top::MaxConstOp op) const {
  double const_val = op.getConstVal().convertToDouble();
  auto weight_type = op.getInput().getType().cast<RankedTensorType>();
  auto input_shape = module::getShape(op.getInput());
  int nums = std::accumulate(input_shape.begin(), input_shape.end(), 1, std::multiplies<int>());
  auto weight_operand =
          top::WeightOp::create(op, "const_max", std::vector<float>(nums, const_val), weight_type);
  std::vector<Value> operands;
  operands.push_back(op->getOperand(0));
  operands.push_back(weight_operand);
