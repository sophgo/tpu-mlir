...
template<typename T>
static void shuffleFilterWeight(
...
//when it is space2depthOp(ReorgOp) and its mode is "crd", convert it to "dcr" reorg + swap nextConv weight/insert ShuffleChannelOp
//because cv18xx reorgOp's backend only support "dcr" reorg.
template<typename T>
static bool convertDepth2Space(PatternRewriter &rewriter, top::Depth2SpaceOp d2sOp) {
  ...
  attrs.push_back(
      rewriter.getNamedAttr("in_is_NCHW", rewriter.getBoolAttr(d2sOp.getInIs_NCHW())));
  attrs.push_back(
      rewriter.getNamedAttr("out_is_NCHW", rewriter.getBoolAttr(d2sOp.getOutIs_NCHW())));
  attrs.push_back(
      rewriter.getNamedAttr("swap_cr", rewriter.getBoolAttr(d2sOp.getSwapCr())));
  auto newloc = NameLoc::get(rewriter.getStringAttr(name + "_DCR"));
  rewriter.setInsertionPointAfterValue(input);
  auto newReorgOp = rewriter.create<top::Depth2SpaceOp>(newloc, output.getType(), operands, attrs);
  ...
  if (convOp && convOp.getGroup() == 1) {
    ...
    attrs.emplace_back(rewriter.getNamedAttr("group", rewriter.getI64IntegerAttr(stride * stride)));
...
void Depth2SpaceLowering::LoweringINT8(PatternRewriter &rewriter, top::Depth2SpaceOp d2sOp,
                               bool asymmetric) const {
  if (convertDepth2Space<int8_t>(rewriter, d2sOp)) {
    return;
  }
  lowering_common_int8<tpu::Depth2SpaceOp>(rewriter, d2sOp.getOperation(), asymmetric);
}
