...
static void LoweringInterp(PatternRewriter &rewriter, top::InterpOp op,
  ...
  if (auto a =
          tpu::symbolizeResizeMode(op->getAttr("mode").cast<StringAttr>())) {
    op->setAttr("mode", tpu::ResizeModeAttr::get(op->getContext(), a.value()));
  }

  if (auto a = tpu::symbolizeResizeCoordMode(
          op->getAttr("coord_mode").cast<StringAttr>())) {
  ...
  if (type.isF32()) {
    rewriter.replaceOpWithNewOp<tpu::InterpOp>(op, op->getResultTypes(),
                                               operands, attrs);
    return;
  }
