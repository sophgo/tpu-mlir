...
void ReverseTryLowering::Lowering(PatternRewriter &rewriter,
  ...
  rewriter.replaceOpWithNewOp<tpu::ShapeReverseOp>(op, new_type,
                                                   op.getOperand(), attrs);
}
...
void LoweringReverse(PatternRewriter &rewriter, top::ReverseOp op, Type type) {
  ...
  auto coeff_type =
      RankedTensorType::get(dim_length, rewriter.getIntegerType(32, true));
...
void ReverseLowering::LoweringINT4(PatternRewriter &rewriter, top::ReverseOp op,
                                   bool asymmetric) const {
  LoweringF32(rewriter, op);
}
