...
void Pow3Lowering::LoweringF32(PatternRewriter &rewriter, top::Pow3Op op) const {
  ...
  auto log_op = rewriter.create<tpu::ActiveOp>(log_loc, type,
                                               ValueRange{op->getOperand(0)}, attrs);

  auto mul_loc = NameLoc::get(rewriter.getStringAttr(name.str() + "_mul"));
  attrs.clear();
  auto mul_op = rewriter.create<tpu::MulOp>(mul_loc, type,
                          ValueRange{op->getOperand(1), log_op.getOutput()}, attrs);
...
void Pow3Lowering::LoweringBF16(PatternRewriter &rewriter, top::Pow3Op op) const {
  LoweringF32(rewriter, op);
}

void Pow3Lowering::LoweringF16(PatternRewriter &rewriter, top::Pow3Op op) const {
  LoweringF32(rewriter, op);
}
