...
static void set_elu_attr(PatternRewriter& rewriter, top::EluOp op) {
  ...
  op_->setAttr("mode", tpu::ActiveModeAttr::get(op.getContext(),
                                                tpu::ActiveMode::ELU));
  op_->setAttr("coeffs",
               rewriter.getF64ArrayAttr(ArrayRef<double>{alpha_}));
  op_->removeAttr("alpha");
 }

void EluLowering::LoweringF32(PatternRewriter &rewriter,
                               top::EluOp op) const {
...
void EluLowering::LoweringF16(PatternRewriter &rewriter,
                                top::EluOp op) const {
  set_elu_attr(rewriter, op);
  lowering_common_f32<tpu::ActiveOp>(rewriter, op);
}
