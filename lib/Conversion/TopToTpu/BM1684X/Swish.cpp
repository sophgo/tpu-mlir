...
static void set_swish_attr(PatternRewriter& rewriter, top::SwishOp op) {
  ...
  op_->setAttr(
      "mode", tpu::ActiveModeAttr::get(op.getContext(), tpu::ActiveMode::SWISH));
  op_->setAttr("coeffs",
               rewriter.getF64ArrayAttr(ArrayRef<double>{beta_}));
  op_->removeAttr("beta");
...
void SwishLowering::LoweringINT8(PatternRewriter &rewriter, top::SwishOp op,
  ...
  auto table =
      create_lookup_table(op.getInput(), op.getOutput(), asymmetric, [beta](double val) {
        return val / (1 + std::exp(-val * beta));
      }, 8, round_mode);
