...
void DtypeCastLowering::LoweringINT4(PatternRewriter &rewriter, top::DtypeCastOp op,
                                bool asymmetric) const {
  LoweringINT8(rewriter, op, asymmetric);
}
void DtypeCastLowering::LoweringINT8(PatternRewriter &rewriter, top::DtypeCastOp op,
                                bool asymmetric) const {
  // auto round_mode = op.getRoundModeAttr().str();
  lowering_common<tpu::DtypeCastOp>(rewriter, op.getOperation(),
                               op.getOutput().getType());
...
//     auto newOp = lowering_common<tpu::DtypeCastOp>(rewriter, op.getOperation(),
//                                  op.getOutput().getType());
//     newOp.setRoundModeAttr(
//       tpu::RoundModeAttr::get(op.getContext(), get_round_mode(round_mode)));
//     return;
