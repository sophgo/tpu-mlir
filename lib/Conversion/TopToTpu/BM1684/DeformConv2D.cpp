...
void DeformConv2DLowering::LoweringF32(PatternRewriter &rewriter, top::DeformConv2DOp op) const {
  ...
  cpu_params.push_back(rewriter.getNamedAttr("cpu_op_name", rewriter.getStringAttr("deform_gather")));

  auto kernel = module::getI64Array(op.getKernelShape());
  gather_attrs.push_back(rewriter.getNamedAttr("kh",
                         rewriter.getI64IntegerAttr(kernel->at(0))));
  gather_attrs.push_back(rewriter.getNamedAttr("kw",
                         rewriter.getI64IntegerAttr(kernel->at(1))));
  ...
  gather_attrs.push_back(rewriter.getNamedAttr("pad_t",
                         rewriter.getI64IntegerAttr(pads->at(0))));
  gather_attrs.push_back(rewriter.getNamedAttr("pad_l",
                         rewriter.getI64IntegerAttr(pads->at(1))));
  gather_attrs.push_back(rewriter.getNamedAttr("pad_b",
                         rewriter.getI64IntegerAttr(pads->at(2))));
  gather_attrs.push_back(rewriter.getNamedAttr("pad_r",
                         rewriter.getI64IntegerAttr(pads->at(3))));
  ...
  gather_attrs.push_back(rewriter.getNamedAttr("stride_h",
                         rewriter.getI64IntegerAttr(strides->at(0))));
  gather_attrs.push_back(rewriter.getNamedAttr("stride_w",
                         rewriter.getI64IntegerAttr(strides->at(1))));
  conv_attrs.push_back(rewriter.getNamedAttr("strides",
                         rewriter.getI64ArrayAttr({1, 1})));

  auto dilations = module::getI64Array(op.getDilations(), kernel->size(), 1);
  gather_attrs.push_back(rewriter.getNamedAttr("dilation_h",
                         rewriter.getI64IntegerAttr(dilations->at(0))));
  gather_attrs.push_back(rewriter.getNamedAttr("dilation_w",
                         rewriter.getI64IntegerAttr(dilations->at(1))));
  ...
  const int conved_H = ((i_s[2] - (dilations->at(0) * (kernel->at(0) - 1) + 1) +
                         pads->at(0) + pads->at(2)) / strides->at(0) + 1);
  const int conved_W = ((i_s[3] - (dilations->at(1) * (kernel->at(1) - 1) + 1) +
                         pads->at(1) + pads->at(3)) / strides->at(1) + 1);
...
void DeformConv2DLowering::LoweringINT8(PatternRewriter &rewriter, top::DeformConv2DOp op,
