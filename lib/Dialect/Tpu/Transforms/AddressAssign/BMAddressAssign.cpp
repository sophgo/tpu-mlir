...
void BMAddressAssign::assignIOByAddrMode(
  ...
  if (module::isAddrMode(module::AddrMode::IO_TAG)) {
    ...
    bool back_to_basic = false;
    // handle case subnet's putput is model'output except last subnet
    // check subnet's input can avoid checking last subnet' output
    m.walk<WalkOrder::PostOrder>([&](func::FuncOp func) {
      if (auto call = module::getCallOp(func)) {
        std::vector<Value> subnet_inputs;
        std::vector<Value> subnet_outputs;
        module::getInputsOutputs(call, subnet_inputs, subnet_outputs);
        for (auto input_values : subnet_inputs) {
          if (std::find(outputs.begin(), outputs.end(), input_values) !=
              outputs.end()) {
            back_to_basic = true;
            return;
          }
