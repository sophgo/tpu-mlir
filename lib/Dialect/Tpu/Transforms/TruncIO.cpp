...
public:
  ...
  int size() const { return bitset.count(); }
  void insert(int num) {
  ...
  iterator end() const { return iterator(this, bitset.size()); }
  void dump() const {
...
public:
  ...
  bool ins_empty() const { return expected_input_names.empty(); }

  bool outs_empty() const { return expected_output_names.empty(); }

  bool empty() const { return ins_empty() && outs_empty(); }

  void add_input_name(std::string name) { expected_input_names.insert(name); }
...
static void register_blk_out(Operation *blk_op, upd_info_t &upd_info,
                             const Value &v) {
...
public:
  ...
  static void analyze_from_bt(Operation *blk_op, Operation *end_op_in_blk,
                              upd_info_t &upd_info) {
  ...
  static void analyze_from_bt(Operation *blk_op, int out_idx,
                              upd_info_t &upd_info) {
    upd_info.rem_out_idxes.insert(out_idx);
...
public:
  ...
  static void analyze_from_bt(Operation *op, Operation *end_op_in_blk,
                              upd_info_t &upd_info) {
    if (isa<tpu::GroupOp>(op)) {
  ...
  static void analyze_from_bt(Operation *op, int out_idx,
                              upd_info_t &upd_info) {
    if (isa<tpu::GroupOp>(op)) {
...
static void update_funcOp_outs(func::FuncOp &funcOp,
                               const Vector<Value> &outs) {
  ...
  funcOp.setType(mlir::FunctionType::get(funcOp.getContext(),
                                         funcOp.getArgumentTypes(), out_types));
}

static void update_funcOp_outs(func::FuncOp &funcOp,
                               const IndexList &rem_out_idxes,
                               const Set<PValue> &new_outs) {
...
public:
   ...
   */
  static void work(Operation *blk_op, const upd_info_t &blk_upd_info,
                   upd_info_t &sn_upd_info) {
    ...
    int64_t store_base_id =
        fix_group_flow(groupOp, blk_upd_info, live_ops, loads);
    Vector<Value> yields;
    ...
    llvm::append_range(yields, new_yields), yield_op->setOperands(yields);
...
private:
  static LayerGroupAttr make_lg_param(const LayerGroupAttr &ginfo,
                                      int64_t stage, int64_t id) {
  ...
  static void set_LgOp_id(Operation *op, int64_t id) {
    ...
    auto ctx = g_moduleOp.getContext();
    auto param = LayerGroupAttr::get(
        ctx, ginfo.getOutAddr(), ginfo.getOutSize(), ginfo.getBufferAddr(),
        ginfo.getBufferSize(), ginfo.getEuAlign(), ginfo.getCanMerge(),
        ginfo.getInHsliceOffset(), ginfo.getNIdx(), ginfo.getNSlice(),
        ginfo.getCIdx(), ginfo.getCSlice(), ginfo.getDIdx(), ginfo.getDSlice(),
        ginfo.getHIdx(), ginfo.getHSlice(), ginfo.getWIdx(), ginfo.getWSlice(),
        id, ginfo.getStage(), ginfo.getSliceIdx(), ginfo.getGroupType());
    op->setAttr(LocalGenInterface::kLayerGroupAttrName, param);
  }
  ...
  static void try_insert_loadOps(Operation *group_op,
                                 const upd_info_t &blk_upd_info,
                                 const Vector<Pair<PValue, int64_t>> &loads) {
    ...
    for (auto [idx, pair] : llvm::enumerate(loads)) {
      ...
      auto loadOp = builder.create<tpu::LoadOp>(
          NameLoc::get(builder.getStringAttr("load_" + name)), v.getType(),
          operands, attrs);
      v.replaceAllUsesWith(loadOp.getOutput());
    }
  }

  static Vector<Value> try_insert_storeOps(const upd_info_t &blk_upd_info,
                                           int64_t base_id) {
    const auto ctx = g_moduleOp.getContext();
    OpBuilder builder(ctx);
    ...
    for (auto &pv : blk_upd_info.new_outs) {
      ...
      auto storeOp = builder.create<tpu::StoreOp>(
          NameLoc::get(builder.getStringAttr(name)), v.getType(), operands,
          attrs);
  ...
  static int64_t get_group_ts_rows(tpu::GroupOp groupOp,
  ...
  static void set_group_ts_rows(tpu::GroupOp groupOp,
                                const Vector<Vector<int64_t>> &ts_rows) {
    const auto ctx = g_moduleOp.getContext();
    OpBuilder builder(ctx);
    Vector<int64_t> flow;
  ...
  static Vector<Operation *>
  find_group_live_ops(tpu::GroupOp groupOp, const upd_info_t &blk_upd_info) {
  ...
  static int64_t fix_group_flow(tpu::GroupOp groupOp,
                                const upd_info_t &blk_upd_info,
  ...
  static Operation *reconstruct(tpu::GroupOp groupOp,
                                const upd_info_t &blk_upd_info,
                                upd_info_t &sn_upd_info) {
    const auto ctx = g_moduleOp.getContext();
    OpBuilder builder(ctx);
    ...
    auto new_groupOp = builder.create<tpu::GroupOp>(
    ...
    int new_idx = 0;
    for (auto idx : blk_upd_info.rem_out_idxes) {
      groupOp->getResult(idx).replaceAllUsesWith(
          new_groupOp->getResult(new_idx));
      new_idx++;
    }
...
public:
   ...
   * 2. create new result and link it to store related op, which should be
   * passed to parent function as `new_outs`
...
private:
  ...
  void analyze_subnet_data_flow_from_bt(
      int subnet_id, Map<Operation *, upd_info_t> &blk_upd_infos) {
    auto &sn_upd_info = sn_upd_infos[subnet_id];
    FuncDataFlowAnalysis::analyze_from_bt(live_values[subnet_id], sn_upd_info,
                                          blk_upd_infos);
  ...
  void update_subnet_io_maps_for_main(const Vector<int> &live_subnet_ids) {
    ...
    auto find_idx_upd_map = [](const IndexList &idxes,
                               Map<int, int> &idx_upd_map) {
      int new_idx = 0;
    ...
    for (auto subnet_id : live_subnet_ids) {
      find_idx_upd_map(sn_upd_infos[subnet_id].rem_in_idxes,
                       in_idx_upd_maps[subnet_id]);
      find_idx_upd_map(sn_upd_infos[subnet_id].rem_out_idxes,
                       out_idx_upd_maps[subnet_id]);
    }
  ...
  void work_for_main() {
    ...
    for (auto subnet_id : live_subnet_ids) {
      ...
      for (auto pv : sn_upd_info.new_ins) {
        ...
        auto inputOp = builder.create<top::InputOp>(v.getLoc(), v.getType(),
                                                    ValueRange{arg});
        new_call_ins.push_back(inputOp.getOutput());
      }
      auto new_callOp = builder.create<func::CallOp>(
          builder.getUnknownLoc(), dyn_cast<func::FuncOp>(subnets[subnet_id]),
          new_call_ins);
      // replace all uses of from operand of groupOp to that of new_groupOp
      int new_idx = 0;
      for (auto idx : sn_upd_info.rem_out_idxes) {
        call_op->getResult(idx).replaceAllUsesWith(
            new_callOp->getResult(new_idx));
        new_idx++;
      }
...
public:
  void invoke(int trunc_mode) {
    ...
    sn_upd_info.new_ins = new_ins;
    update_funcOp_outs(mainFuncOp, sn_upd_info.rem_out_idxes,
                       sn_upd_info.new_outs);
    remove_unused_ops(mainFuncOp);
    update_funcOp_ins(mainFuncOp);
  }
};
...
/**
 * From top to bottom, find first op END_OP whose one output matches the
 * expected output. Set the network outputs as the outputs of END_OP which
 * matches. From bottom to top, find last op START_OP whose one input matches
 * the expected input. Set the network inputs as the outputs of START_OP which
 * matches. Note that the number of START_OP is usually more than one.
 */
