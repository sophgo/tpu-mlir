...
public:
  ...
  LogicalResult matchAndRewriteImpl(tpu::AddOp op,
    ...
    auto leftOp   = dyn_cast_or_null<tpu::CastOp>(op.getOperand(0).getDefiningOp());
    auto rightOp  = dyn_cast_or_null<tpu::CastOp>(op.getOperand(1).getDefiningOp());
    int  leftValid = 0, rightValid = 0;
    auto op_name = module::getName(op.getResult()).str();
    op_name = op_name;

    if (leftOp) {
       if(!leftOp->hasOneUse()) return failure();
       auto leftOutputType = module::getElementType(leftOp.getResult());
       if ((!module::isUniformQuantized(leftOp.getInput())) || (!leftOutputType.isBF16())) {
        leftValid = 0;
       } else {
        leftValid = 1;
       }
    ...
    if (rightOp) {
       if(!rightOp->hasOneUse()) return failure();
       auto rightOutputType = module::getElementType(rightOp.getResult());
       if ((!module::isUniformQuantized(rightOp.getInput())) || (!rightOutputType.isBF16())) {
        rightValid = 0;
       } else {
        rightValid = 1;
       }
    }
    if (!(leftValid || rightValid)) return failure();
    ...
    new_castAdd_op->setAttr("relu_limit", rewriter.getF64FloatAttr(
...
public:
  ...
  LogicalResult matchAndRewriteImpl(tpu::ReshapeOp op,
    ...
    if (auto layerNormOp = dyn_cast_or_null<tpu::LayerNormOp>(op.getInput().getDefiningOp())) {
...
only support Cast: i8 => bf16
*/
class FuseDequantLayerNormPattern : public OpRewriterPatternEx<tpu::LayerNormOp> {
public:
  using OpRewriterPatternEx::OpRewriterPatternEx;
  FuseDequantLayerNormPattern(mlir::MLIRContext *context, int benifit)
      : OpRewriterPatternEx<tpu::LayerNormOp>(context, "FuseDequantLayerNormPattern",
                                           benifit) {}
  LogicalResult matchAndRewriteImpl(tpu::LayerNormOp op,
    ...
    auto FrontOp   = dyn_cast_or_null<tpu::CastOp>(op.getOperand(0).getDefiningOp());
    int  castValid = 0;

    if (FrontOp) {
       if(!FrontOp->hasOneUse()) return failure();
       auto leftOutputType = module::getElementType(FrontOp.getResult());
       if ((!module::isUniformQuantized(FrontOp.getInput())) || (!leftOutputType.isBF16())) {
        castValid = 0;
       } else {
        castValid = 1;
       }
    ...
    auto output = op.getOutput();
    output.replaceAllUsesExcept(new_layer_norm_op.getOutput(), new_layer_norm_op);
    op.erase();
    if (castValid) {
...
public:
  ...
  FuseLayerNormCastPattern(mlir::MLIRContext *context, int benifit)
      : OpRewriterPatternEx<tpu::LayerNormOp>(context, "FuseLayerNormCastPattern",
                                           benifit) {}
  LogicalResult matchAndRewriteImpl(tpu::LayerNormOp op,
                                    PatternRewriter &rewriter) const override {
    if(!module::isMARS3()) return failure();
    ...
    Castoutput.replaceAllUsesExcept(new_layer_norm_op.getOutput(), new_layer_norm_op);
    if (castValid) {
...
public:
  ...
  LogicalResult matchAndRewriteImpl(tpu::MatMulOp op,
    ...
    if (LutOp) {
       if(!LutOp->hasOneUse()) return failure();
       LutValid = 1;
    }
    if (!(LutValid)) return failure();
    std::vector<Value> opds;
...
public:
  ...
  LogicalResult matchAndRewriteImpl(tpu::MatMulOp op,
    ...
    auto op_name  = module::getName(op.getResult()).str();
    op_name = op_name;
    auto LutOp    = dyn_cast_or_null<tpu::LutOp>(*op.getResult().getUsers().begin());
    int  LutValid = 0;
    if (LutOp) {
       LutValid = 1;
    }
    ...
    outputLut.replaceAllUsesExcept(new_lutMatMul_op.getOutput(), new_lutMatMul_op);
    LutOp.erase();
    return success();
  }
};

class ActiveCastFusePattern
...
public:
  ActiveCastFusePattern(mlir::MLIRContext *context, int benefit)
      : OpRewriterPatternEx<tpu::CastOp>(
            context, "ActiveCastFusePattern", benefit) {}

  LogicalResult matchAndRewriteImpl(tpu::CastOp castOp,
                                    PatternRewriter &rewriter) const override {
    if(!module::isMARS3()) return failure();
    if(castOp->hasAttr(LocalGenInterface::kLayerGroupAttrName)) return failure();
    ...
    if (auto activeOp = dyn_cast_or_null<tpu::ActiveOp>(def_op)) {
      ...
      auto newOp = rewriter.create<tpu::FusedActiveCastOp>(
        out.getLoc(), out.getType(), activeOp.getInput(),
        attrs);
