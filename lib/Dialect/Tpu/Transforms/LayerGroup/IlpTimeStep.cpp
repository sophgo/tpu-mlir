...
bool SortByOpCycleDiff(const std::pair<int, std::vector<Operation *>> &v1,
                       const std::pair<int, std::vector<Operation *>> &v2) {
...
bool l2mem_alloc::alloc(int slice_idx, const std::string &name, Value value,
  ...
  if (count == size) {
    ...
    fprintf(stderr, "%s\n",
            llvm::formatv("        alloc ok for {0}, free_addr:{1}", name,
                          free_addr)
                .str()
                .c_str());
    mem_struct mem_s;
...
bool lmem_alloc::_alloc(int slice_idx, const std::string &name, Value value,
  ...
  while (true) {
    ...
    if (count == size) {
      ...
      fprintf(
          stderr, "%s\n",
          llvm::formatv("        alloc ok for {0}, free_addr:{1}, bank_id:{2}",
      ...
      if (!rehearsal) {
        ...
        if (vec_mem_alloc_his.find(key) != vec_mem_alloc_his.end()) {
          if (isBuffer) {
            auto &itr = vec_mem_alloc_his[key].vec_reload_addr.back();
            itr.second.addr = free_addr;
            itr.second.bank_id.assign(ret_bank_id.begin(), ret_bank_id.end());
          } else {
            if (for_move) {
              // auto& itr = vec_mem_alloc_his[key].vec_reload_addr.back();
              // itr.second.addr = free_addr;
              // itr.second.bank_id.assign(ret_bank_id.begin(),
              // ret_bank_id.end());
            } else {
...
bool lmem_alloc::alloc_multi(int ts_idx,
  ...
  if (sort_by_size) {
    ...
    fprintf(stderr, "\n");
    std::vector<Value> vec_req_mem_value;
    for (auto it : vec_mem_req) {
      vec_req_mem_value.push_back(it.value);
    ...
    if (m_pILPTimeStep->inserted_timestep_table_.find(ts_idx) !=
      ...
      if (m_pILPTimeStep->inserted_timestep_table_[ts_idx].size() > 1) {
        std::vector<Value> move_value_all;
        for (auto itr : m_pILPTimeStep->inserted_timestep_table_[ts_idx]) {
          for (auto itr2 : itr.move_value) {
            if (std::find(vec_req_mem_value.begin(), vec_req_mem_value.end(),
                          itr2) == vec_req_mem_value.end() &&
                std::find(move_value_all.begin(), move_value_all.end(), itr2) ==
                    move_value_all.end()) {
              move_value_all.push_back(itr2);
            }
          }
        }
        for (auto v : move_value_all) {
          llvm::errs() << "move_value_all:" << module::getName(v).str() << "\n";
        }
        ts_move_info tmp;
        int64_t dest_add = 0, size = 0, slice_idx = 0;
        std::map<Value, int64_t, value_compare> first_move_src_add;
        for (auto move_v : move_value_all) {
          for (auto itr : m_pILPTimeStep->inserted_timestep_table_[ts_idx]) {
            auto res =
                std::find(itr.move_value.begin(), itr.move_value.end(), move_v);
            if (res != itr.move_value.end()) {
              int pos = std::distance(itr.move_value.begin(), res);
              if (first_move_src_add.find(move_v) == first_move_src_add.end()) {
                first_move_src_add[move_v] = itr.move_src_add[pos];
              }
              dest_add = itr.move_dest_add[pos];
              size = itr.move_size[pos];
              slice_idx = itr.slice_idx[pos];
              tmp.name = itr.name;
            }
          }
          tmp.move_value.push_back(move_v);
          tmp.move_src_add.push_back(first_move_src_add[move_v]);
          tmp.move_dest_add.push_back(dest_add);
          tmp.move_size.push_back(size);
          tmp.slice_idx.push_back(slice_idx);
          fprintf(stderr,
                  "      src_add:%ld, dest_add:%ld, slice_idx:%ld, size:%ld\n",
                  first_move_src_add[move_v], dest_add, slice_idx, size);
        }
        m_pILPTimeStep->inserted_timestep_table_[ts_idx].clear();
  ...
  do {
    ...
    for (auto i : order_idx) {
      if (alloc_ret[i]) {
        free(
            convert_name_to_key(vec_mem_req[i].name, vec_mem_req[i].slice_idx));
      }
    }
    ...
    for (auto i : order_idx) {
      ...
      if (!_alloc(vec_mem_req[i].slice_idx, vec_mem_req[i].name,
                  vec_mem_req[i].value, vec_mem_req[i].size, ret_bank_id,
                  free_addr, conflict_size, false, vec_mem_req[i].isBuffer)) {
        fprintf(stderr, "_alloc fail\n");
        success = false;
        break;
      }
      fprintf(stderr, "  free_addr:%d, conflict_size:%d\n", free_addr,
              conflict_size);
...
// refc：暂时只使用默认值1，考虑弃用
bool lmem_alloc::alloc(int ts_idx, int slice_idx, const std::string &name,
                       Value value, int size, bool isBuffer) {
  fprintf(stderr, "%s\n",
          llvm::formatv("      start alloc for {0}, size:{1}, slice_idx:{2}",
                        name, size, slice_idx)
              .str()
              .c_str());
  assert(size > 0);
  int free_addr = -1, conflict_size = -1;
  std::vector<int> ret_bank_id;
  if (!_alloc(slice_idx, name, value, size, ret_bank_id, free_addr,
              conflict_size, false, isBuffer)) {
    ...
    for (
        int i = 1; i <= 5;
        i++) { //以最大空闲区域为中心，均匀的在两侧寻找最多5块空闲区域,以便能满足本次内存分配空间
      ...
      do {
        ...
        for (int it : order_idx) {
          ...
          if (i == -1) {
            ...
            if (!_alloc(slice_idx, name, value, size, ret_bank_id, free_addr,
                        conflict_size, force_not_care_bank, isBuffer)) {
    ...
    for (int it : min_conflict_order_idx) {
      auto i = new_vec_move_mem[it];
      fprintf(stderr, "  i:%d\n", i);
      if (i == -1) {
        if (!_alloc(slice_idx, name, value, size, ret_bank_id, free_addr,
    ...
    for (int it : min_conflict_order_idx) {
      auto i = new_vec_move_mem[it];
      if (i != -1 && vec_mem_struct2[i].second.type != 3) {
    ...
    if (m_pILPTimeStep->inserted_timestep_table_.find(ts_idx) ==
        m_pILPTimeStep->inserted_timestep_table_.end()) {
      std::vector<ts_move_info> vec_move_info;
      vec_move_info.push_back(tmp);
      m_pILPTimeStep->inserted_timestep_table_[ts_idx] = vec_move_info;
    } else {
      m_pILPTimeStep->inserted_timestep_table_[ts_idx].push_back(tmp);
    }
    fprintf(stderr, "%s\n",
            llvm::formatv("      alloc ok for {0}, size:{1}", name, size)
                .str()
                .c_str());
  }
...
void ILPTimeStep::addBinaryVar(int ts_idx, int slice_idx, int mode,
  ...
  if ((info.mode2 & TIMESTEP2_STORE_AND_LOAD && mode == 0) ||
      info.mode2 & TIMESTEP2_STORE) {
    vecFreeByStoreVar.push_back(value);
  }
}

void ILPTimeStep::addTensorSize(Value value, int slice_idx, int lmem_size) {
  auto key = std::make_pair(value, slice_idx);
  llvm::errs() << "addTensorSize res:" << module::getName(value).str()
               << " slice_idx:" << slice_idx << " lmem_bytes:" << lmem_size
               << "\n";
  dam_tensor_size[key] = align(lmem_size, 64);
...
void ILPTimeStep::addRowConstraint(int ts_idx, Value value,
  ...
  llvm::errs() << "add into mapConsInfo, value: "
               << module::getName(value).str() << ", slice_idx:" << slice_idx
               << ", store:" << store << ", var_names[0]: " << var_names[0]
               << "\n";
  if (store) {
  ...
  if (is_weight) {
    ...
    if (need_load_var.find(value) == need_load_var.end()) {
      std::vector<std::string> null_names;
      need_load_var[value] = null_names;
    }
    need_load_var[value].assign(var_names.begin(), var_names.end());
  }
}
...
bool ILPTimeStep::merge_small_cycle_op(
    TensorInfo &tensor_infos, std::shared_ptr<dot_graph> dot_graph_log) {
  ...
  for (int ts_idx = ts_count - 1; ts_idx >= 0; ts_idx--) {
    if (!(ts_idx == 0 || ts_idx == ts_count - 1)) {
      ...
      min_pos = 0;
      if (cycle_sum.size() > 1) {
        auto min_cycle = *std::min_element(cycle_sum.begin(),
                                           cycle_sum.end()); //找到相差最小的
  ...
  for (int merge_start = ts_count - 1; merge_start >= 0; merge_start--) {
    ...
    for (
        int m = 1; m <= min_pos;
        m++) { // load类型变量，保留融合op中最后一个ts(merge_start)的变量设置，其他ts的变量固定为0
    ...
    for (
        int m = 0; m < min_pos;
        m++) { // store类型变量，保留融合op中第1个ts的变量设置，其他ts的变量固定为0
      for (auto it3 : cycle_contrains[merge_start - m]) {
        int64_t mode2 = mapILPVarInfo[it3.second].tensor_info.mode2;
        if (mode2 & TIMESTEP2_STORE ||
            (mode2 & TIMESTEP2_STORE_AND_LOAD &&
...
bool ILPTimeStep::prepare(TensorInfo &tensor_infos) {
  ...
  for (int i = 0; i < ts_count; i++) {
    ...
    if (mem_contrains_new[i].size() > 0) {
      ...
      for (auto it : timestep_table_new[i].vec_op_infos) {
        if (min_free_mem == -1 ||
            it.mem_size_for_load - ts_reside_value_size < min_free_mem) {
          min_free_mem = it.mem_size_for_load - ts_reside_value_size;
        ...
        for (auto in : get_input_values(it.op)) {
          ...
          if (reside_in_tensor.find(it.op) != reside_in_tensor.end()) {
            ...
            if (find(tensors.begin(), tensors.end(), in) != tensors.end()) {
              ...
              op_reside_value_size += dam_tensor_size[key];
              ts_reside_value_size += dam_tensor_size[key];
            }
          }

          if (find(vecFreeByStoreVar.begin(), vecFreeByStoreVar.end(), in) !=
              vecFreeByStoreVar.end()) {
            op_reside_value_size += dam_tensor_size[key];
            ts_reside_value_size += dam_tensor_size[key];
          }
        }
...
void ILPTimeStep::get_group_cycle_info(
    int &total_cycle, int &total_diff,
  ...
  for (auto &itr : timestep_table_new) {
    ...
    for (auto &itr2 : itr.vec_ts_var) {
      if (itr2.var_value) {
        for (auto itr3 : cycle_contrains_new[i]) {
          if (itr3.second == itr2.varName) {
            if (slice_idx > 1 && map_reside_value_info.find(itr2.value) ==
                                     map_reside_value_info.end()) {
...
bool ILPTimeStep::mem_alloc(mem_alloc_status &alloc_status,
      ...
      if (mem_contrains_new[i].size() > 0) {
        ...
        for (auto it : timestep_table_new[i].vec_op_infos) {
          double free_mem_size =
              it.mem_size_for_load - size; //每个op的可加载内存减去size
  ...
  for (auto &itr : timestep_table_new) {
    ...
    ;
    for (auto itr2 : itr.vec_op_infos) {
      for (auto itr3 = itr2.need_load_var.begin();
           itr3 != itr2.need_load_var.end(); ++itr3) {
  ...
  for (int ts_idx = 0; ts_idx < ts_count; ts_idx++) {
    ...
    if (ts_idx > 0 && ts_idx < ts_count - 1) {
      ...
      for (auto it2 : timestep_table_new[ts_idx].vec_op_infos) {
        ...
        if (buffer_size >
            0) { //将op buffer分配放在outs分配后面能避免碎片处理复杂化
          Value tmp_value;
          mem_alloc_req_info tmp;
          tmp.slice_idx = it2.slice_idx;
        ...
        //   sort_by_size = false;
        // }
        ret = lmem_alloc_ptr->alloc_multi(ts_idx, vec_mem_req, sort_by_size);
        if (!ret) {
        ...
        if (ts_idx > 1 &&
            !have_mem_dependent) { //复合op中某子op已与上一时戳有依赖，则不再重复检查
          for (auto it1 :
               vec_mem_req) { //当前的请求不能与上一个时戳的任一释放有依赖
            mem_struct mem_s;
    ...
    if (ts_idx > 1) {
      for (auto it :
           timestep_table_new[ts_idx]
               .vec_ts_var) { //在最后store，本ts store时也不能给本时隙的load用
        name = module::getName(it.value).str();
        if (it.var_value == 1 &&
