...
void speical_layer_group_base::get_batch_size(shape_secs_t &shape_secs) {
  ...
  if (!col_cut || (name() == "mlp_group" && module::getChip() != module::Chip::BM1690)) {
    shape_secs.h = 1;
  }
...
void speical_layer_group_base::fill_slice_info(ilp_LgInfo &ilp_lg_info) {
  ...
  if (hdim_is_batch) {
    ...
    auto shape =  main_mm_op->getOperand(0).getType().cast<RankedTensorType>().getShape().vec();
...
bool speical_layer_group_base::inc_n_slice_num(int& n_slice_num, int max_n_slice_num) {
...
bool speical_layer_group_base::inc_slice_num(Operation *op, int& n_slice_num, int& try_c_slice_num, int& try_h_slice_num,
                    int max_n_slice_num, int max_c_slice_num, int max_h_slice_num, int old_target_secs,  bool inc_c_slice) {
  bool tmp_inc_c_slice = inc_c_slice;
  int old_n_slice_num = n_slice_num, old_try_c_slice_num = try_c_slice_num, old_try_h_slice_num = try_h_slice_num;
  for (int i = 0; i < 3; i++) { //why 3? If the first two fail, do the first one again
      ...
      if (tmp_inc_c_slice) {
        if (try_c_slice_num < max_c_slice_num) {
          ...
          if (try_h_slice_num < max_h_slice_num && is_cut_h(op)) {
            try_h_slice_num++;
            llvm::errs() << "inc_c_slice, try_h_slice_num++ ->"<<try_h_slice_num<<"\n";
          } else {
...
bool BfsCheckOutValue(Value value, std::vector<Operation*>& grp_ops) {
  ...
  while(!q.empty()) {
    ...
    for (auto user: op->getUsers()) {
      if (!isa<ReturnOp>(user)) {
        if (std::find(grp_ops.begin(), grp_ops.end(), user) == grp_ops.end()) {
          if (std::find(checked_ops.begin(), checked_ops.end(), user) != checked_ops.end()) {
            q.push(user);
          }
        } else {
          llvm::errs() <<"BfsCheckOutValue fail\n";
...
bool speical_layer_group_base::CalcMatMulGroupTpNum(ilp_LgInfo &lg_info, Operation*& failed_op, int64_t core_num) {
  ...
  int glo_n_slice_num = get_best_n_slice_num(batch_size, core_num), glo_c_slice_num = 1, glo_h_slice_num = 1;
  ...
  for (auto op: tmp_ops) {
    ...
    int try_n_slice_num = glo_n_slice_num, try_c_slice_num = glo_c_slice_num, try_h_slice_num = glo_h_slice_num;
    int pre_valid_loc_slice_h = glo_h_slice_num, pre_valid_loc_slice_c = glo_c_slice_num;
    int slice_max_n = get_slice_max_n(batch_size, glo_n_slice_num), pre_valid_loc_slice_n = glo_n_slice_num;
    ...
    do {
      ...
      int64_t in0_lmem_bytes = 0, in1_lmem_bytes = 0, in2_lmem_bytes = 0, out0_lmem_bytes = 0;
      ...
      if (name() == "attention_group") {
        if (isa<tpu::MatMulOp>(op) && try_h_slice_num > 1 && op == ops.back()) {
          ...
          //the store of the previous time slot and the load of the next time slot
          out0_lmem_bytes *= 2;
        }
      }
      ...
      if (ins.size() > 1) {
          ...
          if (isa<tpu::MatMulOp>(op)) {
            if (op == lg_info._lgInfo.group_ops[0]) {
              ...
              if (dyn_cast<tpu::MatMulOp>(op).getRightTranspose()) {
                ...
                //Note that the number of rows for the right matrix of the second matml is try_h_slice_num
                in_c = align(in_c, try_h_slice_num)/try_h_slice_num;
              }
            }
          } else {
        ...
        if (in_n != batch_size && in_n == 1) {
          in1_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(ins[1], 1, in_c, in_d, in_h, in_w));
        } else {
          ...
          if (isa<tpu::MatMulOp>(op)) {
            if (op == ops.back()) {
              ...
              if (in1_lmem_bytes >= Arch::LMEM_BYTES) {
                llvm::errs() << "the right matrix of Matmul before softmax is too large, op:"
      ...
      if (ins.size() > 2) {
        ...
        llvm::errs() << "in2_n:" <<in_n<< ", in_c:" <<in_c << ", in_h:" <<in_h<< ", in_w:" <<in_w<<'\n';
        in_h = align(in_h, try_h_slice_num)/try_h_slice_num;
        llvm::errs() << "new in2_h:" <<in_h<<'\n';
        if (in_n != batch_size && in_n == 1) {
          in2_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(ins[2], 1, in_c, in_d, in_h, in_w));
        } else {
          in2_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(ins[2], slice_max_n, in_c, in_d, in_h, in_w));
        }
        if (name() == "attention_group") {
          if (op == ops.back()) {
            if (try_h_slice_num > 1) {
            ...
            if (in2_lmem_bytes >= Arch::LMEM_BYTES) {
              llvm::errs() << "the bias matrix of Matmul before softmax is too large, op:"
                            <<module::getName(op).str()<<"\n";
              failed_op = op;
              return false;
            }
      ...
      if (mem_enough) {
        ...
        if(!inc_slice_num(op, try_n_slice_num, try_c_slice_num, try_h_slice_num, batch_size, lg_info.shape_secs.c,
                          lg_info.shape_secs.h, old_target_secs, inc_c_slice)) {
          failed_op = op;
          return false;
        }
  ...
  llvm::errs() << "n:" <<lg_info.shape_secs.n<< ", c:" <<lg_info.shape_secs.c<< ", h:" <<lg_info.shape_secs.h
              ...
              << ", glo_c_slice_num:" <<glo_c_slice_num<< ", glo_h_slice_num:" <<glo_h_slice_num<<'\n';
  return true;
}
...
static void CollectElementwiseOpAroundMatmul(Operation *op, const std::vector<Operation*> &subnet_ops,
                                             std::vector<Operation*>& result_ops,
                                             int forward_search = 0) {
  result_ops.push_back(op);
  if (forward_search == 0 || forward_search == 1) {
    for (auto v : op->getOperands()) {
      auto pre_op = v.getDefiningOp();
      if (std::find(subnet_ops.begin(), subnet_ops.end(), pre_op) != subnet_ops.end()) {
        if (!isa<top::NoneOp, top::WeightOp, top::InputOp>(pre_op) && isElementwiseOp(pre_op)) {
...
public:
  ...
  virtual bool pattern_match_and_parser(Operation* start_op, std::vector<Operation*>& subnet_ops) override {
    if (isa<tpu::MatMulOp>(start_op)) {
      main_mm_op = start_op;
      llvm::errs()<<"find single_matmul_group at "<<module::getName(start_op).str()<<"\n";
      ...
      for (auto op: ops) {
        if (isa<tpu::ReshapeOp>(op)) {
          ...
          //reshape + matmul + reshape + (slice0 + slice1) -> matmul + (slice0 + slice1)
          if (isa<tpu::SliceOp, tpu::AddOp>(next_op) && std::find(ops.begin(), ops.end(), next_op) != ops.end()) {
            ...
            if (mmOp && isa<tpu::MatMulOp>(mmOp)) {
              ...
              if (MMinReshapeOp && isa<tpu::ReshapeOp>(MMinReshapeOp)) {
                ...
                MMinReshapeOp->getResult(0).replaceUsesWithIf(MMinReshapeOp->getOperand(0), [&](OpOperand &operand) {
                  Operation *user = operand.getOwner();
                  return find(ops.begin(), ops.end(), user) != ops.end();
                });
                op->getResult(0).replaceUsesWithIf(mmOp->getResult(0), [&](OpOperand &operand) {
                  Operation *user = operand.getOwner();
                  return find(ops.begin(), ops.end(), user) != ops.end();
                });
  ...
  virtual bool convert_to_other_type(std::vector<Operation*>& sub_ops,
...
public:
  ...
  virtual bool CalcMatMulGroupTpNum(ilp_LgInfo &lg_info, Operation *&failed_op, int64_t core_num) override {
    auto mm_op = dyn_cast<tpu::MatMulOp>(main_mm_op);
    if (mm_op && !mm_op.getHdimIsBatch()) {
      return speical_layer_group_base::CalcMatMulGroupTpNum(lg_info, failed_op, core_num);
    }
    hdim_is_batch = true;
    ...
    for (auto op: tmp_ops) {
      ...
      auto outs = get_output_values(op);
      int try_n_slice_num = glo_n_slice_num, try_c_slice_num = glo_c_slice_num, try_h_slice_num = glo_h_slice_num;
      int pre_valid_loc_slice_n = glo_n_slice_num, pre_valid_loc_slice_h = glo_h_slice_num, pre_valid_loc_slice_c = glo_c_slice_num;
      llvm::errs() << "CalcMatMulGroupTpNum for op:" <<module::getName(op).str()<<'\n';
      ...
      do {
        ...
        in0_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(ins[0], cut_n, in_c, 1, cut_h, in_w));
        ...
        out0_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(outs[0], cut_n, out_c, 1, cut_h, out_w));
        if (isa<tpu::MatMulOp>(op) && try_h_slice_num > 1 && op == ops.back()) {
          //The second matmul input and output occupy twice the memory to consider
          //the store of the previous time slot and the load of the next time slot
          out0_lmem_bytes *= 2;
        ...
        auto lg_op = cast<LocalGenInterface>(op);
        int64_t buffer_size = lg_op.getBufferSize(in0_lmem_bytes, out0_lmem_bytes, cut_n, in_c,
                                          cut_h, 1, in_w, cut_n, out_c, cut_h, 1, out_w, type);
        if (ins.size() > 1) {
          ...
          in1_lmem_bytes = align_64(Arch::get_tensor_lmem_bytes(ins[1], cut_n, shape[1], 1, cut_h, shape[3]));
          if (isa<tpu::MatMulOp>(op)) {
            if (op == ops.back()) {
              if (try_h_slice_num > 1) {
                in1_lmem_bytes *= 2;
        ...
        if (mem_enough) {
          ...
          if(!inc_slice_num(op, try_n_slice_num, try_c_slice_num, try_h_slice_num, batch_size, lg_info.shape_secs.c,
                            enable_cut_h?lg_info.shape_secs.h:1, old_target_secs, inc_c_slice)) {
            failed_op = op;
            return false;
          }
          secs = get_secs(op, try_n_slice_num, try_c_slice_num, try_h_slice_num);
  ...
  virtual bool convert_to_other_type(std::vector<Operation*>& sub_ops, std::shared_ptr<speical_layer_group_base>& p_special_grp) override {
    if (!grp_is_valid(sub_ops)) {
      return false;
    }
...
void GroupOps::SearchGroup(std::vector<dag_subnet>& dag_subnets, std::shared_ptr<speical_layer_group_base> grp_ptr) {
  while(true) {
        ...
        for (auto op: dag.ops) {
          ...
          if (tmp_grp_ptr->pattern_match_and_parser(op, dag.ops)) {
            ...
            need_del_ops.insert(need_del_ops.end(),
                          tmp_grp_ptr->need_del_ops.begin(), tmp_grp_ptr->need_del_ops.end());
            dag.matched = true;
            ...
            for (auto grp_ops2: seg_network_by_group_ops(dag.ops, tmp_grp_ptr->ops)) {
    ...
    dag_subnets.insert(dag_subnets.end(), new_dag_subnets.begin(), new_dag_subnets.end());
    if (all_checked) {
      break;
    }
  }
...
void GroupOps::findSpecialGroup(llvm::SetVector<Operation*>& subnet_ops) {
  ...
  for (auto dag: all_dag_subnet) {
    if (grp_is_valid(dag.ops)) {
      ...
      excluded_ops.clear();
      for (auto ops2: seg_grp_ops_by_global_op(dag.ops, global_layers, excluded_ops, options_)) {
        if (grp_is_valid(ops2)) {
          llvm::errs()<<"normal group, ops2.size:"<<ops2.size()<<"\n";
          lg_pass_ir_->tmp_base_groups.push_back(CreateIlpLgInfo(ops2, options_));
        }
      }
    }
  }
