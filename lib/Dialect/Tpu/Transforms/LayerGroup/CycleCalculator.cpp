...
int64_t Bm168xCycleCalculator::getGlobalLayerCycle(Operation *op) {
  ...
  // SmallVector<Operation *> candidateOps;
  if (module::getChip() == module::Chip::BM1688) {
    auto parallels = createTempCoreParallelOp(op);
    candidateOps.append(parallels.begin(), parallels.end());
  }

  if (candidateOps.size() == 0) {
  ...
  if (module::getChip() == module::Chip::BM1688) {
    ...
    bool imp_multi_core_global = isa<tpu::Conv2DOp>(op) || candidateOps.size() > 1;
    float BW = 24;
  ...
  if (candidateOps.size() > 1) {
    removeTempCoreParallelOp(candidateOps);
  }
