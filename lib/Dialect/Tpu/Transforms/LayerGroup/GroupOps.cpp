...
void backward_collect_op(const Value &out, std::list<Value> &tensor_branchs,
...
void GroupOps::find_local_layer_base_group(Operation *op) {
  ...
  for (auto v : op->getOperands()) {
    ...
    auto itr = std::find(all_local_layer_nodes.begin(),
                         all_local_layer_nodes.end(), pre_op);
    if (itr == all_local_layer_nodes.end() && isLgSupport(pre_op) &&
        isPreOpHaveAComputeOp(pre_op)) { // isPreOpHaveAComputeOp ?? todo
      llvm::outs() << "i:" << i << ", op:" << module::getName(op).str()
                   << ", pre_op:" << module::getName(pre_op).str() << "\n";
  ...
  for (auto user : op->getUsers()) {
    auto itr = std::find(all_local_layer_nodes.begin(),
                         all_local_layer_nodes.end(), user);
    if (itr == all_local_layer_nodes.end() && isLgSupport(user)) {
      llvm::outs() << "i:" << i << ", op:" << module::getName(op).str()
                   << ", user:" << module::getName(user).str() << "\n";
...
GroupOps::GroupOps(::mlir::func::FuncOp func, int64_t opt) {
  ...
  if (branch_parallel) {
    ...
    func.walk([&](Operation *op) {
      if (isa<tpu::YieldOp, ReturnOp>(op)) {
        for (auto v : op->getOperands()) {
          ...
          auto itr = std::find(lg_pass_ir_->subnet_ops.begin(),
                               lg_pass_ir_->subnet_ops.end(), tmp_op);
    ...
    for (auto itr : out_var_def_ops) {
      auto itr2 = std::find(last_op_all_pre_ops.begin(),
                            last_op_all_pre_ops.end(), itr.second);
    ...
    for (auto v : lg_pass_ir_->subnet_return_opds) {
      llvm::outs() << "subnet_return_opds:" << i++
  ...
  func.walk([&](Operation *op) {
    if (isa<ReturnOp>(op)) {
      ...
      if (op->getResults().size() == 1 &&
          is_value_weight(op->getResults()[0])) {
...
typedef struct node_info {
  ...
  void show_info(std::string extra_info = "I am") {
    if (global_op) {
      LOG(INFO) << extra_info
                << " at global_op: " << module::getName(global_op).str();
    } else {
      std::string tmp_str = "";
      int i = 0;
      ...
      LOG(INFO) << extra_info
                << " at group:" << tmp_str.substr(3, tmp_str.size());
    }
  }
} node_info;
...
static void nodes_topo_order_dfs(node_info &cur_node,
  ...
  for (auto next_node : cur_node.next_nodes) {
    ...
    nodes.erase(std::remove(nodes.begin(), nodes.end(), &cur_node),
                nodes.end());
    // next_node->show_info("check next_node,
    // indeg:"+std::to_string(next_node->indeg));
  ...
  for (int64_t group_idx = 0; group_idx < group_num; group_idx++) {
    if (lg_infos[group_idx].group_ops.size() > 1 &&
      ...
      for (int pipe_id = 0;
        ...
        llvm::outs() << "tmp_need_store_load_value.size:"
      ...
      llvm::outs() << "group" << group_idx << ", group_id:" << lg_info.group_id
                   << ", in and out:\n";
      ...
      for (int pipe_id = 0;
        ...
        for (auto it : llvm::enumerate(groupOp.getResults())) {
          if (one_grp) {
            outputs[it.index()].replaceUsesWithIf(it.value(), [&](OpOperand &
                                                                      operand) {
              Operation *user = operand.getOwner();
              return find(ops.begin(), ops.end(), user) == ops.end();
            }); //旧的grp outputs用新的groupOp results替换，这些grp
                // output转为OpOperand，其拥有者不在grp中，即组外user的opd会替换
        ...
        for (size_t ts = 0, ts_id = 0; ts < ts_count; ts++, ts_id++) {
          ...
          if (ILP_time_step->inserted_timestep_table_.find(ts) !=
            ...
            CreateLmemMoveOp(ts_id,
                             ILP_time_step->inserted_timestep_table_[ts][0]);
            ts_id++;
          }
          LOG(INFO) << "----------------ts" << ts_id << "-----------------";
          for (auto it : ILP_time_step->timestep_table_new[ts].vec_ts_var) {
            if (it.var_value == 1) {
              ...
              if (it.info.mode2 & TIMESTEP2_LOAD) {
                ...
                auto storeOp_out =
                    CreateStoreOp2(it.value, it.info, ts_id, it.slice_idx,
                                   pipe_id, lg_info.type, can_merge);
...
void GroupOps::CreateLoadToL2mOp(int64_t ts, l2m_value_info &it,
  ...
  if (!it.valid) {
    llvm::outs() << "CreateLoadToL2mOp, name:" << in_name
  ...
  auto l2mem_s_addr =
      l2mem_alloc_ptr->vec_mem_alloc_his[key].vec_reload_addr[0].second.addr;
  attrs.push_back(builder.getNamedAttr(
      "l2m_addr", builder.getI64IntegerAttr(l2mem_s_addr)));
