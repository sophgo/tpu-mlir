...
static std::unique_ptr<std::vector<uint8_t> > packWeight(
              std::shared_ptr<std::vector<int32_t>> &bias,
              std::shared_ptr<std::vector<int64_t>> &rshift,
              std::shared_ptr<std::vector<int64_t>> &multiplier,
      ...
      for(int h_counter = 0;  h_counter < kh; h_counter++) {
        for(int w_counter = 0;  w_counter < kw; w_counter++) {
          for(int c_counter = 0;  c_counter < new_ic; c_counter++){
            uint32_t index_old = c_counter  +
                                 w_counter * kc +
                                 h_counter * kc * kw +
                                 n_counter * kh * kc * kw;
            uint32_t index_new = c_counter +
                                 w_counter * new_ic +
                                 h_counter * new_ic * kw +
...
void tpu::Conv2DOp::weight_reorder_int8_cv18xx() {
  ...
  auto packed_type = RankedTensorType::get(packedShape,
                                        builder.getIntegerType(8));
  auto pack_op = top::WeightOp::create(op, "pack", *packed, packed_type);
  ...
  std::vector<int64_t> filter_shape = {attr.oc, attr.ic / attr.groups,
                                       attr.kh, attr.kw};
  if (attr.dh || attr.dw) {
  ...
  auto weight_op = top::WeightOp::create(op, "lowered", *filter_i8, filter_type);
...
void tpu::Conv2DOp::codegen_global_cv18xx(void* ctx, int64_t layer_id) {
  ...
  gaddr_t ga_pc_info = Module::getAddress(bias());
  ...
  cvi_backend_tg_fixed_conv_kernel(
    *backend_ctx,
    ...
    attr.kh, attr.kw,
    attr.dh, attr.dw,
    attr.pht, attr.phb, attr.pwl, attr.pwr, // pad (t, b, l, r)
