...
#pragma omp parallel for schedule(static, omp_schedule(num_element))
    ...
    if (out_type.isInteger(8) || out_type.isInteger(16) || out_type.isInteger(32)){
      for (int i = 0; i < num_element; i++) {
          int64_t sum_res0 = temp_result0[i];
          sum_res0 = RightShiftRound(sum_res0, -mul1_shift, mul1_round_mode);
          if(out_type.isInteger(8)) temp_result0[i] = saturate_num<int16_t>(sum_res0);
          else if(out_type.isInteger(16)) temp_result0[i] = saturate_num<int32_t>(sum_res0);
          else temp_result0[i] = saturate_num<int32_t>(sum_res0);
      }
    }
    ...
    if (out_type.isInteger(8) || out_type.isInteger(16)||out_type.isInteger(32)){
      for (int i = 0; i < num_element; i++) {
          ...
          if(out_type.isInteger(8)) temp_result1[i] = saturate_num<int16_t>(sum_res1);
          else if(out_type.isInteger(16)) temp_result1[i] = saturate_num<int32_t>(sum_res1);
          else temp_result1[i] = saturate_num<int32_t>(sum_res1);
      }
    }
