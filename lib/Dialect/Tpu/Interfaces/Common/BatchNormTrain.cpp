...
static void normalize_f32(const float *input_data, float *output_data,
                         std::vector<float> mean, std::vector<float> var, float* running_mean, float* running_var, const float *scale_data, const float *shift_data, const float eps, const float momentum,const int batch_size, const int channels, const int spatial_size, int c) {
...
static void normalize_f16(const float *input_data, float *output_data,
  ...
  // Update running averages
  running_mean[c] = F16(momentum) * F16(running_mean[c]) + F16((1 - momentum)) * F16(mean[c]);
  running_var[c] = F16(momentum) * F16(running_var[c]) + F16((1 - momentum)) * F16(var[c]);

  // Normalize and scale/shift
  for (int b = 0; b < batch_size; ++b) {
    for (int s = 0; s < spatial_size; ++s) {
      const int index = b * channels * spatial_size + c * spatial_size + s;
      output_data[index] = F16((F16(input_data[index]) - F16(mean[c])) / F16(std::sqrt(F16(var[c]))));
      if (scale_data) {
...
LogicalResult tpu::BatchNormTrainOp::inference(InferenceParameter &p) {
  ...
  for(int ci = 0; ci < c; ci++){
    ...
    for(int ni = 0; ni < n; ni++){
      for(int hi = 0; hi < h; hi++){
        for(int wi = 0; wi < w; wi++){
          int index  = ni * c * h * w + ci * h * w + hi * w + wi;
          cur_var   += (input_data[index] - cur_save_mean) * (input_data[index] - cur_save_mean);
        }
      }
    }
    cur_var          /= n * h * w;
  ...
  // running mean var
  for(int ci = 0; ci < c; ci++){
    running_mean[ci] = (1 - momentum) * mean_data[ci] + momentum * saved_mean_data[ci];
  ...
  for(int ni = 0; ni < n; ni++){
    for(int ci = 0; ci < c; ci++){
      for(int hi = 0; hi < h; hi++){
        for(int wi = 0; wi < w; wi++){
          int index           = ni * c * h * w + ci * h * w + hi * w + wi;
          output_data[index]  = (input_data[index] - saved_mean_data[ci]) * saved_invstd[ci];
          output_data[index] *= gamma_data[ci];
          output_data[index] += beta_data[ci];
          if(do_relu){
            output_data[index] = output_data[index] > 0 ? output_data[index] : 0;
          }
        }
      }
    }
