...
LogicalResult tpu::MaxPoolWithMaskOp::inference(InferenceParameter &p) {
  pool_attr_t attrs;
  parseParam(&attrs);
  for (int64_t n = 0; n < attrs.n; ++n) {
    for (int64_t c = 0; c < attrs.c; ++c) {
      auto bottom_data = p.inputs[0] + (n * attrs.c + c) * attrs.ih * attrs.iw;
      auto top_data = p.outputs[0] + (n * attrs.c + c) * attrs.oh * attrs.ow;
      auto top_mask = p.outputs[1] + (n * attrs.c + c) * attrs.oh * attrs.ow;
