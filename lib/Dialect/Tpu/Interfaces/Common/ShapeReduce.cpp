...
LogicalResult tpu::ShapeReduceOp::init(InferenceParameter &p) { return success(); }
...
LogicalResult tpu::ShapeReduceOp::inference(InferenceParameter &p) {
  ...
  ASSERT_THIS(axes.size() == 1 && keep_dims && input_shape.size() == 1 && "ShapeReduce only support one dim and keep_dims now");
