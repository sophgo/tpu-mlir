...
LogicalResult tpu::FlexAttentionOp::init(InferenceParameter &p) {
  return success();
}

void tpu::FlexAttentionOp::deinit(InferenceParameter &p) {}

// Mirrors tpu::FAttentionLseOp::inference with the qk_d/v_d split (QK contracts
// over qk_d; AV produces v_d) and has_lse gating. block_bitmap is a kernel
// compute optimisation and is ignored by the reference (see top interface).
LogicalResult tpu::FlexAttentionOp::inference(InferenceParameter &p) {
...
mlir::Type tpu::FlexAttentionOp::type_verify(uint64_t opd_idx,
                                             TypeCastMode &mode) {
...
bool tpu::FlexAttentionOp::support_multi_core() { return true; }
