...
void tpu::MaxPool1DOp::parseParam(void *param) {
...
LogicalResult tpu::MaxPool1DOp::init(InferenceParameter &p) {
...
void tpu::MaxPool1DOp::deinit(InferenceParameter &p) {
  if (p.handle != nullptr) {
...
LogicalResult tpu::MaxPool1DOp::inference(InferenceParameter &p) {
  if (p.handle == nullptr) {
    return failure();
  }
  auto pooling = (Pooling *)p.handle;
...
LogicalResult tpu::MaxPool1DOp::LocalGenSupport() {
  auto stride = Module::getI64Array(strides());
  if (stride->at(0) > 15) {
    return failure();
  }
...
LogicalResult tpu::MaxPool1DOp::BackwardH(int64_t &in_idx, int64_t &in_slice,
