...
template <typename T1>
void tensor_interpolation(T1 *input, T1 *output, const int input_shape[4],
                          int stride_h, int stride_w) {
  int N = input_shape[0], C = input_shape[1], H = input_shape[2],
      W = input_shape[3];
    ...
    for (int c = 0; c < C; ++c) {
      for (int h = 0; h < H; h++) {
        for (int w = 0; w < W; w++) {
          ...
          int pos_output = n * C * H_out * W_out + c * H_out * W_out +
                           h * stride_h * W_out + w * stride_w;
...
void kernel_flip(T1 *input, T1 *output, const int kernel_shape[4]) {
  int oc = kernel_shape[0], ic = kernel_shape[1], kh = kernel_shape[2],
      kw = kernel_shape[3];
    ...
    for (int i_c = 0; i_c < ic; ++i_c) {
      for (int k_h = 0; k_h < kh; k_h++) {
        for (int k_w = 0; k_w < kw; k_w++) {
          ...
          int flippedIndex = o_c * ic * kh * kw + i_c * kh * kw +
                             (kh - 1 - k_h) * kw + (kw - 1 - k_w);
...
void conv_native_core(T2 *ofmap, const T1 *ifmap, const T4 *weight,
                      const T3 *bias, const T1 *pad_ins, int input_n,
                      int input_c, int input_h, int input_w, int output_c,
                      int groups, int kh, int kw, int dh, int dw, int ins_h,
                      int ins_w, int pht, int phb, int pwl, int pwr,
  ...
  int oh = (ih_ext - kh_ext) / stride_h + 1 +
           output_padding_h; // output_padding for convtranspose
  ...
  for (int n = 0; n < input_n; n++) {
    for (int g = 0; g < groups; g++) {
      for (int o = 0; o < o_g; o++) {
        // #ifdef _OPENMP
        // #pragma omp parallel for schedule(static, (oh * ow +
        // omp_get_num_threads() - 1) / omp_get_num_threads()) collapse(2)
        // #endif
        for (int y = 0; y < oh; y++) {
          for (int x = 0; x < ow; x++) {
            o_head = o_g * g;
            ...
            for (int k = 0; k < k_g; k++) {
              for (int p = 0; p < kh; p++) {
                for (int q = 0; q < kw; q++) {
                  ...
                  bool pos_ins =
                      ih_pos % (ins_h + 1) > 0 || iw_pos % (ins_w + 1) > 0;
...
LogicalResult tpu::ConvbwdOp::inference(InferenceParameter &p) {
  ...
  int kernel_shape[4] = {(int)attr.oc, (int)attr.ic, (int)attr.kh,
                         (int)attr.kw};
  int gradout_shape[4] = {(int)attr.n, (int)attr.oc, (int)attr.oh,
                          (int)attr.ow};
  int input_shape[4] = {(int)attr.n, (int)attr.ic, (int)attr.ih, (int)attr.iw};
  int kernel_grad_nc_origin_shape[4] = {(int)attr.ic, (int)attr.oc,
                                        (int)attr.kh, (int)attr.kw};
  ...
  if (cal_grad_weight) {
    ...
    conv_native_core(grad_weight_nc, input_nc, gradout_nc, &bias,
                     pad_ins_weight, attr.ic, attr.n, attr.ih, attr.iw, attr.oc,
                     1, /*group_num*/ attr.oh, attr.ow, attr.sh, attr.sw,
                     attr.insh, attr.insw, attr.pht, pad_h_b_new, attr.pwl,
                     pad_w_r_new, new_stride, new_stride, false, false);
    Tensor_nc_transpose(grad_weight_nc, gradweight,
                        kernel_grad_nc_origin_shape);
  }
  if (cal_grad_input) {
    kernel_flip(kernel, flip_kernel, kernel_shape);
    Tensor_nc_transpose(flip_kernel, flip_kernel_nc, kernel_shape);
    tensor_interpolation(gradout, gradout_interp, gradout_shape, attr.sh,
                         attr.sw);
    float bias = 0;
    ...
    int output_padding_h =
        attr.ih - (interp_h + pad_new_h_b + pad_new_h_t - attr.kh + 1);
    int output_padding_w =
        attr.iw - (interp_w + pad_new_w_l + pad_new_w_r - attr.kw + 1);
    conv_native_core(gradinput, gradout_interp, flip_kernel_nc, &bias,
                     pad_ins_input, attr.n, attr.oc, interp_h, interp_w,
                     attr.ic, 1, /*group_num*/ attr.kh, attr.kw, attr.dh,
                     attr.dw, attr.insh, attr.insw, pad_new_h_b, pad_new_h_t,
                     pad_new_w_l, pad_new_w_r, stride_new, stride_new, false,
                     false, output_padding_h, output_padding_w);
  }
  return success();
}
...
// struct Castfp32output : public OpRewritePattern<tpu::ConvbwdOp> {
//   Castfp32output(mlir::MLIRContext *context)
//       : OpRewritePattern<tpu::ConvbwdOp>(context) {}
//   LogicalResult
//   matchAndRewrite(tpu::ConvbwdOp op,
...
//     auto etype = target_type.getElementType();
//     if (etype.isF32()){
//       return failure();
//     }
...
//     for (int i = 0; i < n; i++) {
//       if (i==1){
//         auto shape = module::getShape(op.getGradWeight());
//         auto f32_type = RankedTensorType::get(shape, rewriter.getF32Type());
...
//     op.getOperation()->getAttrs()); return success();
//   }
// };

// void tpu::ConvbwdOp::getCanonicalizationPatterns(RewritePatternSet &results,
//                                                   MLIRContext *context) {
