...
int64_t tpu::ScaleOp::getBufferSize_bm1684(
...
void tpu::ScaleOp::codegen_local_bm1684(int64_t n_step, int64_t h_step,
  ...
  auto input_g_info = LocalGenInterface::getGroupInfo(getInput(), n_step, h_step);
  auto scale_g_info = LocalGenInterface::getGroupInfo(getScale(), n_step, h_step);
  int has_bias = !getBias().getType().isa<NoneType>();
  if (has_bias) {
  auto bias_g_info = LocalGenInterface::getGroupInfo(getBias(), n_step, h_step);
  ...
  if (module::isUniformQuantized(getOutput())) {
    ...
    auto lshift_g_info = LocalGenInterface::getGroupInfo(getLshift(), n_step, h_step);
...
int64_t tpu::ScaleOp::get_fw_type_bm1684() {
  return -1;
}
