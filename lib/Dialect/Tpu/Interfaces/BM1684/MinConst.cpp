...
void tpu::MinConstOp::codegen_global_bm1684() {
  int64_t n, c, h, w;
  ...
  if (module::isUniformQuantized(getInput())) {
    ...
    BM1684::instance().dl_nodechip_const_binary_fix8b_forward_parallel(
        ...
        module::getShape(getInput()).size(), BINARY_MIN, getMultiplier(), 1,
        getRshift(), 0,
    ...
    BM1684::instance().dl_nodechip_const_binary(
        ...
        BINARY_MIN, 0, getDoRelu(), getReluLimit().convertToDouble(),
        (CMD_ID_NODE *)BM1684::instance()->cmdid_node,
        module::getStorageType(getInput()).isa<IntegerType>());
  }
}

int64_t tpu::MinConstOp::getBufferSize_bm1684(
...
void tpu::MinConstOp::codegen_local_bm1684(int64_t n_step, int64_t h_step,
  ...
  if (module::isUniformQuantized(getOutput())) {
    ...
    BM1684::instance().dl_nodechip_const_binary_local(
        ...
        BINARY_MIN, 0, if_relu, relu_limit,
        (CMD_ID_NODE *)BM1684::instance()->bdc_node);
  }
}

uint32_t tpu::MinConstOp::dyn_codegen_global_bm1684(void *ir_layer_info) {
  GLOBAL_IR_COMMON(const_binary);
}

int64_t tpu::MinConstOp::get_fw_type_bm1684() { return FW_BMNET_CONST_BINARY; }

int32_t tpu::MinConstOp::dyn_codegen_local_bm1684(void *ir_layer_info) {
  int fw_ir_length = 0;
