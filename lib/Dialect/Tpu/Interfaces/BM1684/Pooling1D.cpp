...
void tpu::AvgPool1DOp::codegen_global_bm1684() {
  pool_attr_t attrs;
  parseParam(&attrs);
...
void tpu::MaxPool1DOp::codegen_global_bm1684() {
  pool_attr_t attrs;
  parseParam(&attrs);
  BM1684::instance().dl_nodechip_pooling_fix8b_forward_parallel_with_data_split(
      Module::getAddress(input()), Module::getAddress(output()), attrs.n,
...
int64_t tpu::AvgPool1DOp::getBufferSize_bm1684(
    int64_t in_lmem_bytes, int64_t out_lmem_bytes, int64_t in_nslice,
    int64_t in_hslice, int64_t out_nslice, int64_t out_hslice) {
  return 0;
}

int64_t tpu::MaxPool1DOp::getBufferSize_bm1684(
    int64_t in_lmem_bytes, int64_t out_lmem_bytes, int64_t in_nslice,
    int64_t in_hslice, int64_t out_nslice, int64_t out_hslice) {
  return 0;
}

void tpu::AvgPool1DOp::codegen_local_bm1684(int64_t n_step, int64_t h_step) {
  llvm_unreachable("support later");
}

void tpu::MaxPool1DOp::codegen_local_bm1684(int64_t n_step, int64_t h_step) {
  llvm_unreachable("support later");
}
