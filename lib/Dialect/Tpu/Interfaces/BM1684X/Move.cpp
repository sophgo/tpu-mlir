...
void move_slice(int64_t src_add, int64_t dest_add, int64_t size, DATA_TYPE_T dtype) {
  tensor_spec_t spec;
  ...
  spec.shape[2] = 1;
  spec.shape[3] = size/BM168x::getFmtBytes(dtype); //todo 搬移的参数具有不同的dtype
  ...
  BM168x::call_local_func("backend_api_bdc_cpy_local", &param,
                          sizeof(param), nullptr, input_spec.data(),
                          output_spec.data());
}
...
void tpu::MoveOp::codegen_only_for_moveOp(std::vector<int64_t>& move_src_add, std::vector<int64_t>& move_dest_add,
  ...
  for (int i = 0; i < move_num; i++) {
    ...
    //不重叠的情形
    bm168x->divide_sync_id();
    BM168x::instance()->dl_set_cmd_id_prefix(pid_node, gen_op_id(op, i).c_str());
