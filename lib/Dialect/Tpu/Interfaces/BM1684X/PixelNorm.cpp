...
int64_t tpu::PixelNormOp::getBufferSize_bm1684x(
  ...
  return BM168x::call_local_bfsz_func("backend_api_pixel_norm_local_bfsz", &param, sizeof(param),
                                      &sec_info, input_spec->data(), output_spec->data());
}

void tpu::PixelNormOp::codegen_local_bm1684x(int64_t n_step, int64_t h_step, int64_t d_step, int64_t w_step,
                                             group_type_t group_type,
                                             local_sec_info_t &sec_info) {
  auto op = getOperation();
  auto input_spec = BM168x::get_input_spec(op);
...
int64_t tpu::PixelNormOp::get_fw_type_bm1684x() {
