...
void tpu::StoreOp::codegen_local_bm1684x(int64_t n_step, int64_t h_step, int64_t d_step, int64_t w_step,
                                         group_type_t group_type,
    ...
    for (int64_t d = 0; d < real_dslice; d++) {
      ...
      while (channel_index < csecs) {
        ...
        BM168x::instance()->dl_tensor_stride_move_gen_cmd(
            ...
            g_addr + cur_global_offset, gi.n_slice, real_cslice, real_hslice, real_wslice,
            c_num_local * c_stride, c_stride, real_wslice, 1, C * D * H * W, D * H * W, W,
            1, gdma_format,
