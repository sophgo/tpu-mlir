...
static void filter_merge(std::shared_ptr<std::vector<T>> &filter,
                         ...
                         std::shared_ptr<std::vector<T>> &R,
                         int num_dir, int input_size, int hidden_size) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int h = 0; h < hidden_size; h++) {
        for (int x = 0; x < input_size; x++) {
          int src_offset = d * w_size + i * input_size * hidden_size + h * input_size + x;
          int dst_offset = d * (w_size + r_size) + i * input_size * hidden_size + x * hidden_size + h;
    ...
    for (int i = 0; i < 4; i++) {
      for (int h = 0; h < hidden_size; h++) {
        for (int x = 0; x < hidden_size; x++) {
          int src_offset = d * r_size + i * hidden_size * hidden_size + h * hidden_size + x;
          int dst_offset = d * (w_size + r_size) + 4 * w_size + i * hidden_size * hidden_size + x * hidden_size + h;
...
static void filter_reorder(std::shared_ptr<std::vector<T>> &filter,
                           int offset, int xsize, int hsize) {
...
void tpu::LSTMOp::weight_reorder_f32_bm1684x() {
  ...
  auto filter_merged = std::make_shared<std::vector<float>>(Module::getNumElements(filter()) + Module::getNumElements(recurrence()), 0);
  filter_merge(filter_merged, filter_f32, recurrence_f32, num_dir, input_size, hidden_size);

  int filter_offset = 0;
  for (int i = 0; i < num_layers() * num_dir; i++) {
    //trans w_x
    filter_reorder(filter_merged, filter_offset, i < num_dir ? input_size : hidden_size * num_dir, hidden_size);
    filter_offset += (i < num_dir ? input_size : hidden_size * num_dir) * hidden_size * 4;
    //trans w_h
    filter_reorder(filter_merged, filter_offset, hidden_size, hidden_size);
    filter_offset += hidden_size * hidden_size * 4;
  }
  std::vector<int64_t> filter_reordered_shape = {num_dir, (in0_c * in0_h + in1_c * in1_h) / hidden_size, hidden_size};
  auto filter_type = Module::getStorageType(filter());
  auto new_filter_type = RankedTensorType::get(filter_reordered_shape, filter_type);
  auto newFilterOp =
      top::WeightOp::create(op, "reordered_filter", *filter_merged, new_filter_type);
  ...
  if (!has_initial_h) {
    ...
    auto initial_h = std::make_shared<std::vector<float>>(num_dir * batch_size * hidden_size, 0.0f);
    auto new_initial_h_type = RankedTensorType::get(initial_hc_shape, initial_h_type);
    auto initial_h_Op =
  ...
  if (!has_initial_c) {
    ...
    auto initial_c = std::make_shared<std::vector<float>>(num_dir * batch_size * hidden_size, 0.0f);
    auto new_initial_c_type = RankedTensorType::get(initial_hc_shape, initial_c_type);
   auto initial_c_Op =
...
void tpu::LSTMOp::codegen_global_bm1684x() {
  ...
  p.hn_global_addr = p.y_global_addr + Module::getNumElements(output()) * sizeof(float);
  p.cn_global_addr = p.hn_global_addr + Module::getNumElements(initial_h()) * sizeof(float);
  p.z_global_addr = p.cn_global_addr + Module::getNumElements(initial_c()) * sizeof(float);
  p.bias = have_bias();
