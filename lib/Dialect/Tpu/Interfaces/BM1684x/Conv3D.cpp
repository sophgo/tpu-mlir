...
void tpu::Conv3DOp::weight_reorder_int8_bm1684x() {
  conv_attr_t attr = {0};
  parseParam(&attr);
  // filter
...
void tpu::Conv3DOp::weight_reorder_bf16_bm1684x() {
  conv_attr_t attr = {0};
  parseParam(&attr);
  auto filterOp = filter().getDefiningOp<top::WeightOp>();
...
void tpu::Conv3DOp::weight_reorder_f16_bm1684x() {
  weight_reorder_bf16_bm1684x();
}

void tpu::Conv3DOp::weight_reorder_f32_bm1684x() {
  conv_attr_t attr = {0};
  parseParam(&attr);
  auto op = getOperation();
...
void tpu::Conv3DOp::codegen_global_bm1684x() {
  ...
  BM1684x::instance().call_global_func("backend_api_conv3d_global", &spec,
                                       sizeof(spec));
}
...
int64_t tpu::Conv3DOp::getBufferSize_bm1684x(
  ...
  parseParam(&attr);
  int64_t sz = 0;
  const int64_t npu_num = BM1684x::NPU_NUM;
  ...
  if ((in_type.isF16() || in_type.isBF16()) && !out_type.isF32() &&
    ...
    sz += (oc_per_npu *
           align_up(out_hslice * attr.ow,
                    BM1684x::instance().get_eu_num(sizeof(float))) *
           sizeof(float));
  }
  ...
  if ((in_type.isF16() || in_type.isBF16()) && attr.groups > 1) {
    sz += ceiling_func((int64_t)attr.ic / attr.groups, npu_num) *
          align_up(in_hslice * attr.iw,
                   BM1684x::instance().get_eu_num(sizeof(int16_t))) *
          sizeof(int16_t);
  }
  if ((in_type.isInteger(8)) && attr.groups > 1) {
    sz += ceiling_func((int64_t)attr.ic / attr.groups, npu_num) *
          align_up(in_hslice * attr.iw,
                   BM1684x::instance().get_eu_num(sizeof(int8_t))) *
          sizeof(int8_t);
  }
  return sz;
}
...
void tpu::Conv3DOp::codegen_local_bm1684x(int64_t n_step, int64_t h_step) {
  ...
  BM1684x::instance().call_local_func("backend_api_conv3d_local", &spec,
