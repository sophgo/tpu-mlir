    ...
    for (int ci = 0; ci < c; ci++) {
      for (int hi = 0; hi < cur_h; hi++) {
        for (int wi = 0; wi < cur_w; wi++) {
          int index = ni * c * cur_h * cur_w + ci * cur_h * cur_w + hi * cur_w + wi;
...
void top::MaxPoolingIndicesBwdOp::shape_inference() {
    auto input_shape = module::getI64Array(getInputShape());
    std::vector<int64_t> output_shape = {input_shape->at(0), input_shape->at(1), input_shape->at(2), input_shape->at(3)};
