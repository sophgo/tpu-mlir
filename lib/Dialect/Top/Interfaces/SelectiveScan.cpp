...
LogicalResult top::SelectiveScanOp::init(InferenceParameter &p) {
  return success();
}

void top::SelectiveScanOp::deinit(InferenceParameter &p) {}

LogicalResult top::SelectiveScanOp::inference(InferenceParameter &p) {
...
int64_t top::SelectiveScanOp::getFLOPs() {
  auto deltaA_shape = module::getShape(getDeltaA());
  int64_t Kcdim = deltaA_shape[1];
  int64_t L = deltaA_shape[2];
  int64_t Batch = deltaA_shape[3];
  bool has_D = !getDs().getType().isa<NoneType>();
  int64_t flops = has_D ? 5 * L * Kcdim * Batch : 3 * L * Kcdim * Batch;
  return flops;
}

void top::SelectiveScanOp::shape_inference() {
  auto Cs_shape = module::getShape(getCs());

  module::setShapeOrVerify(getOutput(),
                           {Cs_shape[0], Cs_shape[1], Cs_shape[3]});
}
