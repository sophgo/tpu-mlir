...
int64_t top::TriluOp::getFLOPs() { return module::getNumElements(getOutput()); }

LogicalResult top::TriluOp::init(InferenceParameter &p) { return success(); }
void top::TriluOp::deinit(InferenceParameter &p) {}

LogicalResult top::TriluOp::inference(InferenceParameter &p) {
  auto upper = getUpper();
...
void top::TriluOp::shape_inference() { common_shape_inference(getOperation()); }
