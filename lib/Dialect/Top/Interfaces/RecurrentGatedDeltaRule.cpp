...
int64_t top::RecurrentGatedDeltaRuleOp::getFLOPs() {
  auto v_shape = module::getShape(getValue());
  int64_t B = v_shape[0];
  int64_t num_v_heads = getNumVHeads();
  int64_t d = getD();
  // state*g decay, kv_mem (state^T@key), state update (outer product), output
  // (state^T@query)
  return B * num_v_heads * (7 * d * d + 2 * d);
}

LogicalResult top::RecurrentGatedDeltaRuleOp::init(InferenceParameter &p) {
  return success();
}
void top::RecurrentGatedDeltaRuleOp::deinit(InferenceParameter &p) {}

LogicalResult top::RecurrentGatedDeltaRuleOp::inference(InferenceParameter &p) {
  // Input shapes:
...
void top::RecurrentGatedDeltaRuleOp::shape_inference() {

  auto shape = module::getShape(getValue());
  std::vector<int64_t> o_shape = {shape[0], 1, (int64_t)getNumVHeads(),
                                  (int64_t)getD()};
  module::setShapeOrVerify(getAttnOut(), o_shape);
}
