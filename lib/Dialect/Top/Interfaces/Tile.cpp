...
LogicalResult top::TileOp::inference(InferenceParameter &p) {
  ...
  for(int i = 0; i < last_i + 1; ++i){
    if(tile_vec[i] == 1)
      continue;
    int len = std::accumulate(tmp_shape.begin(),tmp_shape.end(), 1, std::multiplies<int64_t>());
    float* cur_input = new float[len];
    std::copy(last_op, last_op + len, cur_input);
    tile(cur_input, p.outputs[0], llvm::ArrayRef<int64_t>(tmp_shape.data(), tmp_size),
          i, tile_vec[i]);
    last_op = p.outputs[0];
...
void top::TileOp::shape_inference() {
  ...
  if (getTile().has_value()){
      auto tile_v = module::getI64Array(getTile().value());
      tile_vec = *tile_v;
  } else if (auto tile_w = dyn_cast<top::WeightOp>(getTileT().getDefiningOp())){
