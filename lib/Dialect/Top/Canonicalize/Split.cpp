...
struct SplitReshape2ReshapeSplit : public OpRewritePattern<SplitOp> {
  using OpRewritePattern::OpRewritePattern;
  SplitReshape2ReshapeSplit(MLIRContext *context, PatternBenefit benefit = 2)
      : OpRewritePattern<SplitOp>(context, benefit) {}
  ...
  LogicalResult matchAndRewrite(SplitOp op,
    ...
    for (auto out : op.getResults()){
        ...
        if (out == *(op.getResults().begin())) last_RO_shape = RO_shape;
        else{
    ...
    if (axis_shape != in_shape[axis]) return failure();
    ...
    for(auto v : last_RO_shape){shape.push_back(v);}
    shape[new_axis] = in_shape[axis];
    attrs.emplace_back(
            rewriter.getNamedAttr("shape", rewriter.getI64ArrayAttr(shape)));
    // Attention!!!!
    // when create OP, the Type is return Type. And Type is not only data type(int, float..), it contains shape info.
    auto reshape_otype = RankedTensorType::get(shape, module::getElementType(input));
    auto reshape_op = rewriter.create<top::ReshapeOp>(loc, reshape_otype, ValueRange{input}, attrs);
    ...
    for (auto out : split_op.getResults()){
         ReshapeOp rop = dyn_cast<ReshapeOp> (*out.user_begin());
         // Attention !!!
         // if not setType, will cause ERROR such like "doesn't match function result type"
...
void SplitOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                          MLIRContext *context) {
  results.insert<SplitToSlice,
                 SplitReshape2ReshapeSplit>(context);
}
