...
struct TopFuseTransformer : public OpRewritePattern<TransformerOp> {
  ...
  LogicalResult matchAndRewrite(TransformerOp op,
    ...
    for (int i = 0; i < head; ++i) {
      ...
      // auto weight_o = get_weight(op.getOutWeight(), head, i, 0, rewriter.getF32Type(), "weight");
      // auto bias_q = get_weight(op.getQueriesBias(), head, i, 0, rewriter.getF32Type(), "bias");
      // auto bias_k = get_weight(op.getKeysBias(), head, i, 0, rewriter.getF32Type(), "bias");
      // auto bias_v = get_weight(op.getValuesBias(), head, i, 0, rewriter.getF32Type(), "bias");
