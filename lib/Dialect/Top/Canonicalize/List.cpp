 ...
 * List  => Reshape + Concat
 **/
struct TopListReplace : public OpRewritePattern<ListOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(top::ListOp op,
    ...
    for (int i = 0; i < num_inputs; i++){
      ...
      auto reshape_loc = NameLoc::get(rewriter.getStringAttr(in_name + "_reshape"));
      auto reshape_type = RankedTensorType::get(in_shape, module::getElementType(in_op));
    ...
    concat_attrs.emplace_back(rewriter.getNamedAttr("axis", rewriter.getSI32IntegerAttr(0)));
    auto out_ele = module::getNumElements(op.getOutput());
    std::vector<int64_t> out_shape = {out_ele};
    auto concat_type = RankedTensorType::get(out_shape, module::getElementType(op.getInputs()[0]));
    auto concat_loc = NameLoc::get(rewriter.getStringAttr(op_name + "_concat"));
