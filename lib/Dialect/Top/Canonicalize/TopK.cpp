...
//1. Values and Indices only have one SliceOp user, and the parameters of the two slices equal
...
private:
  LogicalResult slice2k(TopKOp op, SliceOp slice_op, PatternRewriter &rewriter, int64_t axis) const{
    ...
    std::string indices_name = module::getName(op.getIndices()).str()+"_Slice";
    ...
    //reverse the order if slice the end of the array
        ...
        attrs.push_back(rewriter.getNamedAttr("axis", rewriter.getI64IntegerAttr(axis)));

        auto slice_indices_op = dyn_cast_or_null<SliceOp>(*op.getIndices().getUsers().begin());
        auto slice_values_op = dyn_cast_or_null<SliceOp>(*op.getValues().getUsers().begin());
        rewriter.setInsertionPointAfter(op);
        if(slice_indices_op && slice_values_op){
            rewriter.replaceOpWithNewOp<ReverseOp>(slice_indices_op, slice_indices_op.getResult().getType(),
                                                  slice_indices_op.getInput(), attrs);
            rewriter.replaceOpWithNewOp<ReverseOp>(slice_values_op, slice_values_op.getResult().getType(),
                                                  slice_values_op.getInput(), attrs);

        }else{
            rewriter.replaceOpWithNewOp<ReverseOp>(slice_op, slice_op.getResult().getType(),
  ...
  LogicalResult which_axes(TopKOp op, SliceOp slice_op, PatternRewriter &rewriter) const{
  ...
  LogicalResult compare_slice(TopKOp op, PatternRewriter &rewriter) const{
    ...
    if (slice_indices_op || slice_values_op){
        ...
        if(slice_indices_op && slice_values_op){
            ...
            if((module::getShape(slice_indices_op) == module::getShape(slice_values_op))
            & (slice_indices_op.getHasparamConvertAxesAttr() == slice_values_op.getHasparamConvertAxesAttr())
