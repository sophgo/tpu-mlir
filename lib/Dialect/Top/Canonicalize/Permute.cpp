...
struct PermutePadSwap : public OpRewritePattern<PermuteOp> {
  ...
  LogicalResult matchAndRewrite(PermuteOp op,
    ...
    for (size_t i = 0; i < num_axis; ++i) {
      new_padded_shape[i] = in_shape[i] + new_paddings[i] + new_paddings[i + num_axis];
    }
...
void _permute_binary_rewrite(PermuteOp pm_op1, PermuteOp pm_op2, OpTy bi_op, PatternRewriter &rewriter)
{
  ...
  auto loc = NameLoc::get(
    rewriter.getStringAttr(module::getName(bi_op.getOutput()).str() + "_PermuteBinary"));
  ...
  auto new_bi_op = rewriter.create<OpTy>(
    loc, new_bi_out_type, opds_bi, attrs_bi);
  std::vector<NamedAttribute> attrs_pm;
  attrs_pm.emplace_back(
    rewriter.getNamedAttr("order", rewriter.getI64ArrayAttr(order)));
  rewriter.replaceOpWithNewOp<PermuteOp>(
    bi_op, bi_op.getResult().getType(), new_bi_op.getOutput(), attrs_pm);
}
...
struct PermuteBinaryPattern : public OpRewritePattern<PermuteOp> {
  ...
  LogicalResult matchAndRewrite(PermuteOp op,
    ...
    for (auto opd : p_next_op->getOperands()) {
      ...
      if (op_ == op.getOperation()) continue;
      if (!opd.hasOneUse()) {
        return failure();
      }
...
void PermuteOp::getCanonicalizationPatterns(RewritePatternSet &results,
  ...
  results.insert<TopPermuteToPixelShuffle, TopPermuteToReorg, Permute5dSplit,
                 ...
                 NonZeroPermutePattern, PermutePadSwap, PermuteBinaryPattern>(context);
}
