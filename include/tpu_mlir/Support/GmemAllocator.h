...
class GmemAllocator {
public:
  GmemAllocator(std::map<ValueInfo, int64_t> &gaddrMap,
  ...
  static void markGmemReusedOp(std::vector<ValueInfo> &ops,
                               std::map<ValueInfo, int64_t> &gaddrMap,
                               std::map<ValueInfo, TensorLive> &liveRange,
                               std::set<ValueInfo> &gmemReusedSet,
                               uint32_t alignment);
  static int64_t assignSpecifiedGmemToOp(Operation *op,
                                         std::map<ValueInfo, int64_t> &gaddrMap,
                                         int64_t baseGaddr, uint32_t alignment);
