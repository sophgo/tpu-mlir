...
struct dot_graph {
  ...
  void add_edge_into_graph(std::string from_node_name,
  ...
  std::string export_dot(std::string file_name, bool always_save = false) {
    ...
    // std::string flag_str = "_bwd.dot.svg";
    // if(file_name.find(flag_str) == std::string::npos ) {
    //   return "";
    // }
    ...
    for (auto itr : main_graph_nodes) {
      ...
      log_stream << llvm::formatv(" {0} [label= \"{0}\\n{1}\"];",
                                  itr.first.c_str(), tmp_str)
                        .str()
                 << std::endl;
    }
    for (auto itr : main_graph_edges) {
      for (auto itr2 : itr.second) {
        // log_stream << llvm::formatv(" {0} -> {1} [label= \"{0}\"];",
        // itr.first.c_str(), itr2.c_str()).str() <<std::endl;
        log_stream << llvm::formatv(" {0} -> {1};", itr.first.c_str(),
                                    itr2.c_str())
                          .str()
                   << std::endl;
      }
    ...
    log_stream << "}" << std::endl;
    log_stream.close();
    system(llvm::formatv("dot -Tsvg {0} -o ./{0}.svg && rm -f {0}",
                         file_name.c_str())
               .str()
               .c_str());
