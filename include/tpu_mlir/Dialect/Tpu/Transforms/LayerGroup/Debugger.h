...
typedef enum {
  ...
  DEBUGGER_DUMP_GIVEN = 1,  /* dump debug infos only for given groups in 'conditional_debug_groups' */
  DEBUGGER_DUMP_MIXED = 2,  /* dump debug infos with type in 'conditional_debug_types'
...
public:
  ...
  void set_type(lg_debugger_type_t type) {
    this->type_ = type;
  }
  ...
  lg_debugger_type_t get_type() {
    return type_;
  }

  std::string get_type_str() {
    return lg_debugger_type_str_map[type_];
  }

  void set_sc_method(lg_sc_method_t method) {
    this->sc_method_ = method;
  }

  void set_sc_method(std::string method) {
    if (lg_sc_method_str_type_map.find(method) != lg_sc_method_str_type_map.end()) {
  ...
  void set_do_debug(bool do_debug) {
    this->do_debug_ = do_debug;
  }
  ...
  bool get_show_function_location() {
    return show_function_location_;
  }
  ...
  std::unordered_set<std::pair<int64_t, int64_t>, PairHash> get_conditional_debug_groups() {
    return conditional_debug_groups_;
  }

  bool is_conditional_debug_group(int64_t start, int64_t end) {
    return conditional_debug_groups_.find(std::make_pair(start, end)) != conditional_debug_groups_.end();
  }
  ...
  bool is_conditional_debug_type(std::string type) {
    return conditional_debug_types_.find(type) != conditional_debug_types_.end();
  }
  ...
  template <typename F>
  void debug_with_type(const char* debug_type,
    ...
    DEBUG_WITH_TYPE(debug_type, {
      llvm::dbgs() << LOG_ACTION(debug_type)
                   << LOG_RANGE(range);
  ...
  void debug_with_type(const char* debug_type, const LgInfo& lg_info,
    ...
    const auto debug_group = std::make_pair(lg_info.func_start_idx, lg_info.func_end_idx);
    ...
    auto should_dump = [&]() {
      switch (type_) {
        case DEBUGGER_DUMP_ALL:    return true;
        case DEBUGGER_DUMP_GIVEN:  return find_debug_group;
        case DEBUGGER_DUMP_MIXED:  return find_debug_type ? find_debug_group : true;
    ...
    auto get_range_str = [&]() {
      switch (type_) {
        case DEBUGGER_DUMP_ALL:    return "all";
        case DEBUGGER_DUMP_GIVEN:  return "given";
        case DEBUGGER_DUMP_MIXED:  return find_debug_group ? "given" : "all";
        default:                   return "all";
      }
    }();
...
private:
  ...
  std::unordered_set<std::pair<int64_t, int64_t>, PairHash> conditional_debug_groups_;
