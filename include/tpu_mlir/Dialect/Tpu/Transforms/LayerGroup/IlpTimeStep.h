...
// private:
  ...
  bool alloc_multi(int ts_idx, int op_idx, std::vector<mem_alloc_req_info>& vec_mem_req, int& lack_mem_szie, bool sort_by_size);
...
public:
  ...
  void addRowConstraint(int ts_idx, Value value, std::vector<std::string> var_names, bool store, bool load_to_l2m);
