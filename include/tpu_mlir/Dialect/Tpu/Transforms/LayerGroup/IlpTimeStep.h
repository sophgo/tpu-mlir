...
public:
  ...
  bool _alloc(int slice_idx, const std::string &name, Value value, int size,
              ...
              bool force_not_care_bank = false, bool isBuffer = false,
              bool for_move = false);
  bool alloc_multi(int ts_idx, std::vector<mem_alloc_req_info> &vec_mem_req,
                   bool sort_by_size);
...
public:
  ...
  // void timestep_assignment_by_ilp(BasicTimeStep *time_step, TensorInfo
  // &tensor_infos);
  int get_tensor_load_pos(const tensor_info_t &tensor_info,
                          std::string &var_name);
  void get_group_cycle_info(
