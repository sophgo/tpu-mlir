...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT]]#0, %arg1, %0, %0) {{{.*}}} : (tensor<3x6xf32>, tensor<6x7xf32>, none, none) -> tensor<3x7xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT]]#1, %arg1, %0, %0) {{{.*}}} : (tensor<2x6xf32>, tensor<6x7xf32>, none, none) -> tensor<2x7xf32> loc({{.*}})
module @MatMul1 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<5x6xf32> loc("a"), %arg1: tensor<6x7xf32> loc("b")) -> tensor<5x7xf32> {
    ...
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<5x6xf32>, tensor<6x7xf32>, none, none) -> tensor<5x7xf32> loc("c")
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%arg0, %arg1, %0, %0) {{{.*}}} : (tensor<1x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<1x4096xf32> loc({{.*}})
module @MatMul2.1 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<1x512x7x7xf32> loc("a"), %arg1: tensor<25088x4096xf32> loc("b")) -> tensor<1x4096xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<1x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<1x4096xf32> loc("c")
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT]]#0, %arg1, %0, %0) {{{.*}}} : (tensor<2x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<2x4096xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT]]#1, %arg1, %0, %0) {{{.*}}} : (tensor<1x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<1x4096xf32> loc({{.*}})
module @MatMul2.2 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<3x512x7x7xf32> loc("a"), %arg1: tensor<25088x4096xf32> loc("b")) -> tensor<3x4096xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<3x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<3x4096xf32> loc("c")
...
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%arg0, %arg1, %0, %0) {{{.*}}} : (tensor<3x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<3x4096xf32> loc({{.*}})
module @MatMul2.3 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<3x512x7x7xf32> loc("a"), %arg1: tensor<25088x4096xf32> loc("b")) -> tensor<3x4096xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) {left_transpose=true}: (tensor<3x512x7x7xf32>, tensor<25088x4096xf32>, none, none) -> tensor<3x4096xf32> loc("c")
    return %1 : tensor<3x4096xf32> loc("d")
  }
}
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT1]]#0, %[[SPLIT2]]#0, %0, %0) {{{.*}}} : (tensor<1x2x5x6xf32>, tensor<1x2x6x7xf32>, none, none) -> tensor<1x2x6x7xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT1]]#1, %[[SPLIT2]]#1, %0, %0) {{{.*}}} : (tensor<1x2x5x6xf32>, tensor<1x2x6x7xf32>, none, none) -> tensor<1x2x6x7xf32> loc({{.*}})
module @MatMul3.1 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<1x4x5x6xf32> loc("a"), %arg1: tensor<1x4x6x7xf32> loc("b")) -> tensor<1x4x6x7xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<1x4x5x6xf32>, tensor<1x4x6x7xf32>, none, none) -> tensor<1x4x6x7xf32> loc("c")
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT1]]#0, %[[SPLIT2]]#0, %0, %0) {{{.*}}} : (tensor<2x5x4x6xf32>, tensor<2x6x4x7xf32>, none, none) -> tensor<2x6x4x7xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT1]]#1, %[[SPLIT2]]#1, %0, %0) {{{.*}}} : (tensor<1x5x4x6xf32>, tensor<1x6x4x7xf32>, none, none) -> tensor<1x6x4x7xf32> loc({{.*}})
module @MatMul3.2 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<3x5x4x6xf32> loc("a"), %arg1: tensor<3x6x4x7xf32> loc("b")) -> tensor<3x6x4x7xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) {hdim_is_batch=true} : (tensor<3x5x4x6xf32>, tensor<3x6x4x7xf32>, none, none) -> tensor<3x6x4x7xf32> loc("c")
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT1]]#0, %arg1, %0, %0) {{{.*}}} : (tensor<2x5x6xf32>, tensor<6x7xf32>, none, none) -> tensor<2x6x7xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT1]]#1, %arg1, %0, %0) {{{.*}}} : (tensor<2x5x6xf32>, tensor<6x7xf32>, none, none) -> tensor<2x6x7xf32> loc({{.*}})
module @MatMul4 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<4x5x6xf32> loc("a"), %arg1: tensor<6x7xf32> loc("b")) -> tensor<4x6x7xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<4x5x6xf32>, tensor<6x7xf32>, none, none) -> tensor<4x6x7xf32> loc("c")
...
// CHECK:           %[[SPLIT1:.*]]:2 = "tpu.Split"(%arg0) : (tensor<4x5x6xf32>) -> (tensor<2x5x6xf32>, tensor<2x5x6xf32>) loc({{.*}})
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%[[SPLIT1]]#0, %arg1, %0, %0) {{{.*}}} : (tensor<2x5x6xf32>, tensor<6xf32>, none, none) -> tensor<2x6xf32> loc({{.*}})
// CHECK:           %[[MAMUL2:.*]] = "tpu.MatMul"(%[[SPLIT1]]#1, %arg1, %0, %0) {{{.*}}} : (tensor<2x5x6xf32>, tensor<6xf32>, none, none) -> tensor<2x6xf32> loc({{.*}})
module @MatMul5 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<4x5x6xf32> loc("a"), %arg1: tensor<6xf32> loc("b")) -> tensor<4x6xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<4x5x6xf32>, tensor<6xf32>, none, none) -> tensor<4x6xf32> loc("c")
...
// CHECK:           %[[MAMUL1:.*]] = "tpu.MatMul"(%arg0, %arg1, %0, %0) {{{.*}}} : (tensor<4096xf32>, tensor<4096x12884xf32>, none, none) -> tensor<1x12884xf32> loc({{.*}})
module @MatMul6 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<4096xf32> loc("a"), %arg1: tensor<4096x12884xf32> loc("b")) -> tensor<1x12884xf32> {
    %0 = "top.None"() : () -> none loc(unknown)
    %1 = "tpu.MatMul"(%arg0, %arg1, %0, %0) : (tensor<4096xf32>, tensor<4096x12884xf32>, none, none) -> tensor<1x12884xf32> loc("c")
