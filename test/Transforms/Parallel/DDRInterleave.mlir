...
// CHECK:    %[[MatMul:.*]] = "tpu.MatMul"(%arg0, %[[Weight]], %0) {{.*}} : (tensor<256x25088xf32>, tensor<25088x4096xf32, #tpu.ddr_interleave<{map = (d0, d1) -> (8, d0, d1 ceildiv 8), interleave_dim = [0], address = 0}>>, none) -> tensor<256x4096xf32, #tpu.ddr_interleave<{map = (d0, d1) -> (8, d0, d1 ceildiv 8), interleave_dim = [0], address = 0}>>
// CHECK:    %[[Out:.*]] = "tpu.MatMul"(%[[MatMul]], %[[MatMul]], %0)
...
module @MatMul2.0 attributes {module.chip = "bm1688", module.cores = 2 : i64, module.mode = "F32", module.platform = "ONNX", module.state = "TPU_LOWERED", module.weight_file = ""} {
  func.func @main(%arg0: tensor<256x25088xf32> loc("a")) -> tensor<256x256xf32> {
    ...
    %2 = "tpu.MatMul"(%arg0, %1, %0) : (tensor<256x25088xf32>, tensor<25088x4096xf32>, none) -> tensor<256x4096xf32> loc("c")
    %3 = "tpu.MatMul"(%2, %2, %0) {right_transpose=true} : (tensor<256x4096xf32>, tensor<256x4096xf32>, none) -> tensor<256x256xf32> loc("d")
